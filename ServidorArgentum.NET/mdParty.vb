...
Module mdParty
	...
	Public Function PuedeCrearParty(ByVal UserIndex As Short) As Boolean
		...
		If CShort(UserList(UserIndex).Stats.UserAtributos(Declaraciones.eAtributos.Carisma)) * UserList(UserIndex).Stats.UserSkills(Declaraciones.eSkill.Liderazgo) < 100 Then
			...
			If .PartyIndex = 0 Then
				If .flags.Muerto = 0 Then
					If .Stats.UserSkills(Declaraciones.eSkill.Liderazgo) >= 5 Then
						...
						If tInt = -1 Then
							Call WriteConsoleMsg(UserIndex, "Por el momento no se pueden crear más parties.", Protocol.FontTypeNames.FONTTYPE_PARTY)
							Exit Sub
						Else
							Parties(tInt) = New clsParty
							If Not Parties(tInt).NuevoMiembro(UserIndex) Then
								Call WriteConsoleMsg(UserIndex, "La party está llena, no puedes entrar.", Protocol.FontTypeNames.FONTTYPE_PARTY)
							...
							Else
								...
								If Not Parties(tInt).HacerLeader(UserIndex) Then
									Call WriteConsoleMsg(UserIndex, "No puedes hacerte líder.", Protocol.FontTypeNames.FONTTYPE_PARTY)
								Else
									Call WriteConsoleMsg(UserIndex, "¡Te has convertido en líder de la party!", Protocol.FontTypeNames.FONTTYPE_PARTY)
								End If
							End If
						End If
					Else
	...
	Public Sub SolicitarIngresoAParty(ByVal UserIndex As Short)
		...
		With UserList(UserIndex)
			If .PartyIndex > 0 Then
				...
				Call WriteConsoleMsg(UserIndex, "Ya perteneces a una party, escribe /SALIRPARTY para abandonarla", Protocol.FontTypeNames.FONTTYPE_PARTY)
				.PartySolicitud = 0
				Exit Sub
			End If
			...
			If tInt > 0 Then
				If UserList(tInt).PartyIndex > 0 Then
					...
					Call WriteConsoleMsg(UserIndex, "El fundador decidirá si te acepta en la party.", Protocol.FontTypeNames.FONTTYPE_PARTY)
				Else
				...
				End If
			Else
				Call WriteConsoleMsg(UserIndex, "Para ingresar a una party debes hacer click sobre el fundador y luego escribir /PARTY", Protocol.FontTypeNames.FONTTYPE_PARTY)
				.PartySolicitud = 0
			End If
		End With
	...
	Public Sub ExpulsarDeParty(ByVal leader As Short, ByVal OldMember As Short)
		...
		If PI = UserList(OldMember).PartyIndex Then
			...
			End If
		Else
			Call WriteConsoleMsg(leader, LCase(UserList(OldMember).name) & " no pertenece a tu party.", Protocol.FontTypeNames.FONTTYPE_INFO)
		End If
				...
				If Not .flags.Muerto = 1 Then
					If .PartyIndex = 0 Then
						If Parties(PI).PuedeEntrar(NewMember, razon) Then
							...
							Else
								...
								Call SendData(modSendData.SendTarget.ToAdmins, leader, PrepareMessageConsoleMsg(" Servidor> CATÁSTROFE EN PARTIES, NUEVOMIEMBRO DIO FALSE! :S ", Protocol.FontTypeNames.FONTTYPE_PARTY))
							End If
						Else
							'no debe entrar
					...
					Else
						If .PartyIndex = PI Then
							Call WriteConsoleMsg(leader, LCase(.name) & " ya es miembro de la party.", Protocol.FontTypeNames.FONTTYPE_PARTY)
						Else
							Call WriteConsoleMsg(leader, .name & " ya es miembro de otra party.", Protocol.FontTypeNames.FONTTYPE_PARTY)
						End If
						
						Exit Sub
					End If
				Else
					Call WriteConsoleMsg(leader, "¡Está muerto, no puedes aceptar miembros en ese estado!", Protocol.FontTypeNames.FONTTYPE_PARTY)
					Exit Sub
				End If
			Else
				If .PartyIndex = PI Then
					Call WriteConsoleMsg(leader, LCase(.name) & " ya es miembro de la party.", Protocol.FontTypeNames.FONTTYPE_PARTY)
				Else
					Call WriteConsoleMsg(leader, LCase(.name) & " no ha solicitado ingresar a tu party.", Protocol.FontTypeNames.FONTTYPE_PARTY)
				End If
	...
	Public Sub ActualizaExperiencias()
		...
		If Not PARTY_EXPERIENCIAPORGOLPE Then
			...
			Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Distribuyendo experiencia en parties.", Protocol.FontTypeNames.FONTTYPE_SERVER))
			For i = 1 To MAX_PARTIES
				If Not Parties(i) Is Nothing Then
					Call Parties(i).FlushExperiencia()
				End If
			Next i
			Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Experiencia distribuida.", Protocol.FontTypeNames.FONTTYPE_SERVER))
			Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessagePauseToggle())
			haciendoBK = False
	...
	Public Sub ObtenerExito(ByVal UserIndex As Short, ByVal Exp As Integer, ByRef mapa As Short, ByRef X As Short, ByRef Y As Short)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
