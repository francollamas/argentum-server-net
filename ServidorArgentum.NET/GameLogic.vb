...
Module Extra
	...
	Public Function esArmada(ByVal UserIndex As Short) As Boolean
		'***************************************************
	...
	Public Function esCaos(ByVal UserIndex As Short) As Boolean
		'***************************************************
		'Autor: Pablo (ToxicWaste)
		'Last Modification: 23/01/2007
		'***************************************************
	...
	Public Function EsGM(ByVal UserIndex As Short) As Boolean
		'***************************************************
		'Autor: Pablo (ToxicWaste)
		'Last Modification: 23/01/2007
		'***************************************************
		
		EsGM = (UserList(UserIndex).flags.Privilegios And (Declaraciones.PlayerType.Admin Or Declaraciones.PlayerType.Dios Or Declaraciones.PlayerType.SemiDios Or Declaraciones.PlayerType.Consejero))
	End Function
	
	Public Sub DoTileEvents(ByVal UserIndex As Short, ByVal Map As Short, ByVal X As Short, ByVal Y As Short)
		'***************************************************
		...
		If InMapBounds(Map, X, Y) Then
			With MapData(Map, X, Y)
				If .ObjInfo.ObjIndex > 0 Then
					FxFlag = ObjData_Renamed(.ObjInfo.ObjIndex).OBJType = Declaraciones.eOBJType.otTeleport
				...
				If .TileExit.Map > 0 And .TileExit.Map <= NumMaps Then
					...
					If UCase(MapInfo_Renamed(DestPos.Map).Restringir) = "NEWBIE" Then
						...
						If EsNewbie(UserIndex) Or EsGM(UserIndex) Then
							...
							End If
						Else 'No es newbie
							Call WriteConsoleMsg(UserIndex, "Mapa exclusivo para newbies.", Protocol.FontTypeNames.FONTTYPE_INFO)
...
Errhandler: 
	...
	Function InRangoVisionNPC(ByVal NpcIndex As Short, ByRef X As Short, ByRef Y As Short) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Function InMapBounds(ByVal Map As Short, ByVal X As Short, ByVal Y As Short) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Sub ClosestLegalPos(ByRef Pos As WorldPos, ByRef nPos As WorldPos, Optional ByRef PuedeAgua As Boolean = False, Optional ByRef PuedeTierra As Boolean = True)
		'*****************************************************************
	...
	Function NameIndex(ByVal name As String) As Short
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		'UPGRADE_ISSUE: No se admite la función InStrB. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="367764E5-F3F8-4E43-AC3E-7FE0B5E074E2"'
	...
	Function CheckForSameIP(ByVal UserIndex As Short, ByVal UserIP As String) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Sub HeadtoPos(ByVal Head As Declaraciones.eHeading, ByRef Pos As WorldPos)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
	...
	Function LegalPos(ByVal Map As Short, ByVal X As Short, ByVal Y As Short, Optional ByVal PuedeAgua As Boolean = False, Optional ByVal PuedeTierra As Boolean = True) As Boolean
		'***************************************************
		'Autor: Pablo (ToxicWaste) & Unknown (orginal version)
		'Last Modification: 23/01/2007
	...
	Function MoveToLegalPos(ByVal Map As Short, ByVal X As Short, ByVal Y As Short, Optional ByVal PuedeAgua As Boolean = False, Optional ByVal PuedeTierra As Boolean = True) As Boolean
		'***************************************************
		'Autor: ZaMa
	...
	Public Sub FindLegalPos(ByVal UserIndex As Short, ByVal Map As Short, ByRef X As Short, ByRef Y As Short)
		'***************************************************
		'Autor: ZaMa
				...
				If OtherUserIndex <> 0 Then
					...
					If UserList(OtherUserIndex).ComUsu.DestUsu > 0 Then
						...
						If UserList(UserList(OtherUserIndex).ComUsu.DestUsu).flags.UserLogged Then
							...
							Call WriteConsoleMsg(UserList(OtherUserIndex).ComUsu.DestUsu, "Comercio cancelado. El otro usuario se ha desconectado.", Protocol.FontTypeNames.FONTTYPE_TALK)
	...
	Function LegalPosNPC(ByVal Map As Short, ByVal X As Short, ByVal Y As Short, ByVal AguaValida As Byte, Optional ByVal IsPet As Boolean = False) As Boolean
		'***************************************************
		'Autor: Unkwnown
		...
		With MapData(Map, X, Y)
			...
			If AguaValida = 0 Then
				LegalPosNPC = (.Blocked <> 1) And (.UserIndex = 0 Or IsDeadChar Or IsAdminInvisible) And (.NpcIndex = 0) And (.trigger <> Declaraciones.eTrigger.POSINVALIDA Or IsPet) And Not HayAgua(Map, X, Y)
			Else
				LegalPosNPC = (.Blocked <> 1) And (.UserIndex = 0 Or IsDeadChar Or IsAdminInvisible) And (.NpcIndex = 0) And (.trigger <> Declaraciones.eTrigger.POSINVALIDA Or IsPet)
			End If
		End With
	End Function
	
	Sub SendHelp(ByVal index As Short)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		For LoopC = 1 To NumHelpLines
			Call WriteConsoleMsg(index, GetVar(DatPath & "Help.dat", "Help", "Line" & LoopC), Protocol.FontTypeNames.FONTTYPE_INFO)
		Next LoopC
		
	End Sub
	
	Public Sub Expresar(ByVal NpcIndex As Short, ByVal UserIndex As Short)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Sub LookatTile(ByVal UserIndex As Short, ByVal Map As Short, ByVal X As Short, ByVal Y As Short)
		'***************************************************
		...
		Dim Stat As String
		Dim ft As Protocol.FontTypeNames
			...
			If InMapBounds(Map, X, Y) Then
				With .flags
					...
					ElseIf MapData(Map, X + 1, Y).ObjInfo.ObjIndex > 0 Then 
						'Informa el nombre
						If ObjData_Renamed(MapData(Map, X + 1, Y).ObjInfo.ObjIndex).OBJType = Declaraciones.eOBJType.otPuertas Then
							.TargetObjMap = Map
							.TargetObjX = X + 1
							.TargetObjY = Y
							FoundSomething = 1
					...
					ElseIf MapData(Map, X + 1, Y + 1).ObjInfo.ObjIndex > 0 Then 
						If ObjData_Renamed(MapData(Map, X + 1, Y + 1).ObjInfo.ObjIndex).OBJType = Declaraciones.eOBJType.otPuertas Then
							'Informa el nombre
							.TargetObjMap = Map
							.TargetObjX = X + 1
							.TargetObjY = Y + 1
					...
					ElseIf MapData(Map, X, Y + 1).ObjInfo.ObjIndex > 0 Then 
						If ObjData_Renamed(MapData(Map, X, Y + 1).ObjInfo.ObjIndex).OBJType = Declaraciones.eOBJType.otPuertas Then
							'Informa el nombre
							.TargetObjMap = Map
							.TargetObjX = X
							.TargetObjY = Y + 1
					...
					If FoundSomething = 1 Then
						...
						If MostrarCantidad(.TargetObj) Then
							Call WriteConsoleMsg(UserIndex, ObjData_Renamed(.TargetObj).name & " - " & MapData(.TargetObjMap, .TargetObjX, .TargetObjY).ObjInfo.Amount & "", Protocol.FontTypeNames.FONTTYPE_INFO)
						Else
							Call WriteConsoleMsg(UserIndex, ObjData_Renamed(.TargetObj).name, Protocol.FontTypeNames.FONTTYPE_INFO)
						End If
								...
								Else
									If Not .flags.Privilegios And Declaraciones.PlayerType.User Then
										...
										If .flags.Privilegios = Declaraciones.PlayerType.Dios Then
											...
											' Gm
										ElseIf .flags.Privilegios = Declaraciones.PlayerType.SemiDios Then 
											ft = Protocol.FontTypeNames.FONTTYPE_GM
											' Conse
										ElseIf .flags.Privilegios = Declaraciones.PlayerType.Consejero Then 
											ft = Protocol.FontTypeNames.FONTTYPE_CONSE
											' Rm o Dsrm
										ElseIf .flags.Privilegios = (Declaraciones.PlayerType.RoleMaster Or Declaraciones.PlayerType.Consejero) Or .flags.Privilegios = (Declaraciones.PlayerType.RoleMaster Or Declaraciones.PlayerType.Dios) Then 
				...
				With .flags
					If FoundChar = 2 Then '¿Encontro un NPC?
						...
						If .Privilegios And (Declaraciones.PlayerType.SemiDios Or Declaraciones.PlayerType.Dios Or Declaraciones.PlayerType.Admin) Then
							estatus = "(" & MinHp & "/" & MaxHp & ") "
						Else
							If .Muerto = 0 Then
						...
						Else
							If Npclist(TempCharIndex).MaestroUser > 0 Then
								Call WriteConsoleMsg(UserIndex, estatus & Npclist(TempCharIndex).name & " es mascota de " & UserList(Npclist(TempCharIndex).MaestroUser).name & ".", Protocol.FontTypeNames.FONTTYPE_INFO)
							Else
					...
					If FoundSomething = 0 Then
						...
						.TargetObjX = 0
						.TargetObjY = 0
						Call WriteMultiMessage(UserIndex, Declaraciones.eMessages.DontSeeAnything)
					End If
				End With
			Else
				If FoundSomething = 0 Then
...
Errhandler: 
	...
	Function FindDirection(ByRef Pos As WorldPos, ByRef Target As WorldPos) As Declaraciones.eHeading
		...
		'NE
		If System.Math.Sign(X) = -1 And System.Math.Sign(Y) = 1 Then
			FindDirection = IIf(RandomNumber(0, 1), Declaraciones.eHeading.NORTH, Declaraciones.eHeading.EAST)
			Exit Function
		End If
		
		'NW
		If System.Math.Sign(X) = 1 And System.Math.Sign(Y) = 1 Then
			FindDirection = IIf(RandomNumber(0, 1), Declaraciones.eHeading.WEST, Declaraciones.eHeading.NORTH)
			Exit Function
		End If
		
		'SW
		If System.Math.Sign(X) = 1 And System.Math.Sign(Y) = -1 Then
			FindDirection = IIf(RandomNumber(0, 1), Declaraciones.eHeading.WEST, Declaraciones.eHeading.SOUTH)
			Exit Function
		End If
		
		'SE
		If System.Math.Sign(X) = -1 And System.Math.Sign(Y) = -1 Then
			FindDirection = IIf(RandomNumber(0, 1), Declaraciones.eHeading.SOUTH, Declaraciones.eHeading.EAST)
			Exit Function
		End If
		
		'Sur
		If System.Math.Sign(X) = 0 And System.Math.Sign(Y) = -1 Then
			FindDirection = Declaraciones.eHeading.SOUTH
			Exit Function
		End If
		
		'norte
		If System.Math.Sign(X) = 0 And System.Math.Sign(Y) = 1 Then
			FindDirection = Declaraciones.eHeading.NORTH
			Exit Function
		End If
		
		'oeste
		If System.Math.Sign(X) = 1 And System.Math.Sign(Y) = 0 Then
			FindDirection = Declaraciones.eHeading.WEST
			Exit Function
		End If
		
		'este
		If System.Math.Sign(X) = -1 And System.Math.Sign(Y) = 0 Then
			FindDirection = Declaraciones.eHeading.EAST
			Exit Function
		End If
		
		'misma
		If System.Math.Sign(X) = 0 And System.Math.Sign(Y) = 0 Then
			FindDirection = 0
			Exit Function
		End If
	...
	Public Function ItemNoEsDeMapa(ByVal index As Short, ByVal bIsExit As Boolean) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		With ObjData_Renamed(index)
			ItemNoEsDeMapa = .OBJType <> Declaraciones.eOBJType.otPuertas And .OBJType <> Declaraciones.eOBJType.otForos And .OBJType <> Declaraciones.eOBJType.otCarteles And .OBJType <> Declaraciones.eOBJType.otArboles And .OBJType <> Declaraciones.eOBJType.otYacimiento And Not (.OBJType = Declaraciones.eOBJType.otTeleport And bIsExit)
			
		End With
		
	End Function
	
	Public Function MostrarCantidad(ByVal index As Short) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		With ObjData_Renamed(index)
			MostrarCantidad = .OBJType <> Declaraciones.eOBJType.otPuertas And .OBJType <> Declaraciones.eOBJType.otForos And .OBJType <> Declaraciones.eOBJType.otCarteles And .OBJType <> Declaraciones.eOBJType.otArboles And .OBJType <> Declaraciones.eOBJType.otYacimiento And .OBJType <> Declaraciones.eOBJType.otTeleport
		End With
		
	End Function
	
	Public Function EsObjetoFijo(ByVal OBJType As Declaraciones.eOBJType) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		EsObjetoFijo = OBJType = Declaraciones.eOBJType.otForos Or OBJType = Declaraciones.eOBJType.otCarteles Or OBJType = Declaraciones.eOBJType.otArboles Or OBJType = Declaraciones.eOBJType.otYacimiento
	End Function
End Module