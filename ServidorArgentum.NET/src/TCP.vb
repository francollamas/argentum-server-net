...
Module TCP
    ...
    Function ValidateSkills(ByVal UserIndex As Short) As Boolean
        ...
        For LoopC = 1 To NUMSKILLS
            If UserList(UserIndex).Stats.UserSkills(LoopC) < 0 Then
                Exit Function
                If UserList(UserIndex).Stats.UserSkills(LoopC) > 100 Then _
    ...
    Function ValidateChr(ByVal UserIndex As Short) As Boolean
        ...
        ValidateChr = UserList(UserIndex).Char_Renamed.Head <> 0 And UserList(UserIndex).Char_Renamed.body <> 0 And
                ...
                Else
                    ...
                    If MapData(.Pos.Map, .Pos.X, .Pos.Y).UserIndex <> 0 Then
                        ...
                        If UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).UserIndex).ComUsu.DestUsu > 0 Then
                            ...
                            If _
                                UserList(UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).UserIndex).ComUsu.DestUsu).flags.
                                    UserLogged Then
                                Call _
                                    FinComerciarUsu(UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).UserIndex).ComUsu.DestUsu)
                                Call _
                            ...
                            If UserList(MapData(.Pos.Map, .Pos.X, .Pos.Y).UserIndex).flags.UserLogged Then
                                ...
                                Call _
                                    WriteErrorMsg(MapData(.Pos.Map, .Pos.X, .Pos.Y).UserIndex,
            ...
            If migr_LenB(tStr) <> 0 Then
                Call _
                    WriteShowMessageBox(UserIndex,
                                        "Tu solicitud de ingreso al clan ha sido rechazada. El clan te explica que: " &
                                        tStr)
            End If
