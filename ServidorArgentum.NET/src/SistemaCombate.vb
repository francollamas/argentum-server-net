...
Module SistemaCombate
	...
	Private Function PoderEvasionEscudo(ByVal UserIndex As Short) As Integer
		...
		PoderEvasionEscudo = (UserList(UserIndex).Stats.UserSkills(Declaraciones.eSkill.Defensa) * ModClase_Renamed(UserList(UserIndex).clase).Escudo) / 2
	End Function
	...
	Private Function PoderEvasion(ByVal UserIndex As Short) As Integer
		...
		With UserList(UserIndex)
			lTemp = (.Stats.UserSkills(Declaraciones.eSkill.Tacticas) + .Stats.UserSkills(Declaraciones.eSkill.Tacticas) / 33 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).Evasion
	...
	Private Function PoderAtaqueArma(ByVal UserIndex As Short) As Integer
		...
		With UserList(UserIndex)
			...
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Armas) < 61 Then 
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Armas) + .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueArmas
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Armas) < 91 Then 
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Armas) + 2 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueArmas
			Else
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Armas) + 3 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueArmas
			End If
	...
	Private Function PoderAtaqueProyectil(ByVal UserIndex As Short) As Integer
		...
		With UserList(UserIndex)
			If .Stats.UserSkills(Declaraciones.eSkill.Proyectiles) < 31 Then
				PoderAtaqueTemp = .Stats.UserSkills(Declaraciones.eSkill.Proyectiles) * ModClase_Renamed(.clase).AtaqueProyectiles
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Proyectiles) < 61 Then 
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Proyectiles) + .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueProyectiles
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Proyectiles) < 91 Then 
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Proyectiles) + 2 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueProyectiles
			Else
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Proyectiles) + 3 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueProyectiles
			End If
	...
	Private Function PoderAtaqueWrestling(ByVal UserIndex As Short) As Integer
		...
		With UserList(UserIndex)
			If .Stats.UserSkills(Declaraciones.eSkill.Wrestling) < 31 Then
				PoderAtaqueTemp = .Stats.UserSkills(Declaraciones.eSkill.Wrestling) * ModClase_Renamed(.clase).AtaqueWrestling
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Wrestling) < 61 Then 
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Wrestling) + .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueWrestling
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Wrestling) < 91 Then 
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Wrestling) + 2 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueWrestling
			Else
				PoderAtaqueTemp = (.Stats.UserSkills(Declaraciones.eSkill.Wrestling) + 3 * .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad)) * ModClase_Renamed(.clase).AtaqueWrestling
			End If
			...
			If Not NpcImpacto Then
				If SkillDefensa + SkillTacticas > 0 Then 'Evitamos división por cero
					...
					If Rechazo Then
						...
						Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_ESCUDO, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y))
						Call WriteMultiMessage(UserIndex, Declaraciones.eMessages.BlockedWithShieldUser) 'Call WriteBlockedWithShieldUser(UserIndex)
	...
	Public Function CalcularDaño(ByVal UserIndex As Short, Optional ByVal NpcIndex As Short = 0) As Integer
		...
		With UserList(UserIndex)
			...
			Else
				CalcularDaño = (3 * DañoArma + ((DañoMaxArma / 5) * MaximoInt(0, .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) - 15)) + DañoUsuario) * ModifClase
			End If
		End With
	End Function
	...
	Public Sub UserDañoNpc(ByVal UserIndex As Short, ByVal NpcIndex As Short)
		...
		If UserList(UserIndex).flags.Navegando = 1 Then
			If UserList(UserIndex).Invent.BarcoObjIndex > 0 Then
				DañoBase = DañoBase + RandomNumber(ObjData_Renamed(UserList(UserIndex).Invent.BarcoObjIndex).MinHIT, ObjData_Renamed(UserList(UserIndex).Invent.BarcoObjIndex).MaxHIT)
			End If
		End If
	...
	Public Sub NpcDaño(ByVal NpcIndex As Short, ByVal UserIndex As Short)
		...
		With UserList(UserIndex)
			...
			If .flags.Meditando Then
				If daño > Fix(.Stats.MinHp / 100 * .Stats.UserAtributos(Declaraciones.eAtributos.Inteligencia) * .Stats.UserSkills(Declaraciones.eSkill.Meditar) / 100 * 12 / (RandomNumber(0, 5) + 7)) Then
			...
			'Muere el usuario
			If .Stats.MinHp <= 0 Then
				Call WriteMultiMessage(UserIndex, Declaraciones.eMessages.NPCKillUser) 'Call WriteNPCKillUser(UserIndex) ' Le informamos que ha muerto ;)
	...
	Public Sub CheckPets(ByVal NpcIndex As Short, ByVal UserIndex As Short, Optional ByVal CheckElementales As Boolean = True)
		'***************************************************
		'Author: Unknown
	...
	Public Function NpcAtacaUser(ByVal NpcIndex As Short, ByVal UserIndex As Short) As Boolean
		...
		With UserList(UserIndex)
			...
			If (Not .flags.Privilegios And Declaraciones.PlayerType.User) <> 0 And Not .flags.AdminPerseguible Then Exit Function
		End With
		...
		With Npclist(NpcIndex)
			...
			If .flags.Snd1 > 0 Then
				Call SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, PrepareMessagePlayWave(.flags.Snd1, .Pos.X, .Pos.Y))
			End If
		End With
		
		If NpcImpacto(NpcIndex, UserIndex) Then
			With UserList(UserIndex)
				Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_IMPACTO, .Pos.X, .Pos.Y))
				
				If .flags.Meditando = False Then
					If .flags.Navegando = 0 Then
						Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(.Char_Renamed.CharIndex, FXSANGRE, 0))
					End If
				End If
	...
	Public Sub NpcAtacaNpc(ByVal Atacante As Short, ByVal Victima As Short, Optional ByVal cambiarMOvimiento As Boolean = True)
		'*************************************************
		'Author: Unknown
		...
		With Npclist(Atacante)
			...
			If Npclist(Victima).Numero = PRKING_NPC Then
				If pretorianosVivos > 0 Then
					Call WriteConsoleMsg(.MaestroUser, "Debes matar al resto del ejército antes de atacar al rey!", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					.TargetNPC = 0
					Exit Sub
				End If
			End If
			...
			If .flags.Snd1 > 0 Then
				Call SendData(modSendData.SendTarget.ToNPCArea, Atacante, PrepareMessagePlayWave(.flags.Snd1, .Pos.X, .Pos.Y))
			End If
			
			If NpcImpactoNpc(Atacante, Victima) Then
				If Npclist(Victima).flags.Snd2 > 0 Then
					Call SendData(modSendData.SendTarget.ToNPCArea, Victima, PrepareMessagePlayWave(Npclist(Victima).flags.Snd2, Npclist(Victima).Pos.X, Npclist(Victima).Pos.Y))
				Else
					Call SendData(modSendData.SendTarget.ToNPCArea, Victima, PrepareMessagePlayWave(SND_IMPACTO2, Npclist(Victima).Pos.X, Npclist(Victima).Pos.Y))
				End If
				
				If .MaestroUser > 0 Then
					Call SendData(modSendData.SendTarget.ToNPCArea, Atacante, PrepareMessagePlayWave(SND_IMPACTO, .Pos.X, .Pos.Y))
				Else
					Call SendData(modSendData.SendTarget.ToNPCArea, Victima, PrepareMessagePlayWave(SND_IMPACTO, Npclist(Victima).Pos.X, Npclist(Victima).Pos.Y))
				End If
				
				Call NpcDañoNpc(Atacante, Victima)
			Else
				If .MaestroUser > 0 Then
					Call SendData(modSendData.SendTarget.ToNPCArea, Atacante, PrepareMessagePlayWave(SND_SWING, .Pos.X, .Pos.Y))
				Else
					Call SendData(modSendData.SendTarget.ToNPCArea, Victima, PrepareMessagePlayWave(SND_SWING, Npclist(Victima).Pos.X, Npclist(Victima).Pos.Y))
				End If
			End If
		End With
	End Sub
	...
	Public Function UsuarioAtacaNpc(ByVal UserIndex As Short, ByVal NpcIndex As Short) As Boolean
		...
		If UserImpactoNpc(UserIndex, NpcIndex) Then
			If Npclist(NpcIndex).flags.Snd2 > 0 Then
				Call SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, PrepareMessagePlayWave(Npclist(NpcIndex).flags.Snd2, Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y))
			Else
				Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_IMPACTO2, Npclist(NpcIndex).Pos.X, Npclist(NpcIndex).Pos.Y))
			End If
			
			Call UserDañoNpc(UserIndex, NpcIndex)
		Else
			Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(SND_SWING, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y))
		...
		With UserList(UserIndex)
			...
			Else
				If .Genero = Declaraciones.eGenero.Hombre Then
					Call WriteConsoleMsg(UserIndex, "Estás muy cansado para luchar.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Else
					Call WriteConsoleMsg(UserIndex, "Estás muy cansada para luchar.", Protocol.FontTypeNames.FONTTYPE_INFO)
				End If
				Exit Sub
			End If
			...
			If Index > 0 Then
				If Npclist(Index).Attackable Then
					If Npclist(Index).MaestroUser > 0 And MapInfo_Renamed(Npclist(Index).Pos.Map).Pk = False Then
						Call WriteConsoleMsg(UserIndex, "No puedes atacar mascotas en zona segura.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
						Exit Sub
					End If
					
					Call UsuarioAtacaNpc(UserIndex, Index)
				Else
					Call WriteConsoleMsg(UserIndex, "No puedes atacar a este NPC.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
	...
	Public Function UsuarioImpacto(ByVal AtacanteIndex As Short, ByVal VictimaIndex As Short) As Boolean
		...
		If UserList(VictimaIndex).Invent.EscudoEqpObjIndex > 0 Then
			...
			If Not UsuarioImpacto Then
				...
				If Rechazo Then
					'Se rechazo el ataque con el escudo
					Call SendData(modSendData.SendTarget.ToPCArea, VictimaIndex, PrepareMessagePlayWave(SND_ESCUDO, UserList(VictimaIndex).Pos.X, UserList(VictimaIndex).Pos.Y))
	...
	Public Function UsuarioAtacaUsuario(ByVal AtacanteIndex As Short, ByVal VictimaIndex As Short) As Boolean
		...
		With UserList(AtacanteIndex)
			If Distancia(.Pos, UserList(VictimaIndex).Pos) > MAXDISTANCIAARCO Then
				Call WriteConsoleMsg(AtacanteIndex, "Estás muy lejos para disparar.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Exit Function
			End If
			...
			If UsuarioImpacto(AtacanteIndex, VictimaIndex) Then
				Call SendData(modSendData.SendTarget.ToPCArea, AtacanteIndex, PrepareMessagePlayWave(SND_IMPACTO, .Pos.X, .Pos.Y))
				
				If UserList(VictimaIndex).flags.Navegando = 0 Then
					Call SendData(modSendData.SendTarget.ToPCArea, VictimaIndex, PrepareMessageCreateFX(UserList(VictimaIndex).Char_Renamed.CharIndex, FXSANGRE, 0))
				End If
			...
			Else
				...
				Else
					Call SendData(modSendData.SendTarget.ToPCArea, AtacanteIndex, PrepareMessagePlayWave(SND_SWING, .Pos.X, .Pos.Y))
				End If
...
Errhandler: 
	...
	Public Sub UserDañoUser(ByVal AtacanteIndex As Short, ByVal VictimaIndex As Short)
		...
		With UserList(AtacanteIndex)
			...
			Call WriteMultiMessage(AtacanteIndex, Declaraciones.eMessages.UserHittedUser, UserList(VictimaIndex).Char_Renamed.CharIndex, Lugar, daño)
			Call WriteMultiMessage(VictimaIndex, Declaraciones.eMessages.UserHittedByUser, .Char_Renamed.CharIndex, Lugar, daño)
...
Errhandler: 
	...
	Sub UsuarioAtacadoPorUsuario(ByVal AttackerIndex As Short, ByVal VictimIndex As Short)
		...
		With UserList(VictimIndex)
			If .flags.Meditando Then
				...
				.Char_Renamed.FX = 0
				.Char_Renamed.loops = 0
				Call SendData(modSendData.SendTarget.ToPCArea, VictimIndex, PrepareMessageCreateFX(.Char_Renamed.CharIndex, 0, 0))
			End If
		End With
	...
	Public Function PuedeAtacar(ByVal AttackerIndex As Short, ByVal VictimIndex As Short) As Boolean
		...
		If UserList(AttackerIndex).flags.EnConsulta Then
			Call WriteConsoleMsg(AttackerIndex, "No puedes atacar usuarios mientras estas en consulta.", Protocol.FontTypeNames.FONTTYPE_INFO)
			Exit Function
		End If
		
		' No podes atacar si esta en consulta
		If UserList(VictimIndex).flags.EnConsulta Then
			Call WriteConsoleMsg(AttackerIndex, "No puedes atacar usuarios mientras estan en consulta.", Protocol.FontTypeNames.FONTTYPE_INFO)
			Exit Function
		End If
		...
		Select Case TriggerZonaPelea(AttackerIndex, VictimIndex)
			...
			Case Declaraciones.eTrigger6.TRIGGER6_AUSENTE
				...
				If (UserList(VictimIndex).flags.Privilegios And Declaraciones.PlayerType.User) = 0 Then
					If UserList(VictimIndex).flags.AdminInvisible = 0 Then Call WriteConsoleMsg(AttackerIndex, "El ser es demasiado poderoso.", Protocol.FontTypeNames.FONTTYPE_WARNING)
					PuedeAtacar = False
					Exit Function
				End If
		End Select
			...
			If Not criminal(AttackerIndex) Then
				...
				If esArmada(AttackerIndex) Then
					...
					If esArmada(VictimIndex) Then
						' No puede
						Call WriteConsoleMsg(AttackerIndex, "Los soldados del ejército real tienen prohibido atacar ciudadanos.", Protocol.FontTypeNames.FONTTYPE_WARNING)
						Exit Function
					End If
				End If
		...
		Else
			...
			If esCaos(VictimIndex) Then
				If esCaos(AttackerIndex) Then
					Call WriteConsoleMsg(AttackerIndex, "Los miembros de la legión oscura tienen prohibido atacarse entre sí.", Protocol.FontTypeNames.FONTTYPE_WARNING)
					Exit Function
				End If
			End If
		End If
		...
		If UserList(AttackerIndex).flags.Seguro Then
			If Not criminal(VictimIndex) Then
				Call WriteConsoleMsg(AttackerIndex, "No puedes atacar ciudadanos, para hacerlo debes desactivar el seguro.", Protocol.FontTypeNames.FONTTYPE_WARNING)
				PuedeAtacar = False
				Exit Function
			End If
		Else
		...
		If MapInfo_Renamed(UserList(VictimIndex).Pos.Map).Pk = False Then
			If esArmada(AttackerIndex) Then
				If UserList(AttackerIndex).Faccion.RecompensasReal > 11 Then
					If UserList(VictimIndex).Pos.Map = 58 Or UserList(VictimIndex).Pos.Map = 59 Or UserList(VictimIndex).Pos.Map = 60 Then
						Call WriteConsoleMsg(VictimIndex, "¡Huye de la ciudad! Estás siendo atacado y no podrás defenderte.", Protocol.FontTypeNames.FONTTYPE_WARNING)
			...
			If esCaos(AttackerIndex) Then
				...
				End If
			End If
			Call WriteConsoleMsg(AttackerIndex, "Esta es una zona segura, aquí no puedes atacar a otros usuarios.", Protocol.FontTypeNames.FONTTYPE_WARNING)
			PuedeAtacar = False
			Exit Function
		End If
		
		'Estas atacando desde un trigger seguro? o tu victima esta en uno asi?
		If MapData(UserList(VictimIndex).Pos.Map, UserList(VictimIndex).Pos.X, UserList(VictimIndex).Pos.Y).trigger = Declaraciones.eTrigger.ZONASEGURA Or MapData(UserList(AttackerIndex).Pos.Map, UserList(AttackerIndex).Pos.X, UserList(AttackerIndex).Pos.Y).trigger = Declaraciones.eTrigger.ZONASEGURA Then
...
Errhandler: 
	...
	Public Function PuedeAtacarNPC(ByVal AttackerIndex As Short, ByVal NpcIndex As Short, Optional ByVal Paraliza As Boolean = False, Optional ByVal IsPet As Boolean = False) As Boolean
		'***************************************************
		...
		' No podes atacar si estas en consulta
		If UserList(AttackerIndex).flags.EnConsulta Then
			Call WriteConsoleMsg(AttackerIndex, "No puedes atacar npcs mientras estas en consulta.", Protocol.FontTypeNames.FONTTYPE_INFO)
			Exit Function
		End If
		...
		If Npclist(NpcIndex).Hostile = 0 Then
			...
			If Npclist(NpcIndex).NPCtype = Declaraciones.eNPCType.Guardiascaos Then
				...
				If esCaos(AttackerIndex) Then
					Call WriteConsoleMsg(AttackerIndex, "No puedes atacar Guardias del Caos siendo de la legión oscura.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Function
				End If
				'Es guardia Real?
			ElseIf Npclist(NpcIndex).NPCtype = Declaraciones.eNPCType.GuardiaReal Then 
				...
				If esArmada(AttackerIndex) Then
					Call WriteConsoleMsg(AttackerIndex, "No puedes atacar Guardias Reales siendo del ejército real.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Function
				End If
				'Tienes el seguro puesto?
				If UserList(AttackerIndex).flags.Seguro Then
					Call WriteConsoleMsg(AttackerIndex, "Para poder atacar Guardias Reales debes quitarte el seguro.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Function
				Else
					Call WriteConsoleMsg(AttackerIndex, "¡Atacaste un Guardia Real! Eres un criminal.", Protocol.FontTypeNames.FONTTYPE_INFO)
			...
			ElseIf Npclist(NpcIndex).MaestroUser = 0 Then 
				...
				If Not criminal(AttackerIndex) Then
					...
					If esArmada(AttackerIndex) Then
						Call WriteConsoleMsg(AttackerIndex, "Los miembros del ejército real no pueden atacar npcs no hostiles.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					End If
					
					'Sos ciudadano, tenes el seguro puesto?
					If UserList(AttackerIndex).flags.Seguro Then
						Call WriteConsoleMsg(AttackerIndex, "Para atacar a este NPC debes quitarte el seguro.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					Else
						'No tiene seguro puesto. Puede atacar pero es penalizado.
						Call WriteConsoleMsg(AttackerIndex, "Atacaste un NPC no-hostil. Continúa haciéndolo y te podrás convertir en criminal.", Protocol.FontTypeNames.FONTTYPE_INFO)
		...
		If Npclist(NpcIndex).MaestroUser > 0 Then
			If Not criminal(Npclist(NpcIndex).MaestroUser) Then
				...
				If esArmada(AttackerIndex) Then
					...
					Call WriteConsoleMsg(AttackerIndex, "Los miembros del ejército real no pueden atacar mascotas de ciudadanos.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Function
				End If
				...
				If Not criminal(AttackerIndex) Then
					...
					If UserList(AttackerIndex).flags.Seguro Then
						...
						Call WriteConsoleMsg(AttackerIndex, "Para atacar mascotas de ciudadanos debes quitarte el seguro.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					Else
						'El atacante no tiene el seguro puesto. Recibe penalización.
						Call WriteConsoleMsg(AttackerIndex, "Has atacado la Mascota de un ciudadano. Eres un criminal.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Call VolverCriminal(AttackerIndex)
						PuedeAtacarNPC = True
						Exit Function
					End If
			...
			Else
				...
				If esCaos(Npclist(NpcIndex).MaestroUser) Then
					...
					If esCaos(AttackerIndex) Then
						...
						Call WriteConsoleMsg(AttackerIndex, "Los miembros de la legión oscura no pueden atacar mascotas de otros legionarios. ", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					End If
				End If
			End If
						...
						If .flags.Inmovilizado = 1 Or .flags.Paralizado = 1 Then
							...
							If Not criminal(AttackerIndex) And Not criminal(OwnerUserIndex) Then
								...
								If esArmada(AttackerIndex) Then
									...
									If esArmada(OwnerUserIndex) Then
										...
										Call WriteConsoleMsg(AttackerIndex, "Los miembros del Ejército Real no pueden paralizar criaturas ya paralizadas pertenecientes a otros miembros del Ejército Real", Protocol.FontTypeNames.FONTTYPE_INFO)
										Exit Function
									...
									Else
										...
										If UserList(AttackerIndex).flags.Seguro Then
											Call WriteConsoleMsg(AttackerIndex, "Para paralizar criaturas ya paralizadas pertenecientes a ciudadanos debes quitarte el seguro.", Protocol.FontTypeNames.FONTTYPE_INFO)
											Exit Function
										Else
											...
											If ToogleToAtackable(AttackerIndex, OwnerUserIndex) Then
												Call WriteConsoleMsg(AttackerIndex, "Has paralizado la criatura de un ciudadano, ahora eres atacable por él.", Protocol.FontTypeNames.FONTTYPE_INFO)
												PuedeAtacarNPC = True
											End If
							...
							Else
								...
								If esCaos(AttackerIndex) And esCaos(OwnerUserIndex) Then
									...
									Call WriteConsoleMsg(AttackerIndex, "Los miembros de la legión oscura no pueden paralizar criaturas ya paralizadas por otros legionarios.", Protocol.FontTypeNames.FONTTYPE_INFO)
									Exit Function
								End If
							End If
						...
						If Not criminal(OwnerUserIndex) Then
							...
							If esArmada(AttackerIndex) Then
								...
								If esArmada(OwnerUserIndex) Then
									...
									Call WriteConsoleMsg(AttackerIndex, "Los miembros del Ejército Real no pueden atacar criaturas pertenecientes a otros miembros del Ejército Real", Protocol.FontTypeNames.FONTTYPE_INFO)
									Exit Function
								...
								Else
									...
									' Si tiene seguro no puede
									If UserList(AttackerIndex).flags.Seguro Then
										Call WriteConsoleMsg(AttackerIndex, "Para atacar criaturas ya pertenecientes a ciudadanos debes quitarte el seguro.", Protocol.FontTypeNames.FONTTYPE_INFO)
										Exit Function
									Else
										' Si ya estaba atacable, no podrá atacar a un npc perteneciente a otro ciuda
										If ToogleToAtackable(AttackerIndex, OwnerUserIndex) Then
											Call WriteConsoleMsg(AttackerIndex, "Has atacado a la criatura de un ciudadano, ahora eres atacable por él.", Protocol.FontTypeNames.FONTTYPE_INFO)
											PuedeAtacarNPC = True
										End If
							...
							Else
								...
								If Not criminal(AttackerIndex) Then
									...
									If UserList(AttackerIndex).flags.Seguro Then
										'El atacante tiene el seguro puesto. No puede atacar.
										Call WriteConsoleMsg(AttackerIndex, "Para atacar criaturas pertenecientes a ciudadanos debes quitarte el seguro.", Protocol.FontTypeNames.FONTTYPE_INFO)
										Exit Function
						...
						Else
							If esCaos(OwnerUserIndex) Then
								...
								If esCaos(AttackerIndex) Then
									...
									Call WriteConsoleMsg(AttackerIndex, "Los miembros de la Legión Oscura no pueden atacar criaturas de otros legionarios. ", Protocol.FontTypeNames.FONTTYPE_INFO)
									Exit Function
								End If
							End If
						End If
		...
		If esPretoriano(NpcIndex) = 4 Then
			If pretorianosVivos > 0 Then
				Call WriteConsoleMsg(AttackerIndex, "Debes matar al resto del ejército antes de atacar al rey.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Exit Function
			End If
		End If
	...
	Sub CalcularDarExp(ByVal UserIndex As Short, ByVal NpcIndex As Short, ByVal ElDaño As Integer)
		...
		If ExpaDar > 0 Then
			...
			Else
				...
				Call WriteConsoleMsg(UserIndex, "Has ganado " & ExpaDar & " puntos de experiencia.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
			End If
			...
			If ObjInd > 0 Then
				If ObjData_Renamed(ObjInd).Envenena = 1 Then
					...
					If RandomNumber(1, 100) < 60 Then
						...
						Call WriteConsoleMsg(VictimaIndex, "¡¡" & UserList(AtacanteIndex).name & " te ha envenenado!!", Protocol.FontTypeNames.FONTTYPE_FIGHT)
						Call WriteConsoleMsg(AtacanteIndex, "¡¡Has envenenado a " & UserList(VictimaIndex).name & "!!", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					End If
				End If
			End If
		End If
