...
Module PraetoriansCoopNPC
    ...
    Sub CrearClanPretoriano(ByVal X As Short)
        ...
        If TeleFrag > 0 Then
            ...
            If (LegalPos(wp2.Map, wp2.X, wp2.Y)) Then
                ...
                Call _
                    SendData(modSendData.SendTarget.ToNPCArea, TeleFrag,
                    ...
                    If _
                        ...
                        Else
                            If (BestTarget > 0) Then
                                ...
                                If _
                                    System.Math.Sqrt((X - NPCPosX)^2 + (Y - NPCPosY)^2) <
                                    System.Math.Sqrt(
    ...
    Sub PRMAGO_AI(ByVal npcind As Short)
        ...
        Else
            If Not (Npclist(npcind).Invent.BarcoSlot = 6) Then
                ...
                Call _
                    SendData(modSendData.SendTarget.ToNPCArea, npcind,
                        ...
                        If _
                            ...
                            ElseIf (UserList(PJEnInd).flags.Paralizado = 1) Then
                                If (BestTarget > 0) Then
                                    If _
                                        Not _
                                        (UserList(BestTarget).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1) _
                                        Then
                ...
                Call _
                    SendData(modSendData.SendTarget.ToNPCArea, npcind,
                             PrepareMessageCreateFX(Npclist(npcind).Char_Renamed.CharIndex,
                                                    Declaraciones.FXIDs.FXMEDITARGRANDE, INFINITE_LOOPS))
                ...
                If (Npclist(npcind).CanAttack = 1) Then ''saltea el analisis si no puede atacar para evitar cuentas
                    If (NPCAlInd > 0) Then ''allie?
                        If (esPretoriano(NPCAlInd) = 0) Then
                            If (Npclist(NPCAlInd).MaestroUser > 0) And (Not (Npclist(NPCAlInd).flags.Paralizado > 0)) _
                                Then
                    ...
                    ElseIf (PJEnInd > 0) Then ''aggressor
                        If Not (UserList(PJEnInd).flags.Muerto = 1) And UserList(PJEnInd).flags.AdminPerseguible Then
                            If (UserList(PJEnInd).flags.Paralizado = 0) Then
                                If (Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1)) _
                                    Then
        ...
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, npcind,
                     PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas,
                                                ...
                                                System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, NPCAlInd,
                     PrepareMessageCreateFX(Npclist(NPCAlInd).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh,
                                            Hechizos(indireccion).loops))
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, NPCAlInd,
                     PrepareMessagePlayWave(Hechizos(indireccion).WAV, Npclist(NPCAlInd).Pos.X, Npclist(NPCAlInd).Pos.Y))
    ...
    Sub NPCparalizaNPC(ByVal paralizador As Short, ByVal Paralizado As Short, ByVal indice As Object)
        ...
        '' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, paralizador,
                     PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas,
                                                Npclist(paralizador).Char_Renamed.CharIndex,
                                                System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, Paralizado,
    ...
    Sub NPCcuraNPC(ByVal curador As Short, ByVal curado As Short, ByVal indice As Short)
        ...
        '' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, curador,
                     PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas,
                                                Npclist(curador).Char_Renamed.CharIndex,
                                                System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, curado,
    ...
    Sub NPCLanzaCegueraPJ(ByVal npcind As Short, ByVal PJEnInd As Short, ByVal indice As Short)
        ...
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, npcind,
                     PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas,
                                                ...
                                                System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
        Call _
            SendData(modSendData.SendTarget.ToNPCArea, PJEnInd,
                     PrepareMessagePlayWave(Hechizos(indireccion).WAV, UserList(PJEnInd).Pos.X, UserList(PJEnInd).Pos.Y))
        Call _
            SendData(modSendData.SendTarget.ToPCArea, PJEnInd,
        ...
        If Npclist(npcind).name = "Rey Pretoriano" Then
            Call _
                WriteConsoleMsg(PJEnInd, "El rey pretoriano te ha vuelto ciego ", Protocol.FontTypeNames.FONTTYPE_FIGHT)
            Call _
    ...
    Sub NpcLanzaSpellSobreUser2(ByVal NpcIndex As Short, ByVal UserIndex As Short, ByVal Spell As Short)
        ...
        ElseIf Hechizos(Spell).SubeHP = 2 Then
            ...
            If UserList(UserIndex).flags.Privilegios And Declaraciones.PlayerType.User Then _
                UserList(UserIndex).Stats.MinHp = UserList(UserIndex).Stats.MinHp - daño
                ...
                For X = 8 To 95
                    For Y = 8 To 95
                        ...
                        If PJInd > 0 Then
                            ...
                            ''efectiviza el danio
                            If UserList(PJInd).flags.Privilegios And Declaraciones.PlayerType.User Then _
                            ...
                            Call _
                                WriteConsoleMsg(PJInd,
                                                ...
                                                Protocol.FontTypeNames.FONTTYPE_FIGHT)
                            Call _
                                SendData(modSendData.SendTarget.ToPCArea, PJInd,
                        ...
                        ElseIf (MascotaInd > 0) Then
                            If (Npclist(MascotaInd).MaestroUser > 0) Then
                                ...
                                Call _
                                    SendData(modSendData.SendTarget.ToNPCArea, MascotaInd,
                                             PrepareMessagePlayWave(Hechizos(indireccion).WAV, Npclist(MascotaInd).Pos.X,
            ...
            Else ''x completitud, esta en la misma Y
                ...
                Else
                    ...
                    If Npclist(npcorig).CanAttack = 1 And (RandomNumber(1, 100) > 95) Then
                        Call _
                            SendData(modSendData.SendTarget.ToNPCArea, npcorig,
    ...
    Sub LiberarCasperBlock(ByVal npcind As Short)
        ...
        If LegalPos(NPCPosM, NPCPosX + 1, NPCPosY + 1) Then
            Call _
                SendData(modSendData.SendTarget.ToNPCArea, npcind,
                         PrepareMessageCharacterMove(Npclist(npcind).Char_Renamed.CharIndex, NPCPosX + 1, NPCPosY + 1))
            'Update map and npc pos
