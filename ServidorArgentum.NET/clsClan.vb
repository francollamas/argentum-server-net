...
Friend Class clsClan
	...
	Public Sub Inicializar(ByVal GuildName As String, ByVal GuildNumber As Short, ByVal Alineacion As modGuilds.ALINEACION_GUILD)
		Dim i As Short
	...
	Public Sub ConectarMiembro(ByVal UserIndex As Short)
		...
		With UserList(UserIndex)
			Call SendData(modSendData.SendTarget.ToDiosesYclan, .GuildIndex, PrepareMessageGuildChat(.name & " se ha conectado."))
		End With
	End Sub
	...
	Public Sub DesConectarMiembro(ByVal UserIndex As Short)
		...
		For i = 1 To p_OnlineMembers.Count()
			...
			If p_OnlineMembers.Item(i) = UserIndex Then
				...
				With UserList(UserIndex)
					Call SendData(modSendData.SendTarget.ToDiosesYclan, .GuildIndex, PrepareMessageGuildChat(.name & " se ha desconectado."))
				End With
		...
		If q > 0 Then
			...
			For i = 1 To q
				...
				If LenB(tempV) <> 0 Then
					...
					If Not IsDbNull(d.At(tempV)) Then 'cuantos votos tiene?
						...
						Call d.AtPut(tempV, CShort(d.At(tempV)) + 1)
					Else
