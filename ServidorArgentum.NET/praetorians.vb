					...
					If (UserList(PJEnInd).flags.invisible = 0 Or UserList(PJEnInd).flags.Oculto = 0) And Not (UserList(PJEnInd).flags.Muerto = 1) And Not UserList(PJEnInd).flags.AdminInvisible = 1 And UserList(PJEnInd).flags.AdminPerseguible Then
						...
						Else
							If (BestTarget > 0) Then
								...
								If System.Math.Sqrt((X - NPCPosX) ^ 2 + (Y - NPCPosY) ^ 2) < System.Math.Sqrt((NPCPosX - UserList(BestTarget).Pos.X) ^ 2 + (NPCPosY - UserList(BestTarget).Pos.Y) ^ 2) Then
	...
	Sub PRMAGO_AI(ByVal npcind As Short)
		...
		Else
			If Not (Npclist(npcind).Invent.BarcoSlot = 6) Then
				...
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageCreateFX(Npclist(npcind).Char_Renamed.CharIndex, 0, 0))
			End If
		...
		Select Case quehacer
			...
			Case 3
				...
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageCreateFX(Npclist(npcind).Char_Renamed.CharIndex, Declaraciones.FXIDs.FXMEDITARGRANDE, INFINITE_LOOPS))
				...
				If (Npclist(npcind).CanAttack = 1) Then ''saltea el analisis si no puede atacar para evitar cuentas
					...
					If PJEnInd > 0 And Not hayPretorianos Then
						If Not (UserList(PJEnInd).flags.Muerto = 1 Or UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1 Or UserList(PJEnInd).flags.Ceguera = 1) And UserList(PJEnInd).flags.AdminPerseguible Then
							...
							dist = System.Math.Sqrt((UserList(PJEnInd).Pos.X - NPCPosX) ^ 2 + (UserList(PJEnInd).Pos.Y - NPCPosY) ^ 2)
					...
					If InMapBounds(nPos.Map, nPos.X, nPos.Y) Then
						...
						If UI > 0 Then
							If NpcAtacaUser(npcind, UI) Then
								Call ChangeNPCChar(npcind, Npclist(npcind).Char_Renamed.body, Npclist(npcind).Char_Renamed.Head, headingloop)
							End If
				...
				If (PJEnInd > 0) Then
					If (Not (UserList(PJEnInd).flags.invisible = 1 Or UserList(PJEnInd).flags.Oculto = 1 Or UserList(PJEnInd).flags.Muerto = 1)) And EsAlcanzable(npcind, PJEnInd) And UserList(PJEnInd).flags.AdminPerseguible Then
						...
						If (BestTarget > 0) Then
							...
							End If
						Else
							distBestTarget = System.Math.Sqrt((UserList(PJEnInd).Pos.X - NPCPosX) ^ 2 + (UserList(PJEnInd).Pos.Y - NPCPosY) ^ 2)
							BestTarget = PJEnInd
						End If
					End If
				End If
	...
	Sub PRCLER_AI(ByVal npcind As Short)
		...
		Select Case quehacer
			...
			Case 1 '' paralizar enemigo PJ
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(Npclist(npcind).Spells(DAT_PARALIZARPJ)).PalabrasMagicas, Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
			...
			Case 2 '' ataque a usuarios (invisibles tambien)
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(Npclist(npcind).Spells(DAT_TORMENTAAVANZADA)).PalabrasMagicas, Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
...
errorh: 
	...
	Function EsMagoOClerigo(ByVal PJEnInd As Short) As Boolean
		...
		EsMagoOClerigo = UserList(PJEnInd).clase = Declaraciones.eClass.Mage Or UserList(PJEnInd).clase = Declaraciones.eClass.Cleric Or UserList(PJEnInd).clase = Declaraciones.eClass.Druid Or UserList(PJEnInd).clase = Declaraciones.eClass.Bard
		Exit Function
...
errorh: 
	...
	Sub NPCRemueveVenenoNPC(ByVal npcind As Short, ByVal NPCAlInd As Short, ByVal indice As Short)
		...
		Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas, Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
		Call SendData(modSendData.SendTarget.ToNPCArea, NPCAlInd, PrepareMessageCreateFX(Npclist(NPCAlInd).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh, Hechizos(indireccion).loops))
		Call SendData(modSendData.SendTarget.ToNPCArea, NPCAlInd, PrepareMessagePlayWave(Hechizos(indireccion).WAV, Npclist(NPCAlInd).Pos.X, Npclist(NPCAlInd).Pos.Y))
...
errorh: 
	...
	Sub NPCparalizaNPC(ByVal paralizador As Short, ByVal Paralizado As Short, ByVal indice As Object)
		...
		Call SendData(modSendData.SendTarget.ToNPCArea, Paralizado, PrepareMessagePlayWave(Hechizos(indireccion).WAV, Npclist(Paralizado).Pos.X, Npclist(Paralizado).Pos.Y))
		Call SendData(modSendData.SendTarget.ToNPCArea, Paralizado, PrepareMessageCreateFX(Npclist(Paralizado).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh, Hechizos(indireccion).loops))
...
errorh: 
	...
	Sub NPCcuraNPC(ByVal curador As Short, ByVal curado As Short, ByVal indice As Short)
		...
		'' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
		Call SendData(modSendData.SendTarget.ToNPCArea, curador, PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas, Npclist(curador).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
		Call SendData(modSendData.SendTarget.ToNPCArea, curado, PrepareMessagePlayWave(Hechizos(indireccion).WAV, Npclist(curado).Pos.X, Npclist(curado).Pos.Y))
		Call SendData(modSendData.SendTarget.ToNPCArea, curado, PrepareMessageCreateFX(Npclist(curado).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh, Hechizos(indireccion).loops))
...
errorh: 
	...
	Sub NPCLanzaCegueraPJ(ByVal npcind As Short, ByVal PJEnInd As Short, ByVal indice As Short)
		...
		'' Envia las palabras magicas, fx y wav del indice-esimo hechizo del npc-hostiles.dat
		Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead(Hechizos(indireccion).PalabrasMagicas, Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
		Call SendData(modSendData.SendTarget.ToNPCArea, PJEnInd, PrepareMessagePlayWave(Hechizos(indireccion).WAV, UserList(PJEnInd).Pos.X, UserList(PJEnInd).Pos.Y))
		Call SendData(modSendData.SendTarget.ToPCArea, PJEnInd, PrepareMessageCreateFX(UserList(PJEnInd).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh, Hechizos(indireccion).loops))
...
errorh: 
	...
	Sub NPCLanzaEstupidezPJ(ByVal npcind As Short, ByVal PJEnInd As Short, ByVal indice As Short)
		...
		'bardea si es el rey
		If Npclist(npcind).name = "Rey Pretoriano" Then
			Call WriteConsoleMsg(PJEnInd, "El rey pretoriano te ha vuelto estúpido.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
		End If
		Exit Sub
...
errorh: 
	...
	Sub NpcLanzaSpellSobreUser2(ByVal NpcIndex As Short, ByVal UserIndex As Short, ByVal Spell As Short)
		...
		ElseIf Hechizos(Spell).SubeHP = 2 Then 
			...
			If UserList(UserIndex).flags.Privilegios And Declaraciones.PlayerType.User Then UserList(UserIndex).Stats.MinHp = UserList(UserIndex).Stats.MinHp - daño
...
errorh: 
	...
	Sub MagoDestruyeWand(ByVal npcind As Short, ByVal bs As Byte, ByVal indice As Short)
		...
		Select Case bs
			...
			Case 4
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead("vôrtax", Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Lime)))
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessagePlayWave(SONIDO_Dragon_VIVO, Npclist(npcind).Pos.X, Npclist(npcind).Pos.Y))
			Case 3
				...
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessagePlayWave(SONIDO_Dragon_VIVO, Npclist(npcind).Pos.X, Npclist(npcind).Pos.Y))
			Case 2
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead("yäkà E'nta", Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Lime)))
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessagePlayWave(SONIDO_Dragon_VIVO, Npclist(npcind).Pos.X, Npclist(npcind).Pos.Y))
			Case 1
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead("¡¡Koràtá!!", Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Lime)))
				Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessagePlayWave(SONIDO_Dragon_VIVO, Npclist(npcind).Pos.X, Npclist(npcind).Pos.Y))
			Case 0
				...
				For X = 8 To 95
					For Y = 8 To 95
						...
						If PJInd > 0 Then
							dist = System.Math.Sqrt((UserList(PJInd).Pos.X - NPCPosX) ^ 2 + (UserList(PJInd).Pos.Y - NPCPosY) ^ 2)
							danio = 880 / (dist ^ (3 / 7))
							danioI = System.Math.Abs(Int(danio))
							''efectiviza el danio
							If UserList(PJInd).flags.Privilegios And Declaraciones.PlayerType.User Then UserList(PJInd).Stats.MinHp = UserList(PJInd).Stats.MinHp - danioI
							
							Call WriteConsoleMsg(PJInd, Npclist(npcind).name & " te ha quitado " & danioI & " puntos de vida al romper su vara.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
							Call SendData(modSendData.SendTarget.ToPCArea, PJInd, PrepareMessagePlayWave(Hechizos(indireccion).WAV, UserList(PJInd).Pos.X, UserList(PJInd).Pos.Y))
							Call SendData(modSendData.SendTarget.ToPCArea, PJInd, PrepareMessageCreateFX(UserList(PJInd).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh, Hechizos(indireccion).loops))
						...
						ElseIf (MascotaInd > 0) Then 
							If (Npclist(MascotaInd).MaestroUser > 0) Then
								...
								dist = System.Math.Sqrt((Npclist(MascotaInd).Pos.X - NPCPosX) ^ 2 + (Npclist(MascotaInd).Pos.Y - NPCPosY) ^ 2)
								danio = 880 / (dist ^ (3 / 7))
								danioI = System.Math.Abs(Int(danio))
								''efectiviza el danio
								Npclist(MascotaInd).Stats.MinHp = Npclist(MascotaInd).Stats.MinHp - danioI
								
								Call SendData(modSendData.SendTarget.ToNPCArea, MascotaInd, PrepareMessagePlayWave(Hechizos(indireccion).WAV, Npclist(MascotaInd).Pos.X, Npclist(MascotaInd).Pos.Y))
								Call SendData(modSendData.SendTarget.ToNPCArea, MascotaInd, PrepareMessageCreateFX(Npclist(MascotaInd).Char_Renamed.CharIndex, Hechizos(indireccion).FXgrh, Hechizos(indireccion).loops))
			...
			Else ''x completitud, esta en la misma Y
				...
				Else
					...
					If Npclist(npcorig).CanAttack = 1 And (RandomNumber(1, 100) > 95) Then
						Call SendData(modSendData.SendTarget.ToNPCArea, npcorig, PrepareMessageChatOverHead("Maldito bastardo, ¡Ven aquí!", CShort(Str(Npclist(npcorig).Char_Renamed.CharIndex)), System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Yellow)))
		...
		ElseIf (NPCx < USRx) Then 
			...
			Else ''x completitud, esta en la misma Y
				...
				Else
					...
					If Npclist(npcorig).CanAttack = 1 And (RandomNumber(1, 100) > 95) Then
						Call SendData(modSendData.SendTarget.ToNPCArea, npcorig, PrepareMessageChatOverHead("Maldito bastardo, ¡Ven aquí!", Npclist(npcorig).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Yellow)))
						Npclist(npcorig).CanAttack = 0
					End If
				End If
			End If
...
errorh: 
	...
	Sub MoverAba(ByVal npcorig As Short)
		...
		Call SendData(modSendData.SendTarget.ToNPCArea, npcorig, PrepareMessageCharacterMove(Npclist(npcorig).Char_Renamed.CharIndex, NPCx, NPCy + 1))
...
errorh: 
	...
	Sub MoverIzq(ByVal npcorig As Short)
		...
		Call SendData(modSendData.SendTarget.ToNPCArea, npcorig, PrepareMessageCharacterMove(Npclist(npcorig).Char_Renamed.CharIndex, NPCx - 1, NPCy))
		'Update map and npc pos
		MapData(mapa, NPCx, NPCy).NpcIndex = 0
...
errorh: 
	...
	Function CasperBlock(ByVal npc As Short) As Boolean
		...
		retvalue = Not (LegalPos(NPCPosM, NPCPosX + 1, NPCPosY) Or LegalPos(NPCPosM, NPCPosX - 1, NPCPosY) Or LegalPos(NPCPosM, NPCPosX, NPCPosY + 1) Or LegalPos(NPCPosM, NPCPosX, NPCPosY - 1))
...
errorh: 
	...
	Sub LiberarCasperBlock(ByVal npcind As Short)
		...
		If LegalPos(NPCPosM, NPCPosX + 1, NPCPosY + 1) Then
			Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageCharacterMove(Npclist(npcind).Char_Renamed.CharIndex, NPCPosX + 1, NPCPosY + 1))
			'Update map and npc pos
		...
		If Npclist(npcind).CanAttack = 1 Then
			Call SendData(modSendData.SendTarget.ToNPCArea, npcind, PrepareMessageChatOverHead("¡Por las barbas de los antiguos reyes! ¡Alejáos endemoniados espectros o sufriréis la furia de los dioses!", Npclist(npcind).Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Yellow)))
			Npclist(npcind).CanAttack = 0
		End If
