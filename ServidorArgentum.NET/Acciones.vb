...
Module Acciones
	...
	Sub Accion(ByVal UserIndex As Short, ByVal Map As Short, ByVal X As Short, ByVal Y As Short)
		...
		If (System.Math.Abs(UserList(UserIndex).Pos.Y - Y) > RANGO_VISION_Y) Or (System.Math.Abs(UserList(UserIndex).Pos.X - X) > RANGO_VISION_X) Then
			Exit Sub
		End If
				...
				If MapData(Map, X, Y).NpcIndex > 0 Then 'Acciones NPCs
					...
					If Npclist(tempIndex).Comercia = 1 Then
						...
						If Distancia(Npclist(tempIndex).Pos, .Pos) > 3 Then
							Call WriteConsoleMsg(UserIndex, "Estás demasiado lejos del vendedor.", Protocol.FontTypeNames.FONTTYPE_INFO)
							Exit Sub
						End If
					...
					ElseIf Npclist(tempIndex).NPCtype = Declaraciones.eNPCType.Revividor Or Npclist(tempIndex).NPCtype = Declaraciones.eNPCType.ResucitadorNewbie Then 
						If Distancia(.Pos, Npclist(tempIndex).Pos) > 10 Then
							Call WriteConsoleMsg(UserIndex, "El sacerdote no puede curarte debido a que estás demasiado lejos.", Protocol.FontTypeNames.FONTTYPE_INFO)
							Exit Sub
						End If
						
						'Revivimos si es necesario
						If .flags.Muerto = 1 And (Npclist(tempIndex).NPCtype = Declaraciones.eNPCType.Revividor Or EsNewbie(UserIndex)) Then
	...
	Sub AccionParaPuerta(ByVal Map As Short, ByVal X As Short, ByVal Y As Short, ByVal UserIndex As Short)
		...
		If Not (Distance(UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y, X, Y) > 2) Then
			If ObjData_Renamed(MapData(Map, X, Y).ObjInfo.ObjIndex).Llave = 0 Then
				If ObjData_Renamed(MapData(Map, X, Y).ObjInfo.ObjIndex).Cerrada = 1 Then
					...
					If ObjData_Renamed(MapData(Map, X, Y).ObjInfo.ObjIndex).Llave = 0 Then
						...
						MapData(Map, X, Y).ObjInfo.ObjIndex = ObjData_Renamed(MapData(Map, X, Y).ObjInfo.ObjIndex).IndexAbierta
						
						Call modSendData.SendToAreaByPos(Map, X, Y, PrepareMessageObjectCreate(ObjData_Renamed(MapData(Map, X, Y).ObjInfo.ObjIndex).GrhIndex, X, Y))
					...
					Else
						Call WriteConsoleMsg(UserIndex, "La puerta esta cerrada con llave.", Protocol.FontTypeNames.FONTTYPE_INFO)
					End If
				Else
					'Cierra puerta
					MapData(Map, X, Y).ObjInfo.ObjIndex = ObjData_Renamed(MapData(Map, X, Y).ObjInfo.ObjIndex).IndexCerrada
			...
			Else
				Call WriteConsoleMsg(UserIndex, "La puerta está cerrada con llave.", Protocol.FontTypeNames.FONTTYPE_INFO)
			End If
		Else
	...
	Sub AccionParaRamita(ByVal Map As Short, ByVal X As Short, ByVal Y As Short, ByVal UserIndex As Short)
		...
		With UserList(UserIndex)
			...
			If MapData(Map, X, Y).trigger = Declaraciones.eTrigger.ZONASEGURA Or MapInfo_Renamed(Map).Pk = False Then
				Call WriteConsoleMsg(UserIndex, "No puedes hacer fogatas en zona segura.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Sub
			End If
			
			If .Stats.UserSkills(Declaraciones.eSkill.Supervivencia) > 1 And .Stats.UserSkills(Declaraciones.eSkill.Supervivencia) < 6 Then
				Suerte = 3
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Supervivencia) >= 6 And .Stats.UserSkills(Declaraciones.eSkill.Supervivencia) <= 10 Then 
				Suerte = 2
			ElseIf .Stats.UserSkills(Declaraciones.eSkill.Supervivencia) >= 10 And .Stats.UserSkills(Declaraciones.eSkill.Supervivencia) Then 
				Suerte = 1
			End If
			...
			If exito = 1 Then
				...
				Else
					Call WriteConsoleMsg(UserIndex, "La ley impide realizar fogatas en las ciudades.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Sub
				End If
			Else
