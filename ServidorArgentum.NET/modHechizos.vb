...
Module modHechizos
	...
	Sub NpcLanzaSpellSobreUser(ByVal NpcIndex As Short, ByVal UserIndex As Short, ByVal Spell As Short)
		...
		With UserList(UserIndex)
			If Hechizos(Spell).SubeHP = 1 Then
				...
				Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(Hechizos(Spell).WAV, .Pos.X, .Pos.Y))
				Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(.Char_Renamed.CharIndex, Hechizos(Spell).FXgrh, Hechizos(Spell).loops))
				
				.Stats.MinHp = .Stats.MinHp + daño
				If .Stats.MinHp > .Stats.MaxHp Then .Stats.MinHp = .Stats.MaxHp
				
				Call WriteConsoleMsg(UserIndex, Npclist(NpcIndex).name & " te ha quitado " & daño & " puntos de vida.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
			...
			ElseIf Hechizos(Spell).SubeHP = 2 Then 
				...
				If .flags.Privilegios And Declaraciones.PlayerType.User Then
					...
					If .Invent.CascoEqpObjIndex > 0 Then
						daño = daño - RandomNumber(ObjData_Renamed(.Invent.CascoEqpObjIndex).DefensaMagicaMin, ObjData_Renamed(.Invent.CascoEqpObjIndex).DefensaMagicaMax)
					End If
					
					If .Invent.AnilloEqpObjIndex > 0 Then
						daño = daño - RandomNumber(ObjData_Renamed(.Invent.AnilloEqpObjIndex).DefensaMagicaMin, ObjData_Renamed(.Invent.AnilloEqpObjIndex).DefensaMagicaMax)
					End If
			...
			If Hechizos(Spell).Paraliza = 1 Or Hechizos(Spell).Inmoviliza = 1 Then
				If .flags.Paralizado = 0 Then
					...
					If .Invent.AnilloEqpObjIndex = SUPERANILLO Then
						Call WriteConsoleMsg(UserIndex, " Tu anillo rechaza los efectos del hechizo.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
						Exit Sub
					End If
	...
	Sub NpcLanzaSpellSobreNpc(ByVal NpcIndex As Short, ByVal TargetNPC As Short, ByVal Spell As Short)
		...
		If Hechizos(Spell).SubeHP = 2 Then
			...
			daño = RandomNumber(Hechizos(Spell).MinHp, Hechizos(Spell).MaxHp)
			Call SendData(modSendData.SendTarget.ToNPCArea, TargetNPC, PrepareMessagePlayWave(Hechizos(Spell).WAV, Npclist(TargetNPC).Pos.X, Npclist(TargetNPC).Pos.Y))
			Call SendData(modSendData.SendTarget.ToNPCArea, TargetNPC, PrepareMessageCreateFX(Npclist(TargetNPC).Char_Renamed.CharIndex, Hechizos(Spell).FXgrh, Hechizos(Spell).loops))
...
Errhandler: 
		...
		With UserList(UserIndex)
			...
			If Not TieneHechizo(hIndex, UserIndex) Then
				...
				If .Stats.UserHechizos(j) <> 0 Then
					Call WriteConsoleMsg(UserIndex, "No tienes espacio para más hechizos.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Else
	...
	Sub DecirPalabrasMagicas(ByVal SpellWords As String, ByVal UserIndex As Short)
		...
		With UserList(UserIndex)
			If .flags.AdminInvisible <> 1 Then
				Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessageChatOverHead(SpellWords, .Char_Renamed.CharIndex, System.Drawing.ColorTranslator.ToOle(System.Drawing.Color.Cyan)))
	...
	Function PuedeLanzar(ByVal UserIndex As Short, ByVal HechizoIndex As Short) As Boolean
		...
		With UserList(UserIndex)
			If .flags.Muerto Then
				Call WriteConsoleMsg(UserIndex, "No puedes lanzar hechizos estando muerto.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Function
			End If
				...
				If .clase = Declaraciones.eClass.Mage Then
					If .Invent.WeaponEqpObjIndex > 0 Then
						If ObjData_Renamed(.Invent.WeaponEqpObjIndex).StaffPower < Hechizos(HechizoIndex).NeedStaff Then
							Call WriteConsoleMsg(UserIndex, "No posees un báculo lo suficientemente poderoso para poder lanzar el conjuro.", Protocol.FontTypeNames.FONTTYPE_INFO)
							Exit Function
						End If
					Else
						Call WriteConsoleMsg(UserIndex, "No puedes lanzar este conjuro sin la ayuda de un báculo.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					End If
				End If
			End If
			...
			If .Stats.UserSkills(Declaraciones.eSkill.Magia) < Hechizos(HechizoIndex).MinSkill Then
				Call WriteConsoleMsg(UserIndex, "No tienes suficientes puntos de magia para lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Function
			End If
			
			If .Stats.MinSta < Hechizos(HechizoIndex).StaRequerido Then
				If .Genero = Declaraciones.eGenero.Hombre Then
					Call WriteConsoleMsg(UserIndex, "Estás muy cansado para lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Else
					Call WriteConsoleMsg(UserIndex, "Estás muy cansada para lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
				End If
				Exit Function
			End If
			...
			If .clase = Declaraciones.eClass.Druid Then
				...
				If Hechizos(HechizoIndex).Warp = 1 Then
					If .Stats.MinMAN <> .Stats.MaxMAN Then
						Call WriteConsoleMsg(UserIndex, "Debes poseer toda tu maná para poder lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
						' Si no tiene mascotas, no tiene sentido que lo use
					ElseIf .NroMascotas = 0 Then 
						Call WriteConsoleMsg(UserIndex, "Debes poseer alguna mascota para poder lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					End If
				End If
			End If
						...
						If InMapBounds(PosCasteadaM, TempX, TempY) Then
							If MapData(PosCasteadaM, TempX, TempY).UserIndex > 0 Then
								...
								If UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).flags.invisible = 1 And UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).flags.AdminInvisible = 0 Then
									Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessageCreateFX(UserList(MapData(PosCasteadaM, TempX, TempY).UserIndex).Char_Renamed.CharIndex, Hechizos(H).FXgrh, Hechizos(H).loops))
								End If
							End If
						End If
					Next TempY
	...
	Sub HechizoInvocacion(ByVal UserIndex As Short, ByRef HechizoCasteado As Boolean)
		...
		With UserList(UserIndex)
			...
			If MapInfo_Renamed(.Pos.Map).Pk = False Or MapData(.Pos.Map, .Pos.X, .Pos.Y).trigger = Declaraciones.eTrigger.ZONASEGURA Then
				Call WriteConsoleMsg(UserIndex, "No puedes invocar criaturas en zona segura.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Sub
			End If
	...
	Sub LanzarHechizo(ByVal SpellIndex As Short, ByVal UserIndex As Short)
		...
		With UserList(UserIndex)
			...
			If .flags.EnConsulta Then
				Call WriteConsoleMsg(UserIndex, "No puedes lanzar hechizos si estás en consulta.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Sub
			End If
				...
				Select Case Hechizos(SpellIndex).Target
					Case Declaraciones.TargetType.uUsuarios
						If .flags.TargetUser > 0 Then
							...
							Else
								Call WriteConsoleMsg(UserIndex, "Estás demasiado lejos para lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_WARNING)
							End If
						Else
							Call WriteConsoleMsg(UserIndex, "Este hechizo actúa sólo sobre usuarios.", Protocol.FontTypeNames.FONTTYPE_INFO)
						End If
					...
					Case Declaraciones.TargetType.uNPC
						If .flags.TargetNPC > 0 Then
							...
							End If
						Else
							Call WriteConsoleMsg(UserIndex, "Este hechizo sólo afecta a los npcs.", Protocol.FontTypeNames.FONTTYPE_INFO)
						End If
...
Errhandler: 
		...
		With UserList(UserIndex)
			...
			If Hechizos(HechizoIndex).Invisibilidad = 1 Then
				...
				If UserList(TargetIndex).Counters.Saliendo Then
					If UserIndex <> TargetIndex Then
						Call WriteConsoleMsg(UserIndex, "¡El hechizo no tiene efecto!", Protocol.FontTypeNames.FONTTYPE_INFO)
						HechizoCasteado = False
						Exit Sub
					Else
						Call WriteConsoleMsg(UserIndex, "¡No puedes hacerte invisible mientras te encuentras saliendo!", Protocol.FontTypeNames.FONTTYPE_WARNING)
						HechizoCasteado = False
						Exit Sub
					End If
				End If
				...
				If MapInfo_Renamed(UserList(TargetIndex).Pos.Map).InviSinEfecto > 0 Then
					Call WriteConsoleMsg(UserIndex, "¡La invisibilidad no funciona aquí!", Protocol.FontTypeNames.FONTTYPE_INFO)
					HechizoCasteado = False
					Exit Sub
				End If
			...
			If Hechizos(HechizoIndex).Mimetiza = 1 Then
				...
				If .flags.Mimetizado = 1 Then
					Call WriteConsoleMsg(UserIndex, "Ya te encuentras mimetizado. El hechizo no ha tenido efecto.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Sub
				End If
			...
			If Hechizos(HechizoIndex).Envenena = 1 Then
				If UserIndex = TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "No puedes atacarte a vos mismo.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Exit Sub
				End If
			...
			If Hechizos(HechizoIndex).Paraliza = 1 Or Hechizos(HechizoIndex).Inmoviliza = 1 Then
				...
				If UserList(TargetIndex).flags.Paralizado = 0 Then
					...
					If UserList(TargetIndex).Invent.AnilloEqpObjIndex = SUPERANILLO Then
						Call WriteConsoleMsg(TargetIndex, " Tu anillo rechaza los efectos del hechizo.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
						Call WriteConsoleMsg(UserIndex, " ¡El hechizo no tiene efecto!", Protocol.FontTypeNames.FONTTYPE_FIGHT)
			...
			If Hechizos(HechizoIndex).Revivir = 1 Then
				If UserList(TargetIndex).flags.Muerto = 1 Then
					...
					If UserList(TargetIndex).flags.SeguroResu Then
						Call WriteConsoleMsg(UserIndex, "¡El espíritu no tiene intenciones de regresar al mundo de los vivos!", Protocol.FontTypeNames.FONTTYPE_INFO)
						HechizoCasteado = False
						Exit Sub
					End If
					...
					If MapInfo_Renamed(UserList(TargetIndex).Pos.Map).ResuSinEfecto > 0 Then
						Call WriteConsoleMsg(UserIndex, "¡Revivir no está permitido aquí! Retirate de la Zona si deseas utilizar el Hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
						HechizoCasteado = False
						Exit Sub
					End If
					...
					If .Stats.MaxSta <> .Stats.MinSta Then
						Call WriteConsoleMsg(UserIndex, "No puedes resucitar si no tienes tu barra de energía llena.", Protocol.FontTypeNames.FONTTYPE_INFO)
						HechizoCasteado = False
						Exit Sub
					End If
					...
					If .clase = Declaraciones.eClass.Mage Then
						If .Invent.WeaponEqpObjIndex > 0 Then
							If ObjData_Renamed(.Invent.WeaponEqpObjIndex).StaffPower < Hechizos(HechizoIndex).NeedStaff Then
								Call WriteConsoleMsg(UserIndex, "Necesitas un báculo mejor para lanzar este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
								HechizoCasteado = False
								Exit Sub
							End If
						End If
					ElseIf .clase = Declaraciones.eClass.Bard Then 
						If .Invent.AnilloEqpObjIndex <> LAUDELFICO And .Invent.AnilloEqpObjIndex <> LAUDMAGICO Then
							Call WriteConsoleMsg(UserIndex, "Necesitas un instrumento mágico para devolver la vida.", Protocol.FontTypeNames.FONTTYPE_INFO)
							HechizoCasteado = False
							Exit Sub
						End If
					...
					If Not criminal(TargetIndex) Then
						If TargetIndex <> UserIndex Then
							...
							Call WriteConsoleMsg(UserIndex, "¡Los Dioses te sonríen, has ganado 500 puntos de nobleza!", Protocol.FontTypeNames.FONTTYPE_INFO)
						End If
					End If
					...
					If (.Stats.MinHp <= 0) Then
						Call UserDie(UserIndex)
						Call WriteConsoleMsg(UserIndex, "El esfuerzo de resucitar fue demasiado grande.", Protocol.FontTypeNames.FONTTYPE_INFO)
						HechizoCasteado = False
					Else
						Call WriteConsoleMsg(UserIndex, "El esfuerzo de resucitar te ha debilitado.", Protocol.FontTypeNames.FONTTYPE_INFO)
						HechizoCasteado = True
					End If
	...
	Sub HechizoEstadoNPC(ByVal NpcIndex As Short, ByVal SpellIndex As Short, ByRef HechizoCasteado As Boolean, ByVal UserIndex As Short)
		'***************************************************
		'Autor: Unknown (orginal version)
		...
		With Npclist(NpcIndex)
			...
			If Hechizos(SpellIndex).Paraliza = 1 Then
				If .flags.AfectaParalisis = 0 Then
					...
					HechizoCasteado = True
				Else
					Call WriteConsoleMsg(UserIndex, "El NPC es inmune a este hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
					HechizoCasteado = False
					Exit Sub
				End If
			End If
					...
					Else
						If .NPCtype = Declaraciones.eNPCType.GuardiaReal Then
							If esArmada(UserIndex) Then
								...
								Exit Sub
							Else
								Call WriteConsoleMsg(UserIndex, "Sólo puedes remover la parálisis de los Guardias si perteneces a su facción.", Protocol.FontTypeNames.FONTTYPE_INFO)
								HechizoCasteado = False
								Exit Sub
							End If
							
							Call WriteConsoleMsg(UserIndex, "Solo puedes remover la parálisis de los NPCs que te consideren su amo.", Protocol.FontTypeNames.FONTTYPE_INFO)
							HechizoCasteado = False
							Exit Sub
						Else
							If .NPCtype = Declaraciones.eNPCType.Guardiascaos Then
								If esCaos(UserIndex) Then
									...
									Exit Sub
								Else
									Call WriteConsoleMsg(UserIndex, "Solo puedes remover la parálisis de los Guardias si perteneces a su facción.", Protocol.FontTypeNames.FONTTYPE_INFO)
									HechizoCasteado = False
									Exit Sub
								End If
							End If
			...
			If Hechizos(SpellIndex).Inmoviliza = 1 Then
				If .flags.AfectaParalisis = 0 Then
					...
					HechizoCasteado = True
				Else
					Call WriteConsoleMsg(UserIndex, "El NPC es inmune al hechizo.", Protocol.FontTypeNames.FONTTYPE_INFO)
				End If
			End If
		End With
		...
		If Hechizos(SpellIndex).Mimetiza = 1 Then
			With UserList(UserIndex)
				...
				Else
					Call WriteConsoleMsg(UserIndex, "Sólo los druidas pueden mimetizarse con criaturas.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Sub
				End If
	...
	Sub HechizoPropNPC(ByVal SpellIndex As Short, ByVal NpcIndex As Short, ByVal UserIndex As Short, ByRef HechizoCasteado As Boolean)
		'***************************************************
		'Autor: Unknown (orginal version)
		...
		With Npclist(NpcIndex)
			...
			If Hechizos(SpellIndex).SubeHP = 1 Then
				...
				.Stats.MinHp = .Stats.MinHp + daño
				If .Stats.MinHp > .Stats.MaxHp Then .Stats.MinHp = .Stats.MaxHp
				Call WriteConsoleMsg(UserIndex, "Has curado " & daño & " puntos de vida a la criatura.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				HechizoCasteado = True
			...
			ElseIf Hechizos(SpellIndex).SubeHP = 2 Then 
				...
				If Hechizos(SpellIndex).StaffAffected Then
					If UserList(UserIndex).clase = Declaraciones.eClass.Mage Then
						If UserList(UserIndex).Invent.WeaponEqpObjIndex > 0 Then
							daño = (daño * (ObjData_Renamed(UserList(UserIndex).Invent.WeaponEqpObjIndex).StaffDamageBonus + 70)) / 100
				...
				If .flags.Snd2 > 0 Then
					Call SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, PrepareMessagePlayWave(.flags.Snd2, .Pos.X, .Pos.Y))
				End If
				...
				.Stats.MinHp = .Stats.MinHp - daño
				Call WriteConsoleMsg(UserIndex, "¡Le has quitado " & daño & " puntos de vida a la criatura!", Protocol.FontTypeNames.FONTTYPE_FIGHT)
		...
		With UserList(UserIndex)
			...
			If tUser > 0 Then
				...
				If .flags.AdminInvisible = 1 And UserIndex = tUser Then
					Call EnviarDatosASlot(UserIndex, PrepareMessageCreateFX(UserList(tUser).Char_Renamed.CharIndex, Hechizos(SpellIndex).FXgrh, Hechizos(SpellIndex).loops))
					Call EnviarDatosASlot(UserIndex, PrepareMessagePlayWave(Hechizos(SpellIndex).WAV, UserList(tUser).Pos.X, UserList(tUser).Pos.Y))
				Else
					Call SendData(modSendData.SendTarget.ToPCArea, tUser, PrepareMessageCreateFX(UserList(tUser).Char_Renamed.CharIndex, Hechizos(SpellIndex).FXgrh, Hechizos(SpellIndex).loops))
					Call SendData(modSendData.SendTarget.ToPCArea, tUser, PrepareMessagePlayWave(Hechizos(SpellIndex).WAV, UserList(tUser).Pos.X, UserList(tUser).Pos.Y)) 'Esta linea faltaba. Pablo (ToxicWaste)
				End If
			ElseIf tNPC > 0 Then 
				Call SendData(modSendData.SendTarget.ToNPCArea, tNPC, PrepareMessageCreateFX(Npclist(tNPC).Char_Renamed.CharIndex, Hechizos(SpellIndex).FXgrh, Hechizos(SpellIndex).loops))
				Call SendData(modSendData.SendTarget.ToNPCArea, tNPC, PrepareMessagePlayWave(Hechizos(SpellIndex).WAV, Npclist(tNPC).Pos.X, Npclist(tNPC).Pos.Y))
			End If
			
			If tUser > 0 Then
				If UserIndex <> tUser Then
					If .showName Then
						Call WriteConsoleMsg(UserIndex, Hechizos(SpellIndex).HechizeroMsg & " " & UserList(tUser).name, Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Else
						Call WriteConsoleMsg(UserIndex, Hechizos(SpellIndex).HechizeroMsg & " alguien.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					End If
					Call WriteConsoleMsg(tUser, .name & " " & Hechizos(SpellIndex).TargetMsg, Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, Hechizos(SpellIndex).PropioMsg, Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			ElseIf tNPC > 0 Then 
				Call WriteConsoleMsg(UserIndex, Hechizos(SpellIndex).HechizeroMsg & " " & "la criatura.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
			End If
		End With
	...
	Public Function HechizoPropUsuario(ByVal UserIndex As Short) As Boolean
		...
		With UserList(TargetIndex)
			If .flags.Muerto Then
				Call WriteConsoleMsg(UserIndex, "No puedes lanzar este hechizo a un muerto.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Function
			End If
			...
			If Hechizos(SpellIndex).SubeHam = 1 Then
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has restaurado " & daño & " puntos de hambre a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de hambre.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has restaurado " & daño & " puntos de hambre.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			ElseIf Hechizos(SpellIndex).SubeHam = 2 Then 
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has quitado " & daño & " puntos de hambre a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de hambre.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has quitado " & daño & " puntos de hambre.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			If Hechizos(SpellIndex).SubeSed = 1 Then
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has restaurado " & daño & " puntos de sed a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de sed.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has restaurado " & daño & " puntos de sed.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			ElseIf Hechizos(SpellIndex).SubeSed = 2 Then 
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has quitado " & daño & " puntos de sed a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de sed.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has quitado " & daño & " puntos de sed.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			If Hechizos(SpellIndex).SubeAgilidad = 1 Then
				...
				.Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) = .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) + daño
				If .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) > MinimoInt(MAXATRIBUTOS, .Stats.UserAtributosBackUP(Declaraciones.eAtributos.Agilidad) * 2) Then .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) = MinimoInt(MAXATRIBUTOS, .Stats.UserAtributosBackUP(Declaraciones.eAtributos.Agilidad) * 2)
			...
			ElseIf Hechizos(SpellIndex).SubeAgilidad = 2 Then 
				...
				.Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) = .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) - daño
				If .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) < MINATRIBUTOS Then .Stats.UserAtributos(Declaraciones.eAtributos.Agilidad) = MINATRIBUTOS
			...
			If Hechizos(SpellIndex).SubeFuerza = 1 Then
				...
				.Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) = .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) + daño
				If .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) > MinimoInt(MAXATRIBUTOS, .Stats.UserAtributosBackUP(Declaraciones.eAtributos.Fuerza) * 2) Then .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) = MinimoInt(MAXATRIBUTOS, .Stats.UserAtributosBackUP(Declaraciones.eAtributos.Fuerza) * 2)
			...
			ElseIf Hechizos(SpellIndex).SubeFuerza = 2 Then 
				...
				.flags.DuracionEfecto = 700
				.Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) = .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) - daño
				If .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) < MINATRIBUTOS Then .Stats.UserAtributos(Declaraciones.eAtributos.Fuerza) = MINATRIBUTOS
			...
			If Hechizos(SpellIndex).SubeHP = 1 Then
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has restaurado " & daño & " puntos de vida a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de vida.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has restaurado " & daño & " puntos de vida.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			ElseIf Hechizos(SpellIndex).SubeHP = 2 Then 
				...
				Call WriteConsoleMsg(UserIndex, "Le has quitado " & daño & " puntos de vida a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de vida.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
			...
			If Hechizos(SpellIndex).SubeMana = 1 Then
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has restaurado " & daño & " puntos de maná a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de maná.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has restaurado " & daño & " puntos de maná.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			ElseIf Hechizos(SpellIndex).SubeMana = 2 Then 
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has quitado " & daño & " puntos de maná a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de maná.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has quitado " & daño & " puntos de maná.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			If Hechizos(SpellIndex).SubeSta = 1 Then
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has restaurado " & daño & " puntos de energía a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha restaurado " & daño & " puntos de energía.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has restaurado " & daño & " puntos de energía.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
			...
			ElseIf Hechizos(SpellIndex).SubeSta = 2 Then 
				...
				If UserIndex <> TargetIndex Then
					Call WriteConsoleMsg(UserIndex, "Le has quitado " & daño & " puntos de energía a " & .name & ".", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					Call WriteConsoleMsg(TargetIndex, UserList(UserIndex).name & " te ha quitado " & daño & " puntos de energía.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				Else
					Call WriteConsoleMsg(UserIndex, "Te has quitado " & daño & " puntos de energía.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
				End If
	...
	Public Function CanSupportUser(ByVal CasterIndex As Short, ByVal TargetIndex As Short, Optional ByVal DoCriminal As Boolean = False) As Boolean
		'***************************************************
		'Author: ZaMa
		'Last Modification: 28/04/2010
		...
		With UserList(CasterIndex)
			...
			If .flags.EnConsulta Then
				Call WriteConsoleMsg(CasterIndex, "No puedes ayudar usuarios mientras estas en consulta.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Function
			End If
			...
			If criminal(TargetIndex) Then
				...
				If Not criminal(CasterIndex) Then
					...
					If esArmada(CasterIndex) Then
						Call WriteConsoleMsg(CasterIndex, "Los miembros del ejército real no pueden ayudar a los criminales.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					End If
					
					' Si el ciuda tiene el seguro puesto no puede ayudar
					If .flags.Seguro Then
						Call WriteConsoleMsg(CasterIndex, "Para ayudar criminales debes sacarte el seguro ya que te volverás criminal como ellos.", Protocol.FontTypeNames.FONTTYPE_INFO)
						Exit Function
					Else
						' Penalizacion
						If DoCriminal Then
			...
			Else
				...
				If esCaos(CasterIndex) Then
					Call WriteConsoleMsg(CasterIndex, "Los miembros de la legión oscura no pueden ayudar a los ciudadanos.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Function
				...
				ElseIf Not criminal(CasterIndex) Then 
					...
					If UserList(TargetIndex).flags.AtacablePor > 0 Then
						...
						If UserList(TargetIndex).flags.AtacablePor <> CasterIndex Then
							...
							If esArmada(CasterIndex) Then
								Call WriteConsoleMsg(CasterIndex, "Los miembros del ejército real no pueden ayudar a ciudadanos en estado atacable.", Protocol.FontTypeNames.FONTTYPE_INFO)
								Exit Function
							End If
							
							' Seguro puesto?
							If .flags.Seguro Then
								Call WriteConsoleMsg(CasterIndex, "Para ayudar ciudadanos en estado atacable debes sacarte el seguro, pero te puedes volver criminal.", Protocol.FontTypeNames.FONTTYPE_INFO)
								Exit Function
							Else
								Call DisNobAuBan(CasterIndex, .Reputacion.NobleRep * 0.5, 10000)
							End If
		...
		With UserList(UserIndex)
			If Dire = 1 Then 'Mover arriba
				If HechizoDesplazado = 1 Then
					Call WriteConsoleMsg(UserIndex, "No puedes mover el hechizo en esa dirección.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Exit Sub
				Else
	...
	Public Sub DisNobAuBan(ByVal UserIndex As Short, ByRef NoblePts As Integer, ByRef BandidoPts As Integer)
		...
		With UserList(UserIndex)
			...
			If .flags.Privilegios And (Declaraciones.PlayerType.User Or Declaraciones.PlayerType.Consejero) Then
				...
				Call WriteMultiMessage(UserIndex, Declaraciones.eMessages.NobilityLost) 'Call WriteNobilityLost(UserIndex)
