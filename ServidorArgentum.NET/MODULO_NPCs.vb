		...
		If UserIndex > 0 Then ' Lo mato un usuario?
			With UserList(UserIndex)
				...
				If MiNPC.flags.Snd3 > 0 Then
					Call SendData(modSendData.SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(MiNPC.flags.Snd3, MiNPC.Pos.X, MiNPC.Pos.Y))
				End If
				.flags.TargetNPC = 0
				...
				If MiNPC.flags.ExpCount > 0 Then
					...
					Else
						...
						Call WriteConsoleMsg(UserIndex, "Has ganado " & MiNPC.flags.ExpCount & " puntos de experiencia.", Protocol.FontTypeNames.FONTTYPE_FIGHT)
					End If
...
Errhandler: 
	...
	Public Sub MakeNPCChar(ByVal toMap As Boolean, ByRef sndIndex As Short, ByRef NpcIndex As Short, ByVal Map As Short, ByVal X As Short, ByVal Y As Short)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		If Not toMap Then
			Call WriteCharacterCreate(sndIndex, Npclist(NpcIndex).Char_Renamed.body, Npclist(NpcIndex).Char_Renamed.Head, Npclist(NpcIndex).Char_Renamed.heading, Npclist(NpcIndex).Char_Renamed.CharIndex, X, Y, 0, 0, 0, 0, 0, vbNullString, 0, 0)
	...
	Public Sub ChangeNPCChar(ByVal NpcIndex As Short, ByVal body As Short, ByVal Head As Short, ByVal heading As Declaraciones.eHeading)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		If NpcIndex > 0 Then
			With Npclist(NpcIndex).Char_Renamed
				...
				Call SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, PrepareMessageCharacterChange(body, Head, heading, .CharIndex, 0, 0, 0, 0, 0))
			End With
		End If
	End Sub
	...
	Private Sub EraseNPCChar(ByVal NpcIndex As Short)
		...
		Call SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, PrepareMessageCharacterRemove(Npclist(NpcIndex).Char_Renamed.CharIndex))
			...
			If LegalPosNPC(.Pos.Map, nPos.X, nPos.Y, .flags.AguaValida = 1, .MaestroUser <> 0) Then
				...
				If UserIndex > 0 Then
					...
					With UserList(UserIndex)
						...
						Call SendData(modSendData.SendTarget.ToPCAreaButIndex, UserIndex, PrepareMessageCharacterMove(UserList(UserIndex).Char_Renamed.CharIndex, .Pos.X, .Pos.Y))
						Call WriteForceCharMove(UserIndex, InvertHeading(nHeading))
					End With
				End If
				
				Call SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, PrepareMessageCharacterMove(.Char_Renamed.CharIndex, nPos.X, nPos.Y))
...
Errhandler: 
	...
	Function SpawnNpc(ByVal NpcIndex As Short, ByRef Pos As WorldPos, ByVal FX As Boolean, ByVal Respawn As Boolean) As Short
		'***************************************************
		'Autor: Unknown (orginal version)
		...
		If FX Then
			...
			Call SendData(modSendData.SendTarget.ToNPCArea, nIndex, PrepareMessageCreateFX(Npclist(nIndex).Char_Renamed.CharIndex, Declaraciones.FXIDs.FXWARP, 0))
		End If
