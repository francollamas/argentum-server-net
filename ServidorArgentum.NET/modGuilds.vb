...
Module modGuilds
	...
	Public Function m_ValidarPermanencia(ByVal UserIndex As Short, ByVal SumaAntifaccion As Boolean, ByRef CambioAlineacion As Boolean) As Boolean
		'***************************************************
	...
	Public Sub ActualizarNoticias(ByVal UserIndex As Short, ByRef Datos As String)
		...
		With UserList(UserIndex)
			...
			Call SendData(modSendData.SendTarget.ToDiosesYclan, .GuildIndex, PrepareMessageGuildChat(.name & " ha actualizado las noticias del clan!"))
		End With
	End Sub
	
	Public Function CrearNuevoClan(ByVal FundadorIndex As Short, ByRef desc As String, ByRef GuildName As String, ByRef URL As String, ByRef codex() As String, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Sub SendGuildNews(ByVal UserIndex As Short)
		...
		With guilds(GuildIndex)
			...
			If .EleccionesAbiertas Then
				Call WriteConsoleMsg(UserIndex, "Hoy es la votación para elegir un nuevo líder para el clan.", Protocol.FontTypeNames.FONTTYPE_GUILD)
				Call WriteConsoleMsg(UserIndex, "La elección durará 24 horas, se puede votar a cualquier miembro del clan.", Protocol.FontTypeNames.FONTTYPE_GUILD)
				Call WriteConsoleMsg(UserIndex, "Para votar escribe /VOTO NICKNAME.", Protocol.FontTypeNames.FONTTYPE_GUILD)
				Call WriteConsoleMsg(UserIndex, "Sólo se computará un voto por miembro. Tu voto no puede ser cambiado.", Protocol.FontTypeNames.FONTTYPE_GUILD)
			End If
		End With
		
	End Sub
	
	Public Function m_PuedeSalirDeClan(ByRef Nombre As String, ByVal GuildIndex As Short, ByVal QuienLoEchaUI As Short) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function PuedeFundarUnClan(ByVal UserIndex As Short, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
		'***************************************************
		'Autor: Unknown
		'Last Modification: 27/11/2009
		...
		If UserList(UserIndex).Stats.ELV < 25 Or UserList(UserIndex).Stats.UserSkills(Declaraciones.eSkill.Liderazgo) < 90 Then
		...
		Select Case Alineacion
			...
			Case ALINEACION_GUILD.ALINEACION_MASTER
				If UserList(UserIndex).flags.Privilegios And (Declaraciones.PlayerType.User Or Declaraciones.PlayerType.Consejero Or Declaraciones.PlayerType.SemiDios) Then
	...
	Public Sub v_RutinaElecciones()
		...
		On Error GoTo errh
		Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Revisando elecciones", Protocol.FontTypeNames.FONTTYPE_SERVER))
		For i = 1 To CANTIDADDECLANES
			If Not guilds(i) Is Nothing Then
				If guilds(i).RevisarElecciones Then
					Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> " & guilds(i).GetLeader & " es el nuevo líder de " & guilds(i).GuildName & ".", Protocol.FontTypeNames.FONTTYPE_SERVER))
				End If
			End If
proximo: 
		Next i
		Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Elecciones revisadas.", Protocol.FontTypeNames.FONTTYPE_SERVER))
		Exit Sub
errh: 
	...
	Public Function m_ListaDeMiembrosOnline(ByVal UserIndex As Short, ByVal GuildIndex As Short) As String
		...
		If GuildIndex > 0 And GuildIndex <= CANTIDADDECLANES Then
			...
			While i > 0
				...
				If i <> UserIndex And ((UserList(i).flags.Privilegios And (Declaraciones.PlayerType.User Or Declaraciones.PlayerType.Consejero Or Declaraciones.PlayerType.SemiDios)) <> 0 Or (UserList(UserIndex).flags.Privilegios And (Declaraciones.PlayerType.Dios Or Declaraciones.PlayerType.Admin) <> 0)) Then m_ListaDeMiembrosOnline = m_ListaDeMiembrosOnline & UserList(i).name & ","
				i = guilds(GuildIndex).m_Iterador_ProximoUserIndex
			End While
		End If
	...
	Public Function GMEscuchaClan(ByVal UserIndex As Short, ByVal GuildName As String) As Short
		...
		If LenB(GuildName) = 0 And UserList(UserIndex).EscucheClan <> 0 Then
			...
			Call WriteConsoleMsg(UserIndex, "Dejas de escuchar a : " & guilds(UserList(UserIndex).EscucheClan).GuildName, Protocol.FontTypeNames.FONTTYPE_GUILD)
	...
	End Sub
	Public Function r_DeclararGuerra(ByVal UserIndex As Short, ByRef GuildGuerra As String, ByRef refError As String) As Short
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function r_AceptarPropuestaDePaz(ByVal UserIndex As Short, ByRef GuildPaz As String, ByRef refError As String) As Short
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function r_RechazarPropuestaDeAlianza(ByVal UserIndex As Short, ByRef GuildPro As String, ByRef refError As String) As Short
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function r_RechazarPropuestaDePaz(ByVal UserIndex As Short, ByRef GuildPro As String, ByRef refError As String) As Short
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function r_AceptarPropuestaDeAlianza(ByVal UserIndex As Short, ByRef GuildAllie As String, ByRef refError As String) As Short
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function r_ClanGeneraPropuesta(ByVal UserIndex As Short, ByRef OtroClan As String, ByVal Tipo As RELACIONES_GUILD, ByRef Detalle As String, ByRef refError As String) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function r_VerPropuesta(ByVal UserIndex As Short, ByRef OtroGuild As String, ByVal Tipo As RELACIONES_GUILD, ByRef refError As String) As String
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function a_RechazarAspirante(ByVal UserIndex As Short, ByRef Nombre As String, ByRef refError As String) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Sub SendDetallesPersonaje(ByVal UserIndex As Short, ByVal Personaje As String)
		...
		If GI <= 0 Or GI > CANTIDADDECLANES Then
			Call Protocol.WriteConsoleMsg(UserIndex, "No perteneces a ningún clan.", Protocol.FontTypeNames.FONTTYPE_INFO)
			Exit Sub
		End If
		
		If Not m_EsGuildLeader(UserList(UserIndex).name, GI) Then
			Call Protocol.WriteConsoleMsg(UserIndex, "No eres el líder de tu clan.", Protocol.FontTypeNames.FONTTYPE_INFO)
			Exit Sub
		End If
		...
		If NroAsp = 0 Then
			...
			If i > UBound(list) Then
				Call Protocol.WriteConsoleMsg(UserIndex, "El personaje no es ni aspirante ni miembro del clan.", Protocol.FontTypeNames.FONTTYPE_INFO)
				Exit Sub
			End If
		End If
		...
		With UserFile
			...
			Call Protocol.WriteCharacterInfo(UserIndex, Personaje, CShort(.GetValue("INIT", "Raza")), CShort(.GetValue("INIT", "Clase")), CShort(.GetValue("INIT", "Genero")), CByte(.GetValue("STATS", "ELV")), CInt(.GetValue("STATS", "GLD")), CInt(.GetValue("STATS", "Banco")), CInt(.GetValue("REP", "Promedio")), .GetValue("GUILD", "Pedidos"), GuildName, Miembro, CBool(.GetValue("FACCIONES", "EjercitoReal")), CBool(.GetValue("FACCIONES", "EjercitoCaos")), CInt(.GetValue("FACCIONES", "CiudMatados")), CInt(.GetValue("FACCIONES", "CrimMatados")))
		End With
...
error_Renamed: 
		'UPGRADE_NOTE: El objeto UserFile no se puede destruir hasta que no se realice la recolección de los elementos no utilizados. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
		UserFile = Nothing
		If Not (FileExist(CharPath & Personaje & ".chr", FileAttribute.Archive)) Then
	...
	Public Function a_NuevoAspirante(ByVal UserIndex As Short, ByRef clan As String, ByRef Solicitud As String, ByRef refError As String) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
	...
	Public Function a_AceptarAspirante(ByVal UserIndex As Short, ByRef Aspirante As String, ByRef refError As String) As Boolean
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
