	...
	Public Sub MandarMensajeAConsola(ByVal texto As String, ByVal Sender As String)
		...
		For i = 1 To PARTY_MAXMEMBERS
			If p_members(i).UserIndex > 0 Then
				Call WriteConsoleMsg(p_members(i).UserIndex, " [" & Sender & "] " & texto, Protocol.FontTypeNames.FONTTYPE_PARTY)
			End If
		Next i
			...
			For j = PARTY_MAXMEMBERS To 1 Step -1
				...
				With p_members(j)
					...
					If .UserIndex > 0 Then
						...
						If j <> 1 Then
							Call WriteConsoleMsg(.UserIndex, "Abandonas la party liderada por " & UserList(p_members(1).UserIndex).name & ".", Protocol.FontTypeNames.FONTTYPE_PARTY)
						End If
						
						Call WriteConsoleMsg(.UserIndex, "Durante la misma has conseguido " & CStr(Fix(.Experiencia)) & " puntos de experiencia.", Protocol.FontTypeNames.FONTTYPE_PARTY)
		...
		Else
			If i <= PARTY_MAXMEMBERS Then
				...
				With UserList(MemberIndex)
					...
					Call WriteConsoleMsg(MemberIndex, "Durante la misma has conseguido " & CStr(Fix(p_members(i).Experiencia)) & " puntos de experiencia.", Protocol.FontTypeNames.FONTTYPE_PARTY)
		...
		If Not PARTY_EXPERIENCIAPORGOLPE Then 'esto sirve SOLO cuando acumulamos la experiencia!
			For i = 1 To PARTY_MAXMEMBERS
				If p_members(i).UserIndex > 0 Then
					...
					Else
						If System.Math.Abs(UserList(p_members(i).UserIndex).Stats.Exp) > System.Math.Abs(Fix(p_members(i).Experiencia)) Then
							UserList(p_members(i).UserIndex).Stats.Exp = UserList(p_members(i).UserIndex).Stats.Exp + Fix(p_members(i).Experiencia)
						Else
