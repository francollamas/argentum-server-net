		...
		With Npclist(NpcIndex)
			If .flags.Inmovilizado = 1 Then
				...
				For i = 1 To ModAreas.ConnGroups(.Pos.Map).CountEntrys
					...
					If System.Math.Abs(UserList(UserIndex).Pos.X - .Pos.X) <= RANGO_VISION_X And System.Math.Sign(UserList(UserIndex).Pos.X - .Pos.X) = SignoEO Then
						If System.Math.Abs(UserList(UserIndex).Pos.Y - .Pos.Y) <= RANGO_VISION_Y And System.Math.Sign(UserList(UserIndex).Pos.Y - .Pos.Y) = SignoNS Then
		...
		With Npclist(NpcIndex)
			If .flags.Paralizado = 1 Or .flags.Inmovilizado = 1 Then
				...
				For i = 1 To ModAreas.ConnGroups(.Pos.Map).CountEntrys
					...
					'Is it in it's range of vision??
					If System.Math.Abs(UserList(UI).Pos.X - .Pos.X) <= RANGO_VISION_X And System.Math.Sign(UserList(UI).Pos.X - .Pos.X) = SignoEO Then
						If System.Math.Abs(UserList(UI).Pos.Y - .Pos.Y) <= RANGO_VISION_Y And System.Math.Sign(UserList(UI).Pos.Y - .Pos.Y) = SignoNS Then
				...
				Case TipoAI.NpcPathfinding
					...
					If ReCalculatePath(NpcIndex) Then
						...
						If .PFINFO.NoPath Then 'Si no existe nos movemos al azar
							'Move randomly
							Call MoveNPCChar(NpcIndex, RandomNumber(Declaraciones.eHeading.NORTH, Declaraciones.eHeading.WEST))
						End If
					Else
