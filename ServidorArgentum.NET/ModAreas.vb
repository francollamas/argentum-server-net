...
Module ModAreas
	...
	Public Sub AreasOptimizacion()
		...
		If (CurDay <> IIf(WeekDay(Today) > 6, 1, 2)) Or (CurHour <> Fix(Hour(TimeOfDay) \ 3)) Then
			...
			For LoopC = 1 To NumMaps
				...
				Call WriteVar(DatPath & "AreasStats.dat", "Mapa" & LoopC, CurDay & "-" & CurHour, CStr(CShort((EntryValue + ConnGroups(LoopC).OptValue) \ 2)))
	...
	Public Sub CheckUpdateNeededUser(ByVal UserIndex As Short, ByVal Head As Byte, Optional ByVal ButIndex As Boolean = False)
		'**************************************************************
		'Author: Lucio N. Tourrilhes (DuNga)
						...
						If UserIndex <> TempInt Then
							...
							If Not (UserList(TempInt).flags.AdminInvisible = 1) Then
								...
								If UserList(TempInt).flags.invisible Or UserList(TempInt).flags.Oculto Then
									If .flags.Privilegios And (Declaraciones.PlayerType.User Or Declaraciones.PlayerType.Consejero Or Declaraciones.PlayerType.RoleMaster) Then
										Call WriteSetInvisible(UserIndex, UserList(TempInt).Char_Renamed.CharIndex, True)
									End If
								End If
							End If
