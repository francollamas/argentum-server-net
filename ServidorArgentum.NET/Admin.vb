...
Module Admin
	...
	Sub WorldSave()
		...
		Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> Iniciando WorldSave", Protocol.FontTypeNames.FONTTYPE_SERVER))
...
#End If
		...
		Call SendData(modSendData.SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Servidor> WorldSave ha concluído.", Protocol.FontTypeNames.FONTTYPE_SERVER))
	...
	Public Sub Encarcelar(ByVal UserIndex As Short, ByVal Minutos As Integer, Optional ByVal GmName As String = vbNullString)
		'***************************************************
		'Author: Unknown
		'Last Modification: -
		'
		...
		If LenB(GmName) = 0 Then
			Call WriteConsoleMsg(UserIndex, "Has sido encarcelado, deberás permanecer en la cárcel " & Minutos & " minutos.", Protocol.FontTypeNames.FONTTYPE_INFO)
		Else
			Call WriteConsoleMsg(UserIndex, GmName & " te ha encarcelado, deberás permanecer en la cárcel " & Minutos & " minutos.", Protocol.FontTypeNames.FONTTYPE_INFO)
		End If
	...
	Public Function BANCheck(ByVal name As String) As Boolean
		...
		BANCheck = (Val(GetVar(My.Application.Info.DirectoryPath & "\charfile\" & name & ".chr", "FLAGS", "Ban")) = 1)
	...
	Public Function UnBan(ByVal name As String) As Boolean
		...
		'Unban the character
		Call WriteVar(My.Application.Info.DirectoryPath & "\charfile\" & name & ".chr", "FLAGS", "Ban", "0")
		
		'Remove it from the banned people database
		Call WriteVar(My.Application.Info.DirectoryPath & "\logs\" & "BanDetail.dat", name, "BannedBy", "NOBODY")
		Call WriteVar(My.Application.Info.DirectoryPath & "\logs\" & "BanDetail.dat", name, "Reason", "NO REASON")
	End Function
	...
	Public Sub BanIpGuardar()
		...
		ArchivoBanIp = My.Application.Info.DirectoryPath & "\Dat\BanIps.dat"
	...
	Public Sub BanCharacter(ByVal bannerUserIndex As Short, ByVal UserName As String, ByVal reason As String)
		...
		With UserList(bannerUserIndex)
			If tUser <= 0 Then
				Call WriteConsoleMsg(bannerUserIndex, "El usuario no está online.", Protocol.FontTypeNames.FONTTYPE_TALK)
				...
				If FileExist(CharPath & UserName & ".chr", vbNormal) Then
					...
					If (userPriv And rank) > (.flags.Privilegios And rank) Then
						Call WriteConsoleMsg(bannerUserIndex, "No puedes banear a al alguien de mayor jerarquía.", Protocol.FontTypeNames.FONTTYPE_INFO)
					Else
						...
						Else
							...
							Call SendData(modSendData.SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> " & .name & " ha baneado a " & UserName & ".", Protocol.FontTypeNames.FONTTYPE_SERVER))
							...
							Call WriteVar(CharPath & UserName & ".chr", "PENAS", "P" & cantPenas + 1, LCase(.name) & ": BAN POR " & LCase(reason) & " " & Today & " " & TimeOfDay)
							
							If (userPriv And rank) = (.flags.Privilegios And rank) Then
								.flags.Ban = 1
								Call SendData(modSendData.SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(.name & " banned by the server por bannear un Administrador.", Protocol.FontTypeNames.FONTTYPE_FIGHT))
					...
					End If
				Else
					Call WriteConsoleMsg(bannerUserIndex, "El pj " & UserName & " no existe.", Protocol.FontTypeNames.FONTTYPE_INFO)
				End If
			Else
				...
				Call SendData(modSendData.SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg("Servidor> " & .name & " ha baneado a " & UserList(tUser).name & ".", Protocol.FontTypeNames.FONTTYPE_SERVER))
