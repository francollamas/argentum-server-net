...
internal class clsParty
{
    ...
    public void UpdateSumaNivelesElevados(short Lvl)
    {
        ...
        p_SumaNivelesElevados = Convert.ToSingle(p_SumaNivelesElevados -
            Math.Pow(Lvl - 1, mdParty.ExponenteNivelParty) + Math.Pow(Lvl, mdParty.ExponenteNivelParty));
    }
    ...
    public void ObtenerExito(int ExpGanada, short mapa, ref short X, ref short Y)
    {
        ...
        for (i = 1; i <= mdParty.PARTY_MAXMEMBERS; i++)
        {
            ...
            if (UI > 0)
            {
                ...
                expThisUser = Convert.ToDouble(ExpGanada *
                                               Math.Pow(Declaraciones.UserList[p_members[i].UserIndex].Stats.ELV,
                                                   mdParty.ExponenteNivelParty) /
                                               p_SumaNivelesElevados);

                if ((mapa == Declaraciones.UserList[UI].Pos.Map) & (Declaraciones.UserList[UI].flags.Muerto == 0))
                    if (Matematicas.Distance(Declaraciones.UserList[UI].Pos.X, Declaraciones.UserList[UI].Pos.Y, X,
                            Y) <= mdParty.PARTY_MAXDISTANCIA)
                    {
    ...
    public bool NuevoMiembro(short UserIndex)
    {
        ...
        if (i <= mdParty.PARTY_MAXMEMBERS)
        {
            ...
            p_SumaNivelesElevados = Convert.ToSingle(p_SumaNivelesElevados +
                                                     Math.Pow(Declaraciones.UserList[UserIndex].Stats.ELV,
                                                         mdParty.ExponenteNivelParty));
        }
        else
        {
                ...
                if (withBlock.UserIndex > 0)
                {
                    ...
                    if (j != 1)
                        Protocol.WriteConsoleMsg(withBlock.UserIndex,
                            "Abandonas la party liderada por " + Declaraciones.UserList[p_members[1].UserIndex].name +
                            ".", Protocol.FontTypeNames.FONTTYPE_PARTY);
                    ...
                    MandarMensajeAConsola(Declaraciones.UserList[withBlock.UserIndex].name + " abandona la party.",
                        "Servidor");
    ...
    public bool HacerLeader(short UserIndex)
    {
        ...
        if (UserIndexIndex == 0)
        {
            ...
            var argdesc = "INCONSISTENCIA DE PARTIES";
    ...
    public bool PuedeEntrar(short UserIndex, ref string razon)
    {
        ...
        rv = Matematicas.Distancia(ref Declaraciones.UserList[p_members[1].UserIndex].Pos,
            ref Declaraciones.UserList[UserIndex].Pos) <= mdParty.MAXDISTANCIAINGRESOPARTY;
        if (rv)
        {
