...
namespace Legacy
{
    ...
    static class GameLoop
    {
        ...
        private readonly static int piqueteCInterval = 6000;
        private readonly static int packetResendInterval = 10;
        private readonly static int fxInterval = 4000;
        private readonly static int auditoriaInterval = 1000;
        private readonly static int gameTimerInterval = 40;
        private readonly static int lluviaEventInterval = 60000;
        public static int lluviaInterval = 500;
        private readonly static int autoSaveInterval = 60000;
        public static int npcAtacaInterval = 4000;
        private readonly static int killLogInterval = 60000;
        public static int timerAIInterval = 100;
        private readonly static int connectionTimerInterval = 5;

        // Últimos tiempos de ejecución
        private static double lastPiqueteC = 0d;
        private static double lastPacketResend = 0d;
        private static double lastFx = 0d;
        private static double lastAuditoria = 0d;
        private static double lastGameTimer = 0d;
        private static double lastLluviaEvent = 0d;
        private static double lastLluvia = 0d;
        private static double lastAutoSave = 0d;
        private static double lastNpcAtaca = 0d;
        private static double lastKillLog = 0d;
        private static double lastTimerAI = 0d;
        private static double lastConnection = 0d;
        ...
        private static void TickPiqueteC()
        {
                ...
                {
                    {
                        ...
                        {
                            if (Declaraciones.MapData[withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y].trigger == Declaraciones.eTrigger.ANTIPIQUETE)
                            {
                                withBlock.Counters.PiqueteC = withBlock.Counters.PiqueteC + 1;
                                Protocol.WriteConsoleMsg(i, "¡¡¡Estás obstruyendo la vía pública, muévete o serás encarcelado!!!", Protocol.FontTypeNames.FONTTYPE_INFO);
                            ...
                            if (withBlock.flags.Muerto == 1)
                            {
                                if (withBlock.flags.Traveling == 1)
                                {
                                    ...
                                    {
                                        Extra.FindLegalPos(i, Declaraciones.Ciudades[(int)withBlock.Hogar].Map, ref Declaraciones.Ciudades[(int)withBlock.Hogar].X, ref Declaraciones.Ciudades[(int)withBlock.Hogar].Y);
                                        UsUaRiOs.WarpUserChar(i, Declaraciones.Ciudades[(int)withBlock.Hogar].Map, Declaraciones.Ciudades[(int)withBlock.Hogar].X, Declaraciones.Ciudades[(int)withBlock.Hogar].Y, true);
                            ...
                            if (GI > 0)
                            {
                                ...
                                {
                                    Protocol.WriteConsoleMsg(i, "Has sido expulsado del clan. ¡El clan ha sumado un punto de antifacción!", Protocol.FontTypeNames.FONTTYPE_GUILD);
                                }
                                if (NuevaA)
                                {
                                    modSendData.SendData(modSendData.SendTarget.ToGuildMembers, GI, Protocol.PrepareMessageConsoleMsg("¡El clan ha pasado a tener alineación " + modGuilds.GuildAlignment(GI) + "!", Protocol.FontTypeNames.FONTTYPE_GUILD));
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en tPiqueteC_Timer " + ex.GetType().Name + ": " + ex.Message;
        ...
        private static void TickPacketResend()
        {
            ...
            try
            {
                ...
                for (i = 1; i <= loopTo; i++)
                {
                    if (Declaraciones.UserList[i].ConnIDValida)
                    {
                        ...
                        {
                            string argdatos = Declaraciones.UserList[i].outgoingData.ReadASCIIStringFixed(Declaraciones.UserList[i].outgoingData.length);
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en packetResend - Error: " + ex.GetType().Name + " - Desc: " + ex.Message;
                General.LogError(ref argdesc);
            }
        }
        ...
        private static byte _TickAuditoria_centinelSecs = default;
        ...
        private static void TickAuditoria()
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en Timer Auditoria. Err: " + ex.Message + " - " + ex.GetType().Name;
                General.LogError(ref argdesc);
            }
        }
        ...
        private static void TickGameTimer()
        {
                        ...
                        if (withBlock.ConnID != -1)
                        {
                            ...
                            if (withBlock.ConnIDValida & withBlock.flags.UserLogged)
                            {
                                ...
                                if (withBlock.flags.Muerto == 0)
                                {
                                    ...
                                    if (withBlock.flags.Desnudo != 0 & (withBlock.flags.Privilegios & Declaraciones.PlayerType.User) != 0)
                                        General.EfectoFrio(iUserIndex);

                                    if (withBlock.flags.Meditando)
                                        Trabajo.DoMeditar(iUserIndex);

                                    if (withBlock.flags.Envenenado != 0 & (withBlock.flags.Privilegios & Declaraciones.PlayerType.User) != 0)
                                        ...
                                        if (Admin.Lloviendo)
                                        {
                                            if (!General.Intemperie(iUserIndex))
                                            {
                                                if (!withBlock.flags.Descansar)
                                                {
                                                    ...
                                                    General.RecStamina(iUserIndex, ref bEnviarStats, Admin.StaminaIntervaloSinDescansar);
                                                    if (bEnviarStats)
                                                    {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (withBlock.Stats.MaxHp == withBlock.Stats.MinHp & withBlock.Stats.MaxSta == withBlock.Stats.MinSta)
                                                    {
                                                        Protocol.WriteRestOK(iUserIndex);
                                                        Protocol.WriteConsoleMsg(iUserIndex, "Has terminado de descansar.", Protocol.FontTypeNames.FONTTYPE_INFO);
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en GameTimer: " + ex.Message + " UserIndex = " + iUserIndex;
                General.LogError(ref argdesc);
            }
        }

        private static int _TickLluviaEvent_MinutosLloviendo = default;
        private static int _TickLluviaEvent_MinutosSinLluvia = default;
        ...
        private static void TickLluviaEvent()
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error tLluviaTimer";
                General.LogError(ref argdesc);
            }
        }
        ...
        private static void TickLluvia()
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "tLluvia " + ex.GetType().Name + ": " + ex.Message;
                General.LogError(ref argdesc);
            }
        }

        private static int _TickAutoSave_Minutos = default;
        private static int _TickAutoSave_MinutosLatsClean = default;
        ...
        private static void TickAutoSave()
        {
            ...
            try
            {
                ...
                {
                    modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Worldsave en 1 minuto ...", Protocol.FontTypeNames.FONTTYPE_VENENO));
                }
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en TimerAutoSave " + ex.GetType().Name + ": " + ex.Message;
                General.LogError(ref argdesc);
            }
        }
        ...
        private static void TickKillLog()
        {
            try
            {
                if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/connect.log"))
                    FileSystem.Kill(AppDomain.CurrentDomain.BaseDirectory + "logs/connect.log");
                if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/haciendo.log"))
                    FileSystem.Kill(AppDomain.CurrentDomain.BaseDirectory + "logs/haciendo.log");
                if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/stats.log"))
                    FileSystem.Kill(AppDomain.CurrentDomain.BaseDirectory + "logs/stats.log");
                if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/Asesinatos.log"))
                    FileSystem.Kill(AppDomain.CurrentDomain.BaseDirectory + "logs/Asesinatos.log");
                if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/HackAttemps.log"))
                    FileSystem.Kill(AppDomain.CurrentDomain.BaseDirectory + "logs/HackAttemps.log");
                if (!System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/nokillwsapi.txt"))
                {
                    if (System.IO.File.Exists(AppDomain.CurrentDomain.BaseDirectory + "logs/wsapi.log"))
        ...
        private static void TickAI()
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en TIMER_AI_Timer " + Declaraciones.Npclist[NpcIndex].name + " mapa:" + Declaraciones.Npclist[NpcIndex].Pos.Map;
                General.LogError(ref argdesc);
                        ...
                        if (withBlock.Counters.IdleCount >= Declaraciones.IdleLimit)
                        {
                            ...
                            if (withBlock.ComUsu.DestUsu > 0)
                            {
                                if (Declaraciones.UserList[withBlock.ComUsu.DestUsu].flags.UserLogged)
                                {
                                    ...
                                    {
                                        Protocol.WriteConsoleMsg(withBlock.ComUsu.DestUsu, "Comercio cancelado por el otro usuario.", Protocol.FontTypeNames.FONTTYPE_TALK);
                                        mdlCOmercioConUsuario.FinComerciarUsu(withBlock.ComUsu.DestUsu);
                                        Protocol.FlushBuffer(withBlock.ComUsu.DestUsu); // flush the buffer to send the message right away
                                    }
                                }
        ...
        public static void CloseServer()
        {
            try
            {
                ...
                // Log
                General.AppendLog("logs/Main.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " server cerrado.");
