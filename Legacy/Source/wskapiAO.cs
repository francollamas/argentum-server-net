...
namespace Legacy
{
    ...
    static class wskapiAO
    {
        /// <summary>
    ///     Maps socket IDs to user indices
    /// </summary>
        private readonly static Dictionary<int, int> SocketToUserMap = new Dictionary<int, int>();
    ...
    /// </summary>
        public static string GetAscIP(int socketID)
        {
            try
            {
                int userIdx = BuscaSlotSock(socketID);
                if (userIdx > 0)
                {
                    return Declaraciones.UserList[userIdx].ip;
                }
            }
            catch
            {
    ...
    /// </summary>
        private static void HandleConnectionReceived(int socketID, string clientIP)
        {
            ...
            if (NewIndex <= Declaraciones.MaxUsers)
            {
                ...
                {
                    string bannedIP = Declaraciones.BanIps[i].ToString();
                ...
                {
                    Declaraciones.LastUser = Convert.ToInt16(NewIndex);
                }
            ...
            {
                string errorMsg = Protocol.PrepareMessageErrorMsg("El servidor se encuentra lleno en este momento. Disculpe las molestias ocasionadas.");
                byte[] data = SocketManager.StringToBytes(errorMsg);
    ...
    /// </summary>
        private static void HandleDataReceived(int socketID, byte[] data)
        {
            ...
            int userIndex = BuscaSlotSock(socketID);
            ...
            {
                {
                    ...
                    {
                        Protocol.HandleIncomingData(Convert.ToInt16(userIndex));
                    }
                }
            }
        }
    ...
    /// </summary>
        public static int WsApiEnviar(int userIndex, ref string message)
        {
            ...
            try
            {
                ...
                {
                    byte[] data = SocketManager.StringToBytes(message);

                    if (!SocketManager.SendData(socketID, data))
                    {
                        returnCode = -1;
                    }
    ...
    /// </summary>
        public static int BuscaSlotSock(int socketID)
        {
            ...
            if (SocketToUserMap.TryGetValue(socketID, out userIndex))
            {
                return userIndex;
            }
    ...
    /// </summary>
        ...
        {
            int count = SocketToUserMap.Count;
