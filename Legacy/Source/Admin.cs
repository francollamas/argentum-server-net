...
namespace Legacy
{
    static class Admin
    {
        ...
        public static clsEstadisticasIPC EstadisticasWeb = new clsEstadisticasIPC();
            ...
            try
            {
                ...
                for (i = 1; i <= loopTo; i++)
                {
                    ...
                    if (Declaraciones.Npclist[i].flags.NPCActive)
                    {
                        ...
                        if (Extra.InMapBounds(Declaraciones.Npclist[i].Orig.Map, Declaraciones.Npclist[i].Orig.X, Declaraciones.Npclist[i].Orig.Y) & Declaraciones.Npclist[i].Numero == Declaraciones.Guardias)
                        {
        ...
        public static void WorldSave()
        {
            ...
            try
            {
                ...
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Iniciando WorldSave", Protocol.FontTypeNames.FONTTYPE_SERVER));
                ...
                if (System.IO.File.Exists(Declaraciones.DatPath + "/bkNpc.dat"))
                ...
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> WorldSave ha concluído.", Protocol.FontTypeNames.FONTTYPE_SERVER));
            }
                ...
                if (Declaraciones.UserList[i].flags.UserLogged)
                {
                    if (Declaraciones.UserList[i].Counters.Pena > 0)
                    {
                        ...
                        if (Declaraciones.UserList[i].Counters.Pena < 1)
                        {
                            ...
                            UsUaRiOs.WarpUserChar(Convert.ToInt16(i), Declaraciones.Libertad.Map, Declaraciones.Libertad.X, Declaraciones.Libertad.Y, true);
                            Protocol.WriteConsoleMsg(Convert.ToInt16(i), "¡Has sido liberado!", Protocol.FontTypeNames.FONTTYPE_INFO);
        ...
        public static void Encarcelar(short UserIndex, int Minutos, string GmName = Constants.vbNullString)
        {
            ...
            UsUaRiOs.WarpUserChar(UserIndex, Declaraciones.Prision.Map, Declaraciones.Prision.X, Declaraciones.Prision.Y, true);

            if (Migration.migr_LenB(GmName) == 0)
            {
                Protocol.WriteConsoleMsg(UserIndex, "Has sido encarcelado, deberás permanecer en la cárcel " + Minutos + " minutos.", Protocol.FontTypeNames.FONTTYPE_INFO);
            }
            else
            {
                Protocol.WriteConsoleMsg(UserIndex, GmName + " te ha encarcelado, deberás permanecer en la cárcel " + Minutos + " minutos.", Protocol.FontTypeNames.FONTTYPE_INFO);
            }
        ...
        public static void BorrarUsuario(string UserName)
        {
            ...
            try
            {
                if (System.IO.File.Exists(Declaraciones.CharPath + UserName.ToUpper() + ".chr"))
                {
        ...
        public static bool BANCheck(string name)
        {
            ...
            BANCheckRet = Migration.ParseVal(ES.GetVar(AppDomain.CurrentDomain.BaseDirectory + "charfile/" + name + ".chr", "FLAGS", "Ban", EmptySpaces: ref argEmptySpaces)) == 1d;
            return BANCheckRet;
        }
        ...
        public static bool PersonajeExiste(string name)
        {
            ...
            PersonajeExisteRet = System.IO.File.Exists(Declaraciones.CharPath + name.ToUpper() + ".chr");
        ...
        public static void MD5sCarga()
        {
            ...
            int argEmptySpaces = 1024;
            Declaraciones.MD5ClientesActivado = Convert.ToByte(Migration.ParseVal(ES.GetVar(Declaraciones.IniPath + "Server.ini", "MD5Hush", "Activado", EmptySpaces: ref argEmptySpaces)));

            if (Declaraciones.MD5ClientesActivado == 1)
            {
                int argEmptySpaces1 = 1024;
                Declaraciones.MD5s = new string[Convert.ToInt32(Migration.ParseVal(ES.GetVar(Declaraciones.IniPath + "Server.ini", "MD5Hush", "MD5Aceptados", EmptySpaces: ref argEmptySpaces1))) + 1];
                var loopTo = Convert.ToInt16(Declaraciones.MD5s.Length - 1);
                for (LoopC = 0; LoopC <= loopTo; LoopC++)
                {
                    int argEmptySpaces2 = 1024;
                    Declaraciones.MD5s[LoopC] = ES.GetVar(Declaraciones.IniPath + "Server.ini", "MD5Hush", "MD5Aceptado" + (LoopC + 1), EmptySpaces: ref argEmptySpaces2);
                    Declaraciones.MD5s[LoopC] = modHexaStrings.txtOffset(modHexaStrings.hexMd52Asc(Declaraciones.MD5s[LoopC]), 55);
                }
            }
        }
        ...
        {
            string ArchivoBanIp = AppDomain.CurrentDomain.BaseDirectory + "Dat/BanIps.dat";
            var lines = new List<string>();

            for (int LoopC = 1, loopTo = Declaraciones.BanIps.Count; LoopC <= loopTo; LoopC++)
                lines.Add(Declaraciones.BanIps[LoopC]);

            System.IO.File.WriteAllLines(ArchivoBanIp, lines);
        }
        ...
        public static void BanIpCargar()
        {
            ...
            foreach (string line in System.IO.File.ReadAllLines(ArchivoBanIp))
                Declaraciones.BanIps.Add(line);
        }

        private static bool _ActualizaEstadisticasWeb_Andando = default;
        private static int _ActualizaEstadisticasWeb_Contador = default;
        ...
        public static void ActualizaEstadisticasWeb()
        {
            ...
            if (_ActualizaEstadisticasWeb_Contador >= 10)
            {
                ...
                if (_ActualizaEstadisticasWeb_Andando == false & Tmp == true)
                {
        ...
        public static void BanCharacter(short bannerUserIndex, string UserName, string reason)
        {
...

            {
                ...
                if (tUser <= 0)
                {
                    Protocol.WriteConsoleMsg(bannerUserIndex, "El usuario no está online.", Protocol.FontTypeNames.FONTTYPE_TALK);
                    ...
                    if (System.IO.File.Exists(Declaraciones.CharPath + UserName + ".chr"))
                    {
                        ...
                        {
                            Protocol.WriteConsoleMsg(bannerUserIndex, "No puedes banear a al alguien de mayor jerarquía.", Protocol.FontTypeNames.FONTTYPE_INFO);
                        }
                        else
                        {
                            int argEmptySpaces1 = 1024;
                            if (ES.GetVar(Declaraciones.CharPath + UserName + ".chr", "FLAGS", "Ban", EmptySpaces: ref argEmptySpaces1) != "0")
                            {
                                Protocol.WriteConsoleMsg(bannerUserIndex, "El personaje ya se encuentra baneado.", Protocol.FontTypeNames.FONTTYPE_INFO);
                            }
                            else
                            {
                                ES.LogBanFromName(UserName, bannerUserIndex, reason);
                                modSendData.SendData(modSendData.SendTarget.ToAdmins, 0, Protocol.PrepareMessageConsoleMsg("Servidor> " + withBlock.name + " ha baneado a " + UserName + ".", Protocol.FontTypeNames.FONTTYPE_SERVER));
                                ...
                                ES.WriteVar(Declaraciones.CharPath + UserName + ".chr", "PENAS", "P" + (cantPenas + 1), withBlock.name.ToLower() + ": BAN POR " + reason.ToLower() + " " + Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay));

                                if ((userPriv & rank) == ((int)withBlock.flags.Privilegios & rank))
                                {
                                    withBlock.flags.Ban = 1;
                                    modSendData.SendData(modSendData.SendTarget.ToAdmins, 0, Protocol.PrepareMessageConsoleMsg(withBlock.name + " banned by the server por bannear un Administrador.", Protocol.FontTypeNames.FONTTYPE_FIGHT));
                                    TCP.CloseSocket(bannerUserIndex);
                                }

                                string argtexto = "BAN a " + UserName;
                    ...
                    {
                        Protocol.WriteConsoleMsg(bannerUserIndex, "El pj " + UserName + " no existe.", Protocol.FontTypeNames.FONTTYPE_INFO);
                    }
                }
                else
                {
                    ...
                    modSendData.SendData(modSendData.SendTarget.ToAdmins, 0, Protocol.PrepareMessageConsoleMsg("Servidor> " + withBlock.name + " ha baneado a " + Declaraciones.UserList[tUser].name + ".", Protocol.FontTypeNames.FONTTYPE_SERVER));
                    ...
                    string argtexto1 = "BAN a " + UserName;
                    ...
                    // ponemos la pena
                    int argEmptySpaces2 = 1024;
                    cantPenas = Convert.ToByte(Migration.ParseVal(ES.GetVar(Declaraciones.CharPath + UserName + ".chr", "PENAS", "Cant", EmptySpaces: ref argEmptySpaces2)));
                    ES.WriteVar(Declaraciones.CharPath + UserName + ".chr", "PENAS", "Cant", (cantPenas + 1).ToString());
                    ES.WriteVar(Declaraciones.CharPath + UserName + ".chr", "PENAS", "P" + (cantPenas + 1), withBlock.name.ToLower() + ": BAN POR " + reason.ToLower() + " " + Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay));
