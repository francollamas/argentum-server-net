...
/// </summary>
    public class SocketManager
    {
        ...
        public static Dictionary<int, SocketState> SocketStates = new Dictionary<int, SocketState>();

        // Pending operations tracking
        private static readonly HashSet<int> _pendingReceives = new HashSet<int>();
        private static readonly HashSet<int> _pendingSends = new HashSet<int>();

        // Simple synchronized message queue
        private static readonly Queue<QueuedMessage> _messageQueue = new Queue<QueuedMessage>();
        private static readonly object _messageQueueLock = new object();
        private static System.Timers.Timer _messageTimer;

        // Lock objects
        private static readonly object _socketStatesLock = new object();
        private static readonly object _pendingReceivesLock = new object();
        private static readonly object _pendingSendsLock = new object();
    ...
    /// </summary>
        public static void Initialize(int port)
        {
            ...
            _messageTimer = new System.Timers.Timer(10d); // Check queue every 10ms
    ...
    /// </summary>
        private static void ProcessMessageQueue(object sender, ElapsedEventArgs e)
        {
            ...
            while (true)
            {
                ...
                {
                    DataReceived?.Invoke(message.SocketID, message.Data);
                }
            }
        }
    ...
    /// </summary>
        private static void AcceptCallback(IAsyncResult ar)
        {
            try
            {
                ...
                if (_listener is not null && _listener.IsBound)
                {
                    _listener.BeginAccept(new AsyncCallback(AcceptCallback), _listener);
                }
            }
    ...
    /// </summary>
        private static void StartReceive(SocketState state)
        {
            ...
            try
            {
                ...
                // Begin receiving data from the client
                state.Socket.BeginReceive(state.Buffer, 0, SocketState.BufferSize, 0, new AsyncCallback(ReadCallback), state);
            }
            catch (ObjectDisposedException ex)
            {
                // Socket was closed
    ...
    /// </summary>
        public static bool SendData(int socketID, byte[] data)
        {
            ...
            lock (_socketStatesLock)
            {
                if (!SocketStates.TryGetValue(socketID, out state))
                {
                    return false;
                }
            ...
            try
            {
                ...
                state.Socket.BeginSend(data, 0, data.Length, 0, new AsyncCallback(SendCallback), state);
                return true;
            }
            catch (ObjectDisposedException ex)
            {
    ...
    /// </summary>
        private static void SendCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (state.Socket is null || state.IsClosing)
                {
                    return;
                }
    ...
    /// </summary>
        private static void CloseSocketInternal(int socketID)
        {
            ...
            if (hasPendingOperations)
            {
                ...
                var timer = new System.Timers.Timer(100d); // Check again after 100ms
