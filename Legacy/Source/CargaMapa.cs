...
namespace Legacy
{
    ...
    static class CargaMapa
    {
        ...
        public static void CargarMapa(short ind, string MAPFl)
        {
            ...
            using (var fileStream = new FileStream(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Maps", ind + ".map"), FileMode.Open))
            ...
            for (t = 0; t <= loopTo15; t++)
            {
                ...
                if (Convert.ToInt32(General.LeerNPCs.GetValue("NPC" + Declaraciones.MapData[ind, x, y].NpcIndex, "PosOrig")) == 1)
                {
            ...
            string jsonString = File.ReadAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Maps", ind + ".json"));
