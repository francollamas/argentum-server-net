...
internal static class Extra
{
    ...
    public static bool EsGM(short UserIndex)
    {
        ...
        EsGMRet = (Declaraciones.UserList[UserIndex].flags.Privilegios & (Declaraciones.PlayerType.Admin |
    ...
    public static void DoTileEvents(short UserIndex, short Map, short X, short Y)
    {
            ...
            if (InMapBounds(Map, X, Y))
            {
                ...
                if ((withBlock.TileExit.Map > 0) & (withBlock.TileExit.Map <= Declaraciones.NumMaps))
                {
                    ...
                    if (FxFlag & (TelepRadio > 0))
                    {
                        do
                        {
                            DestPos.X = (short)(withBlock.TileExit.X +
                                                Convert.ToInt16(Matematicas.RandomNumber(TelepRadio * -1, TelepRadio)));
                            DestPos.Y = (short)(withBlock.TileExit.Y +
                                                Convert.ToInt16(Matematicas.RandomNumber(TelepRadio * -1, TelepRadio)));

                            attemps = attemps + 1;

                            exitMap =
                                (Declaraciones.MapData[withBlock.TileExit.Map, DestPos.X, DestPos.Y].TileExit.Map > 0) &
                                (Declaraciones.MapData[withBlock.TileExit.Map, DestPos.X, DestPos.Y].TileExit.Map <=
                                 Declaraciones.NumMaps);
                        } while (!((attemps >= 5) | !exitMap));
                    ...
                    if (Declaraciones.MapInfo_Renamed[DestPos.Map].Restringir.ToUpper() == "NEWBIE")
                    {
                        ...
                        if (EsNewbie(UserIndex) | EsGM(UserIndex))
                        {
                            ...
                            else
                            {
                                ...
                                ClosestLegalPos(ref DestPos, ref nPos, ref argPuedeAgua, ref argPuedeTierra);
                             ...
                             "ARMADA") // ¿Es mapa de Armadas?
                    {
                        ...
                        if (esArmada(UserIndex) | EsGM(UserIndex))
                        {
                            ...
                            else
                            {
                                ...
                                ClosestLegalPos(ref DestPos, ref nPos, ref argPuedeAgua2, ref argPuedeTierra2);
                                if ((nPos.X != 0) & (nPos.Y != 0))
                                    UsUaRiOs.WarpUserChar(UserIndex, nPos.Map, nPos.X, nPos.Y, FxFlag);
                            }
                        }
                             ...
                             "CAOS") // ¿Es mapa de Caos?
                    {
                        ...
                        if (esCaos(UserIndex) | EsGM(UserIndex))
                        {
                            ...
                            else
                            {
                                ...
                                ClosestLegalPos(ref DestPos, ref nPos, ref argPuedeAgua3, ref argPuedeTierra3);
                                if ((nPos.X != 0) & (nPos.Y != 0))
                                    UsUaRiOs.WarpUserChar(UserIndex, nPos.Map, nPos.X, nPos.Y, FxFlag);
                            }
                        }
                    ...
                    else if (Declaraciones.MapInfo_Renamed[DestPos.Map].Restringir.ToUpper() == "FACCION")
                    {
                        ...
                        if (esArmada(UserIndex) | esCaos(UserIndex) | EsGM(UserIndex))
                        {
                            ...
                            else
                            {
                                ...
                                ClosestLegalPos(ref DestPos, ref nPos, ref argPuedeAgua4, ref argPuedeTierra4);
                                if ((nPos.X != 0) & (nPos.Y != 0))
                                    UsUaRiOs.WarpUserChar(UserIndex, nPos.Map, nPos.X, nPos.Y, FxFlag);
                            }
                        }
                    ...
                    else if (LegalPos(DestPos.Map, DestPos.X, DestPos.Y,
                                 UsUaRiOs.PuedeAtravesarAgua(
                                     UserIndex))) // No es un mapa de newbies, ni Armadas, ni Caos, ni faccionario.
                    {
                        UsUaRiOs.WarpUserChar(UserIndex, DestPos.Map, DestPos.X, DestPos.Y, FxFlag);
                    }
                    else
                    {
                        ...
                        ClosestLegalPos(ref DestPos, ref nPos, ref argPuedeAgua1, ref argPuedeTierra1);
                        if ((nPos.X != 0) & (nPos.Y != 0))
                            UsUaRiOs.WarpUserChar(UserIndex, nPos.Map, nPos.X, nPos.Y, FxFlag);
                    }
        ...
        catch (Exception ex)
        {
            ...
            var argdesc = "Error en DotileEvents. Error: " + ex.GetType().Name + " - Desc: " + ex.Message;
    ...
    public static bool InRangoVision(short UserIndex, short X, short Y)
    {
        ...
        if ((X > (short)(Declaraciones.UserList[UserIndex].Pos.X - Declaraciones.MinXBorder)) &
            (X < (short)(Declaraciones.UserList[UserIndex].Pos.X + Declaraciones.MinXBorder)))
            if ((Y > (short)(Declaraciones.UserList[UserIndex].Pos.Y - Declaraciones.MinYBorder)) &
                (Y < (short)(Declaraciones.UserList[UserIndex].Pos.Y + Declaraciones.MinYBorder)))
            {
    ...
    public static bool InRangoVisionNPC(short NpcIndex, ref short X, ref short Y)
    {
        ...
        if ((X > (short)(Declaraciones.Npclist[NpcIndex].Pos.X - Declaraciones.MinXBorder)) &
            (X < (short)(Declaraciones.Npclist[NpcIndex].Pos.X + Declaraciones.MinXBorder)))
            if ((Y > (short)(Declaraciones.Npclist[NpcIndex].Pos.Y - Declaraciones.MinYBorder)) &
                (Y < (short)(Declaraciones.Npclist[NpcIndex].Pos.Y + Declaraciones.MinYBorder)))
            {
    ...
    public static bool InMapBounds(short Map, short X, short Y)
    {
        ...
        if ((Map <= 0) | (Map > Declaraciones.NumMaps) | (X < Declaraciones.MinXBorder) |
            (X > Declaraciones.MaxXBorder) | (Y < Declaraciones.MinYBorder) | (Y > Declaraciones.MaxYBorder))
    ...
    public static void ClosestLegalPos(ref Declaraciones.WorldPos Pos, ref Declaraciones.WorldPos nPos,
        [Optional] [DefaultParameterValue(false)] ref bool PuedeAgua,
        [Optional] [DefaultParameterValue(true)] ref bool PuedeTierra)
    {
        // *****************************************************************
    ...
    private static void ClosestStablePos(ref Declaraciones.WorldPos Pos, ref Declaraciones.WorldPos nPos)
    {
        ...
        while (!LegalPos(Pos.Map, nPos.X, nPos.Y))
        {
            ...
            for (tY = Pos.Y - LoopC; tY <= loopTo; tY++)
            {
                var loopTo1 = Pos.X + LoopC;
                for (tX = Pos.X - LoopC; tX <= loopTo1; tX++)
                    if (LegalPos(nPos.Map, Convert.ToInt16(tX), Convert.ToInt16(tY)) & (Declaraciones
                            .MapData[nPos.Map, Convert.ToInt16(tX), Convert.ToInt16(tY)].TileExit.Map == 0))
                    {
                        nPos.X = Convert.ToInt16(tX);
                        nPos.Y = Convert.ToInt16(tY);
    ...
    public static short NameIndex(string name)
    {
        ...
        if (Migration.migr_InStrB(name, "+") != 0) name = name.Replace("+", " ").ToUpper();
    ...
    public static bool MoveToLegalPos(short Map, short X, short Y, bool PuedeAgua = false, bool PuedeTierra = true)
    {
        ...
        else
        {
            ...
            if (PuedeAgua & PuedeTierra)
                MoveToLegalPosRet = (withBlock.Blocked != 1) & ((UserIndex == 0) | IsDeadChar | IsAdminInvisible) &
                                    (withBlock.NpcIndex == 0);
            else if (PuedeTierra & !PuedeAgua)
                MoveToLegalPosRet = (withBlock.Blocked != 1) & ((UserIndex == 0) | IsDeadChar | IsAdminInvisible) &
                                    (withBlock.NpcIndex == 0) & !General.HayAgua(Map, X, Y);
            else if (PuedeAgua & !PuedeTierra)
                MoveToLegalPosRet = (withBlock.Blocked != 1) & ((UserIndex == 0) | IsDeadChar | IsAdminInvisible) &
                                    (withBlock.NpcIndex == 0) & General.HayAgua(Map, X, Y);
            else
            ...
            for (Rango = 1; Rango <= 5; Rango++)
            {
                ...
                for (tY = Y - Rango; tY <= loopTo; tY++)
                {
                    ...
                    for (tX = X - Rango; tX <= loopTo1; tX++)
                        ...
                        if ((Declaraciones.MapData[Map, Convert.ToInt16(tX), Convert.ToInt16(tY)].UserIndex == 0) &
                            (Declaraciones.MapData[Map, Convert.ToInt16(tX), Convert.ToInt16(tY)].NpcIndex == 0))
                        {
            ...
            else
            {
                ...
                if (OtherUserIndex != 0)
                {
                    ...
                    if (Declaraciones.UserList[OtherUserIndex].ComUsu.DestUsu > 0)
                    {
                        ...
                        if (Declaraciones.UserList[Declaraciones.UserList[OtherUserIndex].ComUsu.DestUsu].flags
                            .UserLogged)
                        {
                            mdlCOmercioConUsuario.FinComerciarUsu(Declaraciones.UserList[OtherUserIndex].ComUsu
                                .DestUsu);
    ...
    public static bool LegalPosNPC(short Map, short X, short Y, byte AguaValida, bool IsPet = false)
    {
...

        {
            ...
            if (AguaValida == 0)
                LegalPosNPCRet = (withBlock.Blocked != 1) &
                                 ((withBlock.UserIndex == 0) | IsDeadChar | IsAdminInvisible) &
                                 (withBlock.NpcIndex == 0) &
                                 ((withBlock.trigger != Declaraciones.eTrigger.POSINVALIDA) | IsPet) &
                                 !General.HayAgua(Map, X, Y);
            else
                LegalPosNPCRet = (withBlock.Blocked != 1) &
                                 ((withBlock.UserIndex == 0) | IsDeadChar | IsAdminInvisible) &
    ...
    public static void SendHelp(short Index)
    {
        ...
        NumHelpLines = Convert.ToInt16(Migration.ParseVal(ES.GetVar(Declaraciones.DatPath + "Help.dat", "INIT",
            "NumLines", ref argEmptySpaces)));

        var loopTo = NumHelpLines;
        for (LoopC = 1; LoopC <= loopTo; LoopC++)
        {
            var argEmptySpaces1 = 1024;
            Protocol.WriteConsoleMsg(Index,
                ES.GetVar(Declaraciones.DatPath + "Help.dat", "Help", "Line" + LoopC, ref argEmptySpaces1),
                Protocol.FontTypeNames.FONTTYPE_INFO);
        }
    }
    ...
    public static void LookatTile(short UserIndex, short Map, short X, short Y)
    {
        ...
        try
        {
            ...
            string sDesc;
            {
                ...
                if ((Math.Abs((short)(withBlock.Pos.Y - Y)) > AI.RANGO_VISION_Y) |
                    (Math.Abs((short)(withBlock.Pos.X - X)) > AI.RANGO_VISION_X)) return;
                ...
                {
                    {
                        ...
                        if (FoundSomething == 1)
                        {
                            withBlock1.TargetObj = Declaraciones
                    ...
                    if (FoundChar == 1) // ¿Encontro un Usuario?
                        if ((Declaraciones.UserList[TempCharIndex].flags.AdminInvisible == 0) |
                            ((withBlock.flags.Privilegios & Declaraciones.PlayerType.Dios) != 0))
                        {
                            {
                                ...
                                if ((Migration.migr_LenB(withBlock2.DescRM) == 0) & withBlock2.showName)
                                {
                                    ...
                                    if (EsNewbie(TempCharIndex)) Stat = " <NEWBIE>";

                                    if (withBlock2.Faccion.ArmadaReal == 1)
                                        Stat = Stat + " <Ejército Real> " + "<" +
                                               ModFacciones.TituloReal(TempCharIndex) + ">";
                                    else if (withBlock2.Faccion.FuerzasCaos == 1)
                                        Stat = Stat + " <Legión Oscura> " + "<" +
                                               ModFacciones.TituloCaos(TempCharIndex) + ">";
                                    ...
                                    else if ((withBlock2.flags.Privilegios & Declaraciones.PlayerType.ChaosCouncil) !=
                                             0)
                                    {
                                    ...
                                    else if ((withBlock2.flags.Privilegios & Declaraciones.PlayerType.User) == 0)
                                    {
                                        ...
                                        else if ((withBlock2.flags.Privilegios == (Declaraciones.PlayerType.RoleMaster |
                                                     Declaraciones.PlayerType.Consejero)) |
                                                 (withBlock2.flags.Privilegios == (Declaraciones.PlayerType.RoleMaster |
                                                     Declaraciones.PlayerType.Dios)))
                            ...
                            if (Migration.migr_LenB(Stat) > 0) Protocol.WriteConsoleMsg(UserIndex, Stat, ft);
...

                    {
                        ...
                        if (FoundChar == 2) // ¿Encontro un NPC?
                        {
                            ...
                            SupervivenciaSkill = Declaraciones.UserList[UserIndex].Stats
                                .UserSkills[(int)Declaraciones.eSkill.Supervivencia];

                            if ((withBlock3.Privilegios & (Declaraciones.PlayerType.SemiDios |
                                                           Declaraciones.PlayerType.Dios |
                            ...
                            else
                            {
                                ...
                                if (Declaraciones.Npclist[TempCharIndex].Owner > 0)
                                    sDesc = sDesc + " le pertenece a " + Declaraciones
                                        .UserList[Declaraciones.Npclist[TempCharIndex].Owner].name;
                                sDesc = sDesc + ".";

                                Protocol.WriteConsoleMsg(UserIndex, sDesc, Protocol.FontTypeNames.FONTTYPE_INFO);

                                if ((withBlock3.Privilegios &
        ...
        catch (Exception ex)
        {
            ...
            var argdesc = "Error en LookAtTile. Error " + ex.GetType().Name + " : " + ex.Message;
            General.LogError(ref argdesc);
        }
    }

    public static Declaraciones.eHeading FindDirection(ref Declaraciones.WorldPos Pos,
        ref Declaraciones.WorldPos Target)
    {
        ...
        {
            FindDirectionRet = Matematicas.RandomNumber(0, 1) != 0
                ? Declaraciones.eHeading.NORTH
                : Declaraciones.eHeading.EAST;
    ...
    public static bool ItemNoEsDeMapa(short Index, bool bIsExit)
    {
...

        {
            ...
            ItemNoEsDeMapaRet = (withBlock.OBJType != Declaraciones.eOBJType.otPuertas) &
                                ...
                                (withBlock.OBJType != Declaraciones.eOBJType.otArboles) &
                                (withBlock.OBJType != Declaraciones.eOBJType.otYacimiento) &
                                !((withBlock.OBJType == Declaraciones.eOBJType.otTeleport) & bIsExit);
        }
    ...
    public static bool MostrarCantidad(short Index)
    {
...

        {
            ...
            MostrarCantidadRet = (withBlock.OBJType != Declaraciones.eOBJType.otPuertas) &
                                 ...
                                 (withBlock.OBJType != Declaraciones.eOBJType.otArboles) &
                                 (withBlock.OBJType != Declaraciones.eOBJType.otYacimiento) &
                                 (withBlock.OBJType != Declaraciones.eOBJType.otTeleport);
        }
    ...
    public static bool EsObjetoFijo(Declaraciones.eOBJType OBJType)
    {
        ...
        EsObjetoFijoRet = (OBJType == Declaraciones.eOBJType.otForos) | (OBJType == Declaraciones.eOBJType.otCarteles) |
                          (OBJType == Declaraciones.eOBJType.otArboles) |
                          (OBJType == Declaraciones.eOBJType.otYacimiento);
