...
internal static class modSendData
{
    // **************************************************************
            ...
            switch (sndRoute)
            {
                ...
                case SendTarget.ToAdmins:
                {
                    ...
                    for (LoopC = 1; LoopC <= loopTo; LoopC++)
                        if (Declaraciones.UserList[LoopC].ConnID != -1)
                            if ((Declaraciones.UserList[LoopC].flags.Privilegios & (Declaraciones.PlayerType.Admin |
                                    Declaraciones.PlayerType.Dios | Declaraciones.PlayerType.SemiDios |
                    ...
                    for (LoopC = 1; LoopC <= loopTo3; LoopC++)
                        if (Declaraciones.UserList[LoopC].ConnID != -1)
                            if ((Declaraciones.UserList[LoopC].flags.Privilegios &
                                 Declaraciones.PlayerType.RoyalCouncil) != 0)
                                TCP.EnviarDatosASlot(Convert.ToInt16(LoopC), ref sndData);
                ...
                case SendTarget.ToConsejoCaos:
                {
                    ...
                    for (LoopC = 1; LoopC <= loopTo4; LoopC++)
                        if (Declaraciones.UserList[LoopC].ConnID != -1)
                            if ((Declaraciones.UserList[LoopC].flags.Privilegios &
                                 Declaraciones.PlayerType.ChaosCouncil) != 0)
                                TCP.EnviarDatosASlot(Convert.ToInt16(LoopC), ref sndData);
                ...
                if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveY & AreaY) != 0)
                    ...
                    if (Declaraciones.UserList[tempIndex].ConnIDValida &
                        ((Declaraciones.UserList[tempIndex].flags.Muerto == 1) |
                         ((Declaraciones.UserList[tempIndex].flags.Privilegios & (Declaraciones.PlayerType.Admin |
                             Declaraciones.PlayerType.Dios | Declaraciones.PlayerType.SemiDios |
                             Declaraciones.PlayerType.Consejero)) != 0)))
                        TCP.EnviarDatosASlot(tempIndex, ref sdData);
        }
    ...
    private static void SendToUserGuildArea(short UserIndex, string sdData)
    {
        ...
        for (LoopC = 1; LoopC <= loopTo; LoopC++)
        {
            ...
            if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveX & AreaX) != 0) // Esta en el area?
                if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveY & AreaY) != 0)
                    if (Declaraciones.UserList[tempIndex].ConnIDValida &
                        ((Declaraciones.UserList[tempIndex].GuildIndex ==
                          Declaraciones.UserList[UserIndex].GuildIndex) |
                         (((Declaraciones.UserList[tempIndex].flags.Privilegios & Declaraciones.PlayerType.Dios) != 0) &
    ...
    private static void SendToAdminsButConsejerosArea(short UserIndex, string sdData)
    {
        ...
        for (LoopC = 1; LoopC <= loopTo; LoopC++)
        {
            ...
            if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveX & AreaX) != 0) // Esta en el area?
                if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveY & AreaY) != 0)
                    if (Declaraciones.UserList[tempIndex].ConnIDValida)
                        if ((Declaraciones.UserList[tempIndex].flags.Privilegios & (Declaraciones.PlayerType.SemiDios |
    ...
    public static void SendToMap(short Map, string sdData)
    {
        ...
        for (LoopC = 1; LoopC <= loopTo; LoopC++)
        {
            ...
            if (Declaraciones.UserList[tempIndex].ConnIDValida) TCP.EnviarDatosASlot(tempIndex, ref sdData);
        }
    }
                ...
                if ((withBlock.AreasInfo.AreaReciveX & AreaX) != 0) // Esta en el area?
                    if ((withBlock.AreasInfo.AreaReciveY & AreaY) != 0)
                        if (withBlock.ConnIDValida)
                            ...
                            if (((int)(withBlock.flags.Privilegios & ~Declaraciones.PlayerType.User &
                                       ~Declaraciones.PlayerType.Consejero & ~Declaraciones.PlayerType.RoleMaster) !=
                                 0) & (withBlock.flags.Privilegios != 0))
                                TCP.EnviarDatosASlot(tempIndex, ref sdData);
            }
        }
    }
    ...
    private static void SendToUsersAndRmsAndCounselorsAreaButGMs(short UserIndex, string sdData)
    {
        ...
        for (LoopC = 1; LoopC <= loopTo; LoopC++)
        {
            ...
            if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveX & AreaX) != 0) // Esta en el area?
                if ((Declaraciones.UserList[tempIndex].AreasInfo.AreaReciveY & AreaY) != 0)
                    if (Declaraciones.UserList[tempIndex].ConnIDValida)
                        if ((Declaraciones.UserList[tempIndex].flags.Privilegios & (Declaraciones.PlayerType.User |
                                Declaraciones.PlayerType.Consejero | Declaraciones.PlayerType.RoleMaster)) != 0)
                            TCP.EnviarDatosASlot(tempIndex, ref sdData);
        }
    }
        ...
        for (LoopC = 1; LoopC <= loopTo; LoopC++)
        {
            ...
            if (Declaraciones.UserList[tempIndex].ConnIDValida)
                if (tempIndex != UserIndex)
                    ...
                    {
                        var argdatos = Protocol.PrepareMessageConsoleMsg(
                            "Escuchas el llamado de un compañero que proviene del " +
                            UsUaRiOs.GetDireccion(UserIndex, tempIndex), Font);
