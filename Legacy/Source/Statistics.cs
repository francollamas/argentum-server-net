...
namespace Legacy
{
    static class Statistics
    {
        ...
        private readonly static int[,] fragAlignmentLvlData = new int[51, 5];

        // Currency just in case.... chats are way TOO often...
        private readonly static decimal[] keyOcurrencies = new decimal[256];
        ...
        public static void UserConnected(short UserIndex)
        {
            ...
            int argEmptySpaces = 30;
            trainningInfo[UserIndex].trainningTime = Convert.ToInt32(Migration.ParseVal(ES.GetVar(Declaraciones.CharPath + Declaraciones.UserList[UserIndex].name.ToUpper() + ".chr", "RESEARCH", "TrainningTime", ref argEmptySpaces)));
        ...
        public static void UserDisconnected(short UserIndex)
        {
...

            {
                ...
                withBlock.trainningTime = Convert.ToInt32(withBlock.trainningTime + (Migration.GetTickCount() - withBlock.startTick) / 1000d);

                withBlock.startTick = Convert.ToInt32(Migration.GetTickCount());

                // Store info in char file
                ES.WriteVar(Declaraciones.CharPath + Declaraciones.UserList[UserIndex].name.ToUpper() + ".chr", "RESEARCH", "TrainningTime", withBlock.trainningTime.ToString());
            }
        }
        ...
        public static void UserLevelUp(short UserIndex)
        {
...

            {
                ref var withBlock = ref trainningInfo[UserIndex];
                // Log the data
                General.AppendLog("logs/statistics.log", Declaraciones.UserList[UserIndex].name.ToUpper() + " completó el nivel " + Declaraciones.UserList[UserIndex].Stats.ELV.ToString() + " en " + (withBlock.trainningTime + (Migration.GetTickCount() - withBlock.startTick) / 1000d).ToString() + " segundos.");
        ...
        public static void StoreFrag(short killer, short victim)
        {
            ...
            fragLvlRaceData[clase].matrix[Declaraciones.UserList[killer].Stats.ELV, raza] = fragLvlRaceData[clase].matrix[Declaraciones.UserList[killer].Stats.ELV, raza] + 1;

            fragLvlLvlData[clase].matrix[Declaraciones.UserList[killer].Stats.ELV, Declaraciones.UserList[victim].Stats.ELV] = fragLvlLvlData[clase].matrix[Declaraciones.UserList[killer].Stats.ELV, Declaraciones.UserList[victim].Stats.ELV] + 1;

            fragAlignmentLvlData[Declaraciones.UserList[killer].Stats.ELV, alignment] = fragAlignmentLvlData[Declaraciones.UserList[killer].Stats.ELV, alignment] + 1;
        }
        ...
        public static void DumpStatistics()
        {
            ...
            using (var writer = new System.IO.StreamWriter(AppDomain.CurrentDomain.BaseDirectory + "logs/frags.txt"))
            {
            ...
            using (var writer = new System.IO.StreamWriter(AppDomain.CurrentDomain.BaseDirectory + "logs/huffman.log"))
            {
