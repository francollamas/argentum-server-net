...
namespace Legacy
{
    static class modGuilds
    {
        ...
        private readonly static clsClan[] guilds = new clsClan[(MAX_GUILDS + 1)];
        ...
        public static void LoadGuildsDB()
        {
            ...
            CantClanes = ES.GetVar(GUILDINFOFILE, "INIT", "nroGuilds", EmptySpaces: ref argEmptySpaces);
            ...
            for (i = 1; i <= loopTo; i++)
            {
                ...
                tempStr = ES.GetVar(GUILDINFOFILE, "GUILD" + i, "GUILDNAME", EmptySpaces: ref argEmptySpaces1);
                int argEmptySpaces2 = 1024;
                string argS = ES.GetVar(GUILDINFOFILE, "GUILD" + i, "Alineacion", EmptySpaces: ref argEmptySpaces2);
        ...
        public static bool m_ValidarPermanencia(short UserIndex, bool SumaAntifaccion, ref bool CambioAlineacion)
        {
            ...
            if (!m_EstadoPermiteEntrar(UserIndex, GuildIndex))
            {
                ...
                {
                    General.LogClanes(Declaraciones.UserList[UserIndex].name + ", líder de " + guilds[GuildIndex].GuildName + " hizo bajar la alienación de su clan.");
                ...
                {
                    General.LogClanes(Declaraciones.UserList[UserIndex].name + " de " + guilds[GuildIndex].GuildName + " es expulsado en validar permanencia.");
                    ...
                    General.LogClanes(Declaraciones.UserList[UserIndex].name + " de " + guilds[GuildIndex].GuildName + (CambioAlineacion ? " SI " : " NO ") + "provoca cambio de alineación. MAXANT:" + CambioAlineacion);
                        ...
                        if (m_EsGuildLeader(ref MemberName, GuildIndex)) // hay que sacarlo de las facciones
                        {
                            ...
                            if (UserIndex > 0)
                            {
                                if (Declaraciones.UserList[UserIndex].Faccion.ArmadaReal != 0)
                                {
                                    ...
                                    Declaraciones.UserList[UserIndex].Faccion.Reenlistadas = Convert.ToByte(Declaraciones.UserList[UserIndex].Faccion.Reenlistadas - 1);
                                }
                                else if (Declaraciones.UserList[UserIndex].Faccion.FuerzasCaos != 0)
                                {
                                    bool argExpulsado1 = true;
                                    ModFacciones.ExpulsarFaccionCaos(UserIndex, Expulsado: ref argExpulsado1);
                                    // No cuenta como reenlistada :p.
                                    Declaraciones.UserList[UserIndex].Faccion.Reenlistadas = Convert.ToByte(Declaraciones.UserList[UserIndex].Faccion.Reenlistadas - 1);
                                }
                            }
                            else if (System.IO.File.Exists(Declaraciones.CharPath + MemberName + ".chr"))
                            {
                                ES.WriteVar(Declaraciones.CharPath + MemberName + ".chr", "FACCIONES", "EjercitoCaos", 0.ToString());
                                ES.WriteVar(Declaraciones.CharPath + MemberName + ".chr", "FACCIONES", "EjercitoReal", 0.ToString());
                                int argEmptySpaces = 1024;
                                Reenlistadas = Convert.ToInt16(ES.GetVar(Declaraciones.CharPath + MemberName + ".chr", "FACCIONES", "Reenlistadas", EmptySpaces: ref argEmptySpaces));
                                ES.WriteVar(Declaraciones.CharPath + MemberName + ".chr", "FACCIONES", "Reenlistadas", (Reenlistadas > 1 ? Reenlistadas - 1 : Reenlistadas).ToString());
                            }
                        }
            ...
            if (UserIndex > 0)
            {
                ...
                if (GI > 0)
                {
                    if (m_PuedeSalirDeClan(ref Expulsado, GI, Expulsador))
                    {
                        ...
                        General.LogClanes(Expulsado + " ha sido expulsado de " + guilds[GI].GuildName + " Expulsador = " + Expulsador);
        ...
        public static void ChangeCodexAndDesc(ref string desc, ref string[] codex, short GuildIndex)
        {
...

            {
                ...
                for (i = i; i <= loopTo1; i++)
                {
                    string argcodex = Constants.vbNullString;
        ...
        public static void ActualizarNoticias(short UserIndex, ref string datos)
        {
...

            {
                ...
                modSendData.SendData(modSendData.SendTarget.ToDiosesYclan, withBlock.GuildIndex, Protocol.PrepareMessageGuildChat(withBlock.name + " ha actualizado las noticias del clan!"));
            }
        }

        public static bool CrearNuevoClan(short FundadorIndex, ref string desc, ref string GuildName, ref string URL, ref string[] codex, ALINEACION_GUILD Alineacion, ref string refError)
        {
            ...
            if (CANTIDADDECLANES < guilds.Length - 1)
            {
...

                {
                    ...
                    withBlock.SetDesc(ref desc);
                    string argNews = "Clan creado con alineación: " + Alineacion2String(Alineacion);
        ...
        public static void SendGuildNews(short UserIndex)
        {
            ...
            string[] allies;
            {
                ...
                {
                    Protocol.WriteConsoleMsg(UserIndex, "Hoy es la votación para elegir un nuevo líder para el clan.", Protocol.FontTypeNames.FONTTYPE_GUILD);
                    Protocol.WriteConsoleMsg(UserIndex, "La elección durará 24 horas, se puede votar a cualquier miembro del clan.", Protocol.FontTypeNames.FONTTYPE_GUILD);
                    Protocol.WriteConsoleMsg(UserIndex, "Para votar escribe /VOTO NICKNAME.", Protocol.FontTypeNames.FONTTYPE_GUILD);
                    Protocol.WriteConsoleMsg(UserIndex, "Sólo se computará un voto por miembro. Tu voto no puede ser cambiado.", Protocol.FontTypeNames.FONTTYPE_GUILD);
                }
            }
        }
        ...
        public static bool m_PuedeSalirDeClan(ref string Nombre, short GuildIndex, short QuienLoEchaUI)
        {
            ...
            {
                bool localm_EsGuildLeader() { string argPJ = Declaraciones.UserList[QuienLoEchaUI].name.ToUpper(); var ret = m_EsGuildLeader(ref argPJ, GuildIndex); return ret; }

                if (!localm_EsGuildLeader())
                {
                    if ((Declaraciones.UserList[QuienLoEchaUI].name.ToUpper() ?? "") != (Nombre.ToUpper() ?? "")) // si no sale voluntariamente...
                    {
                        return m_PuedeSalirDeClanRet;
                    }
                }
        ...
        public static bool PuedeFundarUnClan(short UserIndex, ALINEACION_GUILD Alineacion, ref string refError)
        {
            ...
            if (Declaraciones.UserList[UserIndex].Stats.ELV < 25 | Declaraciones.UserList[UserIndex].Stats.UserSkills[(int)Declaraciones.eSkill.Liderazgo] < 90)
            {
            ...
            switch (Alineacion)
            {
                ...
                case ALINEACION_GUILD.ALINEACION_MASTER:
                    {
                        if ((Declaraciones.UserList[UserIndex].flags.Privilegios & (Declaraciones.PlayerType.User | Declaraciones.PlayerType.Consejero | Declaraciones.PlayerType.SemiDios)) != 0)
                        {
                ...
                case ALINEACION_GUILD.ALINEACION_NEUTRO:
                    {
                        if (Declaraciones.UserList[UserIndex].Faccion.ArmadaReal != 0 | Declaraciones.UserList[UserIndex].Faccion.FuerzasCaos != 0)
                        {
        ...
        private static bool m_EstadoPermiteEntrarChar(ref string Personaje, short GuildIndex)
        {
            ...
            if (System.IO.File.Exists(Declaraciones.CharPath + Personaje + ".chr"))
            {
                int argEmptySpaces = 1024;
                Promedio = Convert.ToInt32(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "REP", "Promedio", EmptySpaces: ref argEmptySpaces));
                switch (guilds[GuildIndex].Alineacion)
                {
                    case ALINEACION_GUILD.ALINEACION_ARMADA:
                        {
                            ...
                            {
                                int argEmptySpaces1 = 1024;
                                ELV = Convert.ToInt16(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "Stats", "ELV", EmptySpaces: ref argEmptySpaces1));
                                if (ELV >= 25)
                                {
                                    int argEmptySpaces2 = 1024;
                                    f = Convert.ToByte(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "Facciones", "EjercitoReal", EmptySpaces: ref argEmptySpaces2));
                                }
                    ...
                    case ALINEACION_GUILD.ALINEACION_NEUTRO:
                        {
                            ...
                            m_EstadoPermiteEntrarCharRet = Convert.ToByte(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "Facciones", "EjercitoReal", EmptySpaces: ref argEmptySpaces3)) == 0;
                            int argEmptySpaces4 = 1024;
                            m_EstadoPermiteEntrarCharRet = m_EstadoPermiteEntrarCharRet & Convert.ToByte(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "Facciones", "EjercitoCaos", EmptySpaces: ref argEmptySpaces4)) == 0;
                            break;
                        }
                    case ALINEACION_GUILD.ALINEACION_LEGION:
                        {
                            if (Promedio < 0)
                            {
                                ...
                                ELV = Convert.ToInt16(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "Stats", "ELV", EmptySpaces: ref argEmptySpaces5));
                                if (ELV >= 25)
                                {
                                    int argEmptySpaces6 = 1024;
                                    f = Convert.ToByte(ES.GetVar(Declaraciones.CharPath + Personaje + ".chr", "Facciones", "EjercitoCaos", EmptySpaces: ref argEmptySpaces6));
                                }
                                m_EstadoPermiteEntrarCharRet = ELV >= 25 ? f != 0 : true;
                            }
        ...
        private static bool m_EstadoPermiteEntrar(short UserIndex, short GuildIndex)
        {
            ...
            switch (guilds[GuildIndex].Alineacion)
            {
                case ALINEACION_GUILD.ALINEACION_ARMADA:
                    {
                        m_EstadoPermiteEntrarRet = !ES.criminal(UserIndex) & (Declaraciones.UserList[UserIndex].Stats.ELV >= 25 ? Declaraciones.UserList[UserIndex].Faccion.ArmadaReal != 0 : true);
                        break;
                    }

                case ALINEACION_GUILD.ALINEACION_LEGION:
                    {
                        m_EstadoPermiteEntrarRet = ES.criminal(UserIndex) & (Declaraciones.UserList[UserIndex].Stats.ELV >= 25 ? Declaraciones.UserList[UserIndex].Faccion.FuerzasCaos != 0 : true);
                        break;
                    }

                case ALINEACION_GUILD.ALINEACION_NEUTRO:
                    {
                        m_EstadoPermiteEntrarRet = Declaraciones.UserList[UserIndex].Faccion.ArmadaReal == 0 & Declaraciones.UserList[UserIndex].Faccion.FuerzasCaos == 0;
                        break;
                    }
        ...
        public static void v_RutinaElecciones()
        {
            ...
            try
            {
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Revisando elecciones", Protocol.FontTypeNames.FONTTYPE_SERVER));
                var loopTo = CANTIDADDECLANES;
                for (i = 1; i <= loopTo; i++)
                {
                    if (guilds[i] is not null)
                    {
                        ...
                        {
                            modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> " + guilds[i].GetLeader() + " es el nuevo líder de " + guilds[i].GuildName + ".", Protocol.FontTypeNames.FONTTYPE_SERVER));
                        }
                    }
                proximo:
                    ;
                ...
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Elecciones revisadas.", Protocol.FontTypeNames.FONTTYPE_SERVER));
            }
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "modGuilds.v_RutinaElecciones():" + ex.Message;
        ...
        private static short GetGuildIndexFromChar(ref string PlayerName)
        {
            ...
            string Temps;
            if (Migration.migr_InStrB(PlayerName, @"\") != 0)
            {
                PlayerName = PlayerName.Replace(@"\", Constants.vbNullString);
            }
            if (Migration.migr_InStrB(PlayerName, "/") != 0)
            {
                PlayerName = PlayerName.Replace("/", Constants.vbNullString);
            ...
            int argEmptySpaces = 1024;
            Temps = ES.GetVar(Declaraciones.CharPath + PlayerName + ".chr", "GUILD", "GUILDINDEX", EmptySpaces: ref argEmptySpaces);
        ...
        public static string m_ListaDeMiembrosOnline(short UserIndex, short GuildIndex)
        {
            ...
            if (GuildIndex > 0 & GuildIndex <= CANTIDADDECLANES)
            {
                ...
                while (i > 0)
                {
                    ...
                    if (i != UserIndex & ((Declaraciones.UserList[i].flags.Privilegios & (Declaraciones.PlayerType.User | Declaraciones.PlayerType.Consejero | Declaraciones.PlayerType.SemiDios)) != 0 | (Declaraciones.UserList[UserIndex].flags.Privilegios & (Declaraciones.PlayerType.Dios | Declaraciones.PlayerType.Admin)) != 0))
        ...
        public static void SendGuildDetails(short UserIndex, ref string GuildName)
        {
...

            {
                ...
                Protocol.WriteGuildDetails(UserIndex, GuildName, withBlock.Fundador, withBlock.GetFechaFundacion(), withBlock.GetLeader(), withBlock.GetURL(), withBlock.CantidadDeMiembros, withBlock.EleccionesAbiertas(), Alineacion2String(withBlock.Alineacion), withBlock.CantidadEnemys, withBlock.CantidadAllies, withBlock.PuntosAntifaccion + "/" + MAXANTIFACCION.ToString(), codex, withBlock.GetDesc());
            }
        }
        ...
        public static short GMEscuchaClan(short UserIndex, string GuildName)
        {
            ...
            if (Migration.migr_LenB(GuildName) == 0 & Declaraciones.UserList[UserIndex].EscucheClan != 0)
            {
                ...
                Protocol.WriteConsoleMsg(UserIndex, "Dejas de escuchar a : " + guilds[Declaraciones.UserList[UserIndex].EscucheClan].GuildName, Protocol.FontTypeNames.FONTTYPE_GUILD);
        ...
        public static short r_DeclararGuerra(short UserIndex, ref string GuildGuerra, ref string refError)
        {
            ...
            {
                string argdesc = "ModGuilds.r_DeclararGuerra: " + GI + " declara a " + GuildGuerra;
                General.LogError(ref argdesc);
        ...
        public static short r_AceptarPropuestaDePaz(short UserIndex, ref string GuildPaz, ref string refError)
        {
            ...
            if (GIG < 1 | GIG > CANTIDADDECLANES)
            {
                string argdesc = "ModGuilds.r_AceptarPropuestaDePaz: " + GI + " acepta de " + GuildPaz;
                General.LogError(ref argdesc);
                refError = "Inconsistencia en el sistema de clanes. Avise a un administrador (GIG fuera de rango).";
                return r_AceptarPropuestaDePazRet;
            }
        ...
        public static short r_RechazarPropuestaDeAlianza(short UserIndex, ref string GuildPro, ref string refError)
        {
            ...
            if (GIG < 1 | GIG > CANTIDADDECLANES)
            {
                string argdesc = "ModGuilds.r_RechazarPropuestaDeAlianza: " + GI + " acepta de " + GuildPro;
                General.LogError(ref argdesc);
                refError = "Inconsistencia en el sistema de clanes. Avise a un administrador (GIG fuera de rango).";
                return r_RechazarPropuestaDeAlianzaRet;
            }
            ...
            string argNews = guilds[GI].GuildName + " ha rechazado nuestra propuesta de alianza. " + guilds[GIG].GetGuildNews();
        ...
        public static short r_RechazarPropuestaDePaz(short UserIndex, ref string GuildPro, ref string refError)
        {
            ...
            if (GIG < 1 | GIG > CANTIDADDECLANES)
            {
                string argdesc = "ModGuilds.r_RechazarPropuestaDePaz: " + GI + " acepta de " + GuildPro;
                General.LogError(ref argdesc);
                refError = "Inconsistencia en el sistema de clanes. Avise a un administrador (GIG fuera de rango).";
                return r_RechazarPropuestaDePazRet;
            }
            ...
            guilds[GI].AnularPropuestas(GIG);
            // avisamos al otro clan
            string argNews = guilds[GI].GuildName + " ha rechazado nuestra propuesta de paz. " + guilds[GIG].GetGuildNews();
            guilds[GIG].SetGuildNews(ref argNews);
        ...
        public static short r_AceptarPropuestaDeAlianza(short UserIndex, ref string GuildAllie, ref string refError)
        {
            ...
            if (GIG < 1 | GIG > CANTIDADDECLANES)
            {
                string argdesc = "ModGuilds.r_AceptarPropuestaDeAlianza: " + GI + " acepta de " + GuildAllie;
                General.LogError(ref argdesc);
                refError = "Inconsistencia en el sistema de clanes. Avise a un administrador (GIG fuera de rango).";
                return r_AceptarPropuestaDeAlianzaRet;
            }
        ...
        public static bool r_ClanGeneraPropuesta(short UserIndex, ref string OtroClan, RELACIONES_GUILD Tipo, ref string Detalle, ref string refError)
        {
        ...
        public static string r_VerPropuesta(short UserIndex, ref string OtroGuild, RELACIONES_GUILD Tipo, ref string refError)
        {
        ...
        public static void a_RechazarAspiranteChar(ref string Aspirante, short guild, ref string Detalles)
        {
            ...
            if (Migration.migr_InStrB(Aspirante, @"\") != 0)
            {
        ...
        public static string a_ObtenerRechazoDeChar(ref string Aspirante)
        {
            ...
            int argEmptySpaces = 1024;
            a_ObtenerRechazoDeCharRet = ES.GetVar(Declaraciones.CharPath + Aspirante + ".chr", "GUILD", "MotivoRechazo", EmptySpaces: ref argEmptySpaces);
        ...
        public static string a_DetallesAspirante(short UserIndex, ref string Nombre)
        {
            ...
            if (NroAspirante > 0)
            {
                a_DetallesAspiranteRet = guilds[GI].DetallesSolicitudAspirante(NroAspirante);
            }
        ...
        public static void SendDetallesPersonaje(short UserIndex, string Personaje)
        {
            ...
            try
            {
                ...
                if (NroAsp == 0)
                {
                    ...
                    if (i > list.Length - 1)
                    {
                        Protocol.WriteConsoleMsg(UserIndex, "El personaje no es ni aspirante ni miembro del clan.", Protocol.FontTypeNames.FONTTYPE_INFO);
                        return;
                    }
                }
...

                {
                    ...
                    if (Miembro.Length > 400)
                    {
                        Miembro = ".." + Miembro.Substring(Math.Max(0, Miembro.Length - 400));
                    }

                    Protocol.WriteCharacterInfo(UserIndex, Personaje, (Declaraciones.eRaza)Convert.ToInt16(withBlock.GetValue("INIT", "Raza")), (Declaraciones.eClass)Convert.ToInt16(withBlock.GetValue("INIT", "Clase")), (Declaraciones.eGenero)Convert.ToInt16(withBlock.GetValue("INIT", "Genero")), Convert.ToByte(withBlock.GetValue("STATS", "ELV")), Convert.ToInt32(withBlock.GetValue("STATS", "GLD")), Convert.ToInt32(withBlock.GetValue("STATS", "Banco")), Convert.ToInt32(withBlock.GetValue("REP", "Promedio")), withBlock.GetValue("GUILD", "Pedidos"), GuildName, Miembro, Convert.ToBoolean(withBlock.GetValue("FACCIONES", "EjercitoReal")), Convert.ToBoolean(withBlock.GetValue("FACCIONES", "EjercitoCaos")), Convert.ToInt32(withBlock.GetValue("FACCIONES", "CiudMatados")), Convert.ToInt32(withBlock.GetValue("FACCIONES", "CrimMatados")));
                }
            ...
            catch (Exception ex)
            {
                ...
                if (!System.IO.File.Exists(Declaraciones.CharPath + Personaje + ".chr"))
                {
                    string argdesc = "El usuario " + Declaraciones.UserList[UserIndex].name + " (" + UserIndex + " ) ha pedido los detalles del personaje " + Personaje + " que no se encuentra.";
                    General.LogError(ref argdesc);
                }
                else
                {
                    string argdesc1 = "[" + ex.GetType().Name + "] " + ex.Message + " En la rutina SendDetallesPersonaje, por el usuario " + Declaraciones.UserList[UserIndex].name + " (" + UserIndex + " ), pidiendo información sobre el personaje " + Personaje;
        ...
        public static bool a_NuevoAspirante(short UserIndex, ref string clan, ref string Solicitud, ref string refError)
        {
            ...
            int argEmptySpaces = 1024;
            ViejoSolicitado = ES.GetVar(Declaraciones.CharPath + Declaraciones.UserList[UserIndex].name + ".chr", "GUILD", "ASPIRANTEA", EmptySpaces: ref argEmptySpaces);
            ...
            if (Migration.migr_LenB(ViejoSolicitado) != 0)
            {
                ...
                {
                    ViejoNroAspirante = guilds[ViejoGuildINdex].NumeroDeAspirante(ref Declaraciones.UserList[UserIndex].name);
                    if (ViejoNroAspirante > 0)
                    {
                        guilds[ViejoGuildINdex].RetirarAspirante(ref Declaraciones.UserList[UserIndex].name, ref ViejoNroAspirante);
                    }
                }
                else
                {
