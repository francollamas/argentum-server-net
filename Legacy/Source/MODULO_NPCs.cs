    ...
    static class NPCs
    {
        ...
        public static void QuitarMascota(short UserIndex, short NpcIndex)
        {
            ...
            for (i = 1; i <= Declaraciones.MAXMASCOTAS; i++)
            {
                if (Declaraciones.UserList[UserIndex].MascotasIndex[i] == NpcIndex)
                {
                    ...
                    Declaraciones.UserList[UserIndex].NroMascotas = Convert.ToInt16(Declaraciones.UserList[UserIndex].NroMascotas - 1);
                    break;
                }
            }
        }
        ...
        public static void MuereNpc(short NpcIndex, short UserIndex)
        {
                ...
                {
                    {
                        ...
                        {
                            modSendData.SendData(modSendData.SendTarget.ToPCArea, UserIndex, Protocol.PrepareMessagePlayWave(Convert.ToByte(MiNPC.flags.Snd3), Convert.ToByte(MiNPC.Pos.X), Convert.ToByte(MiNPC.Pos.Y)));
                        }
                        ...
                        if (MiNPC.flags.ExpCount > 0)
                        {
                            ...
                            {
                                mdParty.ObtenerExito(UserIndex, MiNPC.flags.ExpCount, ref MiNPC.Pos.Map, ref MiNPC.Pos.X, ref MiNPC.Pos.Y);
                            }
                            else
                            {
                                ...
                                Protocol.WriteConsoleMsg(UserIndex, "Has ganado " + MiNPC.flags.ExpCount + " puntos de experiencia.", Protocol.FontTypeNames.FONTTYPE_FIGHT);
                            }
                            MiNPC.flags.ExpCount = 0;
                        }

                        // [/KEVIN]
                        Protocol.WriteConsoleMsg(UserIndex, "¡Has matado a la criatura!", Protocol.FontTypeNames.FONTTYPE_FIGHT);
                        ...
                        {
                            withBlock.Reputacion.NobleRep = Convert.ToInt32(withBlock.Reputacion.NobleRep + Declaraciones.vlASESINO / 2d);
                        ...
                        else if (!EraCriminal & ES.criminal(UserIndex))
                        {
                            UsUaRiOs.RefreshCharStatus(UserIndex);
                        }
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en MuereNpc - Error: " + ex.GetType().Name + " - Desc: " + ex.Message;
        ...
        public static void QuitarNPC(short NpcIndex)
        {
            ...
            try
            {

                {
                    ...
                    if (Extra.InMapBounds(withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y))
                    {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en QuitarNPC";
                General.LogError(ref argdesc);
            }
        }
        ...
        public static void QuitarPet(short UserIndex, short NpcIndex)
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en QuitarPet. Error: " + ex.GetType().Name + " Desc: " + ex.Message + " NpcIndex: " + NpcIndex + " UserIndex: " + UserIndex + " PetIndex: " + PetIndex;
                General.LogError(ref argdesc);
            }
        }

        private static bool TestSpawnTrigger(ref Declaraciones.WorldPos Pos, [Optional, DefaultParameterValue(false)] ref bool PuedeAgua)
        {
            ...
            {
                TestSpawnTriggerRet = (int)Declaraciones.MapData[Pos.Map, Pos.X, Pos.Y].trigger != 3 & (int)Declaraciones.MapData[Pos.Map, Pos.X, Pos.Y].trigger != 2 & (int)Declaraciones.MapData[Pos.Map, Pos.X, Pos.Y].trigger != 1;
            }
        ...
        public static void CrearNPC(ref short NroNPC, ref short mapa, ref Declaraciones.WorldPos OrigPos)
        {
            ...
            else
            {
                ...
                {
                    Pos.X = Convert.ToInt16(Matematicas.RandomNumber(Declaraciones.MinXBorder, Declaraciones.MaxXBorder)); // Obtenemos posicion al azar en x
                    Pos.Y = Convert.ToInt16(Matematicas.RandomNumber(Declaraciones.MinYBorder, Declaraciones.MaxYBorder)); // Obtenemos posicion al azar en y

                    Extra.ClosestLegalPos(ref Pos, ref newpos, ref PuedeAgua, ref PuedeTierra); // Nos devuelve la posicion valida mas cercana
                    ...
                    else
                    {
                        ...
                        Extra.ClosestLegalPos(ref Pos, ref newpos, ref PuedeAgua, PuedeTierra: ref argPuedeTierra);
                        if (newpos.X != 0 & newpos.Y != 0)
                        {
                            altpos.X = newpos.X;
                    ...
                    if (Extra.LegalPosNPC(newpos.Map, newpos.X, newpos.Y, Convert.ToByte(PuedeAgua)) & !TCP.HayPCarea(ref newpos) & TestSpawnTrigger(ref newpos, ref PuedeAgua))
                    {
                    ...
                    if (Iteraciones > Declaraciones.MAXSPAWNATTEMPS)
                    {
                        ...
                        else
                        {
                            ...
                            Extra.ClosestLegalPos(ref altpos, ref newpos, PuedeAgua: ref argPuedeAgua, PuedeTierra: ref argPuedeTierra1);
                            if (newpos.X != 0 & newpos.Y != 0)
                            {
                                Declaraciones.Npclist[nIndex].Pos.Map = newpos.Map;
                                Declaraciones.Npclist[nIndex].Pos.X = newpos.X;
                            ...
                            {
                                QuitarNPC(nIndex);
                                string argdesc = Declaraciones.MAXSPAWNATTEMPS + " iteraciones en CrearNpc Mapa:" + mapa + " NroNpc:" + NroNPC;
                                General.LogError(ref argdesc);
                                return;
                            }
                        }
        ...
        public static void MakeNPCChar(bool toMap, ref short sndIndex, ref short NpcIndex, short Map, short X, short Y)
        {
            ...
            if (!toMap)
            {
                Protocol.WriteCharacterCreate(sndIndex, Declaraciones.Npclist[NpcIndex].Char_Renamed.body, Declaraciones.Npclist[NpcIndex].Char_Renamed.Head, Declaraciones.Npclist[NpcIndex].Char_Renamed.heading, Declaraciones.Npclist[NpcIndex].Char_Renamed.CharIndex, Convert.ToByte(X), Convert.ToByte(Y), 0, 0, 0, 0, 0, Constants.vbNullString, 0, 0);
            ...
            {
                {
                    ...
                    modSendData.SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, Protocol.PrepareMessageCharacterChange(body, Head, heading, withBlock.CharIndex, 0, 0, 0, 0, 0));
                }
            }
        }
        ...
        private static void EraseNPCChar(short NpcIndex)
        {
            ...
            modSendData.SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, Protocol.PrepareMessageCharacterRemove(Declaraciones.Npclist[NpcIndex].Char_Renamed.CharIndex));
        ...
        public static void MoveNPCChar(short NpcIndex, byte nHeading)
        {
            ...
            try
            {
...

                {
                    ...
                    if (Extra.LegalPosNPC(withBlock.Pos.Map, nPos.X, nPos.Y, withBlock.flags.AguaValida, withBlock.MaestroUser != 0))
                    {
                        ...
                        if (UserIndex > 0)
                        {
                            ...
                            if (General.HayAgua(withBlock.Pos.Map, nPos.X, nPos.Y) & !General.HayAgua(withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y))
                                return;
...

                            {
                                ...
                                Declaraciones.MapData[withBlock1.Pos.Map, withBlock1.Pos.X, withBlock1.Pos.Y].UserIndex = UserIndex;

                                // Avisamos a los usuarios del area, y al propio usuario lo forzamos a moverse
                                modSendData.SendData(modSendData.SendTarget.ToPCAreaButIndex, UserIndex, Protocol.PrepareMessageCharacterMove(Declaraciones.UserList[UserIndex].Char_Renamed.CharIndex, Convert.ToByte(withBlock1.Pos.X), Convert.ToByte(withBlock1.Pos.Y)));
                                Protocol.WriteForceCharMove(UserIndex, UsUaRiOs.InvertHeading((Declaraciones.eHeading)nHeading));
                            }
                        }

                        modSendData.SendData(modSendData.SendTarget.ToNPCArea, NpcIndex, Protocol.PrepareMessageCharacterMove(withBlock.Char_Renamed.CharIndex, Convert.ToByte(nPos.X), Convert.ToByte(nPos.Y)));
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en move npc " + NpcIndex;
                General.LogError(ref argdesc);
            }
        }
        ...
        public static short NextOpenNPC()
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en NextOpenNPC";
                General.LogError(ref argdesc);
            }
        ...
        public static void NpcEnvenenarUser(short UserIndex)
        {
            ...
            if (N < 30)
            {
                ...
                Protocol.WriteConsoleMsg(UserIndex, "¡¡La criatura te ha envenenado!!", Protocol.FontTypeNames.FONTTYPE_FIGHT);
            }
        }
        ...
        public static short SpawnNpc(short NpcIndex, ref Declaraciones.WorldPos Pos, bool FX, bool Respawn)
        {
            ...
            Extra.ClosestLegalPos(ref Pos, ref newpos, ref PuedeAgua, ref PuedeTierra); // Nos devuelve la posicion valida mas cercana
            bool argPuedeTierra = true;
            Extra.ClosestLegalPos(ref Pos, ref altpos, ref PuedeAgua, PuedeTierra: ref argPuedeTierra);
            // Si X e Y son iguales a 0 significa que no se encontro posicion valida
            ...
            if (FX)
            {
                modSendData.SendData(modSendData.SendTarget.ToNPCArea, nIndex, Protocol.PrepareMessagePlayWave(Declaraciones.SND_WARP, Convert.ToByte(X), Convert.ToByte(Y)));
                modSendData.SendData(modSendData.SendTarget.ToNPCArea, nIndex, Protocol.PrepareMessageCreateFX(Declaraciones.Npclist[nIndex].Char_Renamed.CharIndex, (short)Declaraciones.FXIDs.FXWARP, 0));
            }
        ...
        private static void NPCTirarOro(ref Declaraciones.npc MiNPC)
        {
            ...
            if (MiNPC.GiveGLD > 0)
            {
                ...
                while (MiAux > Declaraciones.MAX_INVENTORY_OBJS)
                {
                    ...
                    InvNpc.TirarItemAlPiso(ref MiNPC.Pos, ref MiObj, NotPirata: ref argNotPirata);
                ...
                if (MiAux > 0)
                {
                    ...
                    InvNpc.TirarItemAlPiso(ref MiNPC.Pos, ref MiObj, NotPirata: ref argNotPirata1);
                }
            }
        }
        ...
        public static short OpenNPC(short NpcNumber, bool Respawn = true)
        {
...

            {
                ...
                withBlock.Movement = (AI.TipoAI)Math.Round(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "Movement")));
                withBlock.flags.OldMovement = withBlock.Movement;

                withBlock.flags.AguaValida = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "AguaValida")));
                withBlock.flags.TierraInvalida = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "TierraInValida")));
                withBlock.flags.Faccion = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "Faccion")));
                withBlock.flags.AtacaDoble = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "AtacaDoble")));

                withBlock.NPCtype = (Declaraciones.eNPCType)Math.Round(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "NpcType")));
                ...
                withBlock.Char_Renamed.heading = (Declaraciones.eHeading)Math.Round(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "Heading")));
                ...
                withBlock.PoderAtaque = (int)Math.Round(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "PoderAtaque")));
                withBlock.PoderEvasion = (int)Math.Round(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "PoderEvasion")));
...

                {
                    ...
                    withBlock1.Alineacion = Convert.ToInt16(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "Alineacion")));
                }

                withBlock.Invent.NroItems = Convert.ToInt16(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "NROITEMS")));
                ...
                withBlock.flags.LanzaSpells = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "LanzaSpells")));
                ...
                for (LoopC = 1; LoopC <= loopTo1; LoopC++)
                    withBlock.Spells[LoopC] = Convert.ToInt16(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "Sp" + LoopC)));

                if (withBlock.NPCtype == Declaraciones.eNPCType.Entrenador)
                {
                    withBlock.NroCriaturas = Convert.ToInt16(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "NroCriaturas")));
                    ...
                    {
                        withBlock.Criaturas[LoopC].NpcIndex = Convert.ToInt16(Leer.GetValue("NPC" + NpcNumber, "CI" + LoopC));
...

                {
                    ...
                    if (Respawn)
                    {
                        withBlock2.Respawn = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "ReSpawn")));
                    }
                    else
                    {
                    ...
                    withBlock2.RespawnOrigPos = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "OrigPos")));
                    withBlock2.AfectaParalisis = Convert.ToByte(Migration.ParseVal(Leer.GetValue("NPC" + NpcNumber, "AfectaParalisis")));
