...
internal static class mdlCOmercioConUsuario
{
    ...
    public static void IniciarComercioConUsuario(short Origen, short Destino)
    {
        ...
        catch (Exception ex)
        {
            ...
            var argdesc = "Error en IniciarComercioConUsuario: " + ex.Message;
    ...
    public static void FinComerciarUsu(short UserIndex)
    {
        ...
        {
            ref var withBlock = ref Declaraciones.UserList[UserIndex];
            if (withBlock.ComUsu.DestUsu > 0) Protocol.WriteUserCommerceEnd(UserIndex);
    ...
    public static void AceptarComercioUsu(short UserIndex)
    {
        ...
        if (!Declaraciones.UserList[OtroUserIndex].ComUsu.Acepto) return;
            ...
            // Items del 1er usuario
            {
                ...
                if (OfferSlot == GOLD_OFFER_SLOT)
                {
                    ...
                    if (withBlock.ComUsu.GoldAmount > MAX_ORO_LOGUEABLE)
                        General.LogDesarrollo(withBlock.name + " soltó oro en comercio seguro con " +
                                              Declaraciones.UserList[OtroUserIndex].name + ". Cantidad: " +
                                              withBlock.ComUsu.GoldAmount);
                    // Update Usuario
                ...
                else if (withBlock.ComUsu.Objeto[OfferSlot] > 0)
                {
                    ...
                    if (!InvUsuario.MeterItemEnInventario(OtroUserIndex, ref TradingObj))
                    {
                        ...
                        InvNpc.TirarItemAlPiso(ref Declaraciones.UserList[OtroUserIndex].Pos, ref TradingObj,
                            ref argNotPirata);
                    }
            ...
            // Items del 2do usuario
            {
                ...
                if (OfferSlot == GOLD_OFFER_SLOT)
                {
                    ...
                    if (withBlock1.ComUsu.GoldAmount > MAX_ORO_LOGUEABLE)
                        General.LogDesarrollo(withBlock1.name + " soltó oro en comercio seguro con " +
                                              Declaraciones.UserList[UserIndex].name + ". Cantidad: " +
                                              withBlock1.ComUsu.GoldAmount);
                    // Update Usuario
                    Protocol.WriteUpdateUserStats(OtroUserIndex);
                    // y se la doy al otro
                    Declaraciones.UserList[UserIndex].Stats.GLD =
                        Declaraciones.UserList[UserIndex].Stats.GLD + withBlock1.ComUsu.GoldAmount;
                    if (withBlock1.ComUsu.GoldAmount > MAX_ORO_LOGUEABLE)
                        General.LogDesarrollo(Declaraciones.UserList[UserIndex].name +
                                              " recibió oro en comercio seguro con " + withBlock1.name +
                                              ". Cantidad: " + withBlock1.ComUsu.GoldAmount);
                    // Update Otro Usuario
                    Protocol.WriteUpdateUserStats(UserIndex);
                }
                ...
                else if (withBlock1.ComUsu.Objeto[OfferSlot] > 0)
                {
                    ...
                    if (!InvUsuario.MeterItemEnInventario(UserIndex, ref TradingObj))
                    {
                        ...
                        InvNpc.TirarItemAlPiso(ref Declaraciones.UserList[UserIndex].Pos, ref TradingObj,
                            ref argNotPirata1);
                    }
    ...
    public static bool PuedeSeguirComerciando(short UserIndex)
    {
...

        {
            ...
            if (!ComercioInvalido)
                // Estan logueados?
                if (!Declaraciones.UserList[OtroUserIndex].flags.UserLogged | !withBlock.flags.UserLogged)
                    ComercioInvalido = true;
