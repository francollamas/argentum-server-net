Option Strict On
Option Explicit On
Friend Class SoundMapInfo
    Private Structure p_tSoundMapInfo
        Dim Cantidad As Short
        Dim SoundIndex() As Short
        Dim flags() As Integer
        Dim Probabilidad() As Single
    End Structure

    Private Enum p_eSoundFlags
        ninguna = 0
        Lluvia = 1
    End Enum

    Private p_Mapas() As p_tSoundMapInfo


    'sonidos conocidos, pasados a enum para intelisense
    Public Enum e_SoundIndex
        MUERTE_HOMBRE = 11
        MUERTE_MUJER = 74
        FLECHA_IMPACTO = 65
        CONVERSION_BARCO = 55
        MORFAR_MANZANA = 82
        SOUND_COMIDA = 7
    End Enum

    'UPGRADE_NOTE: Class_Initialize se actualizó a Class_Initialize_Renamed. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="A9E4979A-37FA-4718-9994-97DD76ED70A7"'
    Private Sub Class_Initialize_Renamed()
        'armar el array
        'UPGRADE_WARNING: El límite inferior de la matriz p_Mapas ha cambiado de 1 a 0. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="0F1C9BE1-AF9D-476E-83B1-17D43BECFF20"'
        ReDim p_Mapas(NumMaps)
        Call LoadSoundMapInfo()
    End Sub

    Public Sub New()
        MyBase.New()
        Class_Initialize_Renamed()
    End Sub

    Public Sub LoadSoundMapInfo()
        Dim i As Integer
        Dim j As Short
        Dim Temps As String
        Dim MAPFILE As String

        MAPFILE = AppDomain.CurrentDomain.BaseDirectory & MapPath & "MAPA"

        For i = 1 To p_Mapas.Length - 1
            Temps = GetVar(MAPFILE & i & ".dat", "SONIDOS", "Cantidad")

            If IsNumeric(Temps) Then
                p_Mapas(i).Cantidad = Convert.ToInt16(ParseVal(Temps))

                ReDim p_Mapas(i).flags(p_Mapas(i).Cantidad)
                ReDim p_Mapas(i).Probabilidad(p_Mapas(i).Cantidad)
                ReDim p_Mapas(i).SoundIndex(p_Mapas(i).Cantidad)

                For j = 1 To p_Mapas(i).Cantidad
                    p_Mapas(i).flags(j) = Convert.ToInt32(ParseVal(GetVar(MAPFILE & i & ".dat", "SONIDO" & j, "Flags")))
                    p_Mapas(i).Probabilidad(j) = Convert.ToSingle(ParseVal(GetVar(MAPFILE & i & ".dat", "SONIDO" & j, "Probabilidad")))
                    p_Mapas(i).SoundIndex(j) = Convert.ToInt16(ParseVal(GetVar(MAPFILE & i & ".dat", "SONIDO" & j, "Sonido")))
                Next j
            Else
                p_Mapas(i).Cantidad = 0
            End If
        Next i
    End Sub

    Public Sub ReproducirSonidosDeMapas()
        Dim i As Integer
        Dim SonidoMapa As Byte
        Dim posX As Byte
        Dim posY As Byte

        posX = Convert.ToByte(RandomNumber(XMinMapSize, XMaxMapSize))
        posY = Convert.ToByte(RandomNumber(YMinMapSize, YMaxMapSize))

        For i = 1 To p_Mapas.Length - 1
            If p_Mapas(i).Cantidad > 0 Then
                SonidoMapa = Convert.ToByte(RandomNumber(1, p_Mapas(i).Cantidad))
                If RandomNumber(1, 100) <= p_Mapas(i).Probabilidad(SonidoMapa) Then
                    If Lloviendo Then
                        If (p_Mapas(i).flags(SonidoMapa) Xor p_eSoundFlags.Lluvia) <> 0 Then
                            Call _
                                SendData(SendTarget.toMap, Convert.ToInt16(i),
                                         PrepareMessagePlayWave(Convert.ToByte(p_Mapas(i).SoundIndex(SonidoMapa)), Convert.ToByte(posX), Convert.ToByte(posY)))
                        End If
                    Else
                        If (p_Mapas(i).flags(SonidoMapa) Xor p_eSoundFlags.ninguna) <> 0 Then
                            Call _
                                SendData(SendTarget.toMap, Convert.ToInt16(i),
                                         PrepareMessagePlayWave(Convert.ToByte(p_Mapas(i).SoundIndex(SonidoMapa)), Convert.ToByte(posX), Convert.ToByte(posY)))
                        End If
                    End If
                End If
            End If
        Next i
    End Sub

    Public Sub ReproducirSonido(Destino As SendTarget, index As Short, SoundIndex As Short)
        Call SendData(Destino, index, PrepareMessagePlayWave(Convert.ToByte(SoundIndex), Convert.ToByte(UserList(index).Pos.X), Convert.ToByte(UserList(index).Pos.Y)))
    End Sub
End Class
