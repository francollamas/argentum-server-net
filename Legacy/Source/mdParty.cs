...
namespace Legacy
{
    static class mdParty
    {
        ...
        public static bool PuedeCrearParty(short UserIndex)
        {
            ...
            if (Convert.ToInt16(Declaraciones.UserList[UserIndex].Stats.UserAtributos[(int)Declaraciones.eAtributos.Carisma]) * Declaraciones.UserList[UserIndex].Stats.UserSkills[(int)Declaraciones.eSkill.Liderazgo] < 100)
            {
                Protocol.WriteConsoleMsg(UserIndex, "Tu carisma y liderazgo no son suficientes para liderar una party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
        ...
        public static void CrearParty(short UserIndex)
        {
...

            {
                ...
                if (withBlock.PartyIndex == 0)
                {
                    if (withBlock.flags.Muerto == 0)
                    {
                        if (withBlock.Stats.UserSkills[(int)Declaraciones.eSkill.Liderazgo] >= 5)
                        {
                            ...
                            if (tInt == -1)
                            {
                                Protocol.WriteConsoleMsg(UserIndex, "Por el momento no se pueden crear más parties.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                                return;
                            }
                            else
                            {
                                ...
                                {
                                    Protocol.WriteConsoleMsg(UserIndex, "La party está llena, no puedes entrar.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                                ...
                                {
                                    Protocol.WriteConsoleMsg(UserIndex, "¡Has formado una party!", Protocol.FontTypeNames.FONTTYPE_PARTY);
                                    withBlock.PartyIndex = tInt;
                                    withBlock.PartySolicitud = 0;
                                    if (!Declaraciones.Parties[tInt].HacerLeader(UserIndex))
                                    {
                                        Protocol.WriteConsoleMsg(UserIndex, "No puedes hacerte líder.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                                    }
                                    else
                                    {
                                        Protocol.WriteConsoleMsg(UserIndex, "¡Te has convertido en líder de la party!", Protocol.FontTypeNames.FONTTYPE_PARTY);
                                    }
                                }
                            }
                        }
                        ...
                        {
                            Protocol.WriteConsoleMsg(UserIndex, "No tienes suficientes puntos de liderazgo para liderar una party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                        }
                    }
                    else
                    {
                ...
                {
                    Protocol.WriteConsoleMsg(UserIndex, "Ya perteneces a una party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                }
            }
        }
        ...
        public static void SolicitarIngresoAParty(short UserIndex)
        {
...

            {
                ...
                if (withBlock.PartyIndex > 0)
                {
                    ...
                    Protocol.WriteConsoleMsg(UserIndex, "Ya perteneces a una party, escribe /SALIRPARTY para abandonarla", Protocol.FontTypeNames.FONTTYPE_PARTY);
                    withBlock.PartySolicitud = 0;
                    return;
                }
                ...
                if (tInt > 0)
                {
                    if (Declaraciones.UserList[tInt].PartyIndex > 0)
                    {
                        ...
                        Protocol.WriteConsoleMsg(UserIndex, "El fundador decidirá si te acepta en la party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                    }
                    else
                    {
                        Protocol.WriteConsoleMsg(UserIndex, Declaraciones.UserList[tInt].name + " no es fundador de ninguna party.", Protocol.FontTypeNames.FONTTYPE_INFO);
                        withBlock.PartySolicitud = 0;
                        return;
                    }
                }
                ...
                {
                    Protocol.WriteConsoleMsg(UserIndex, "Para ingresar a una party debes hacer click sobre el fundador y luego escribir /PARTY", Protocol.FontTypeNames.FONTTYPE_PARTY);
                    withBlock.PartySolicitud = 0;
                }
            }
        }
        ...
        public static void SalirDeParty(short UserIndex)
        {
            ...
            {
                Protocol.WriteConsoleMsg(UserIndex, "No eres miembro de ninguna party.", Protocol.FontTypeNames.FONTTYPE_INFO);
            }
        }
        ...
        public static void ExpulsarDeParty(short leader, short OldMember)
        {
            ...
            {
                Protocol.WriteConsoleMsg(leader, Declaraciones.UserList[OldMember].name.ToLower() + " no pertenece a tu party.", Protocol.FontTypeNames.FONTTYPE_INFO);
            }
        }
        ...
        public static void AprobarIngresoAParty(short leader, short NewMember)
        {
...

            {
                ...
                if (withBlock.PartySolicitud == PI)
                {
                    if (!(withBlock.flags.Muerto == 1))
                    {
                        if (withBlock.PartyIndex == 0)
                        {
                            if (Declaraciones.Parties[PI].PuedeEntrar(NewMember, ref razon))
                            {
                                ...
                                {
                                    Declaraciones.Parties[PI].MandarMensajeAConsola(Declaraciones.UserList[leader].name + " ha aceptado a " + withBlock.name + " en la party.", "Servidor");
                                ...
                                else
                                {
                                    ...
                                    modSendData.SendData(modSendData.SendTarget.ToAdmins, leader, Protocol.PrepareMessageConsoleMsg(" Servidor> CATÁSTROFE EN PARTIES, NUEVOMIEMBRO DIO FALSE! :S ", Protocol.FontTypeNames.FONTTYPE_PARTY));
                                }
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                Protocol.WriteConsoleMsg(leader, withBlock.name.ToLower() + " ya es miembro de la party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                            }
                            else
                            {
                                Protocol.WriteConsoleMsg(leader, withBlock.name + " ya es miembro de otra party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                            }
                    ...
                    {
                        Protocol.WriteConsoleMsg(leader, "¡Está muerto, no puedes aceptar miembros en ese estado!", Protocol.FontTypeNames.FONTTYPE_PARTY);
                        return;
                    }
                }
                else
                {
                    ...
                    {
                        Protocol.WriteConsoleMsg(leader, withBlock.name.ToLower() + " no ha solicitado ingresar a tu party.", Protocol.FontTypeNames.FONTTYPE_PARTY);
                    }
        ...
        public static void OnlineParty(short UserIndex)
        {
            ...
            if (PI > 0)
            {
                ...
                for (i = 1; i <= PARTY_MAXMEMBERS; i++)
                {
                    ...
                    {
                        Text = Text + " - " + Declaraciones.UserList[MembersOnline[i]].name + " (" + Conversion.Fix(Declaraciones.Parties[PI].MiExperiencia(MembersOnline[i])) + ")";
                    }
                }
        ...
        public static void TransformarEnLider(short OldLeader, short NewLeader)
        {
            ...
            if (PI == Declaraciones.UserList[NewLeader].PartyIndex)
            {
                if (Declaraciones.UserList[NewLeader].flags.Muerto == 0)
                {
                    ...
                    {
                        Declaraciones.Parties[PI].MandarMensajeAConsola("El nuevo líder de la party es " + Declaraciones.UserList[NewLeader].name, Declaraciones.UserList[OldLeader].name);
                    }
                    else
                    {
                        Protocol.WriteConsoleMsg(OldLeader, "¡No se ha hecho el cambio de mando!", Protocol.FontTypeNames.FONTTYPE_PARTY);
                    }
                }
                else
                {
            ...
            {
                Protocol.WriteConsoleMsg(OldLeader, Declaraciones.UserList[NewLeader].name.ToLower() + " no pertenece a tu party.", Protocol.FontTypeNames.FONTTYPE_INFO);
            }
        }
        ...
        public static void ActualizaExperiencias()
        {
            ...
            if (!PARTY_EXPERIENCIAPORGOLPE)
            {
                ...
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Distribuyendo experiencia en parties.", Protocol.FontTypeNames.FONTTYPE_SERVER));
                for (i = 1; i <= MAX_PARTIES; i++)
                {
                ...
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Experiencia distribuida.", Protocol.FontTypeNames.FONTTYPE_SERVER));
                modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessagePauseToggle());
        ...
        public static void ActualizarSumaNivelesElevados(short UserIndex)
        {
            ...
            if (Declaraciones.UserList[UserIndex].PartyIndex > 0)
            {
                Declaraciones.Parties[Declaraciones.UserList[UserIndex].PartyIndex].UpdateSumaNivelesElevados(Declaraciones.UserList[UserIndex].Stats.ELV);
            }
        }
    }
}