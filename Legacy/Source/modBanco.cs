    ...
    static class modBanco
    {
        ...
        public static void UserReciveObj(short UserIndex, short ObjIndex, short Cantidad)
        {
...

            {
                ...
                Slot = 1;
                while (!(withBlock.Invent.Object_Renamed[Slot].ObjIndex == obji & (short)(withBlock.Invent.Object_Renamed[Slot].Amount + Cantidad) <= Declaraciones.MAX_INVENTORY_OBJS))
                ...
                if (Slot > withBlock.CurrentInventorySlots)
                {
                    ...
                    while (withBlock.Invent.Object_Renamed[Slot].ObjIndex != 0)
                    {
                        ...
                        if (Slot > withBlock.CurrentInventorySlots)
                        {
                            Protocol.WriteConsoleMsg(UserIndex, "No podés tener mas objetos.", Protocol.FontTypeNames.FONTTYPE_INFO);
                            return;
                        }
                    }
                ...
                if ((short)(withBlock.Invent.Object_Renamed[Slot].Amount + Cantidad) <= Declaraciones.MAX_INVENTORY_OBJS)
                {
                    ...
                    withBlock.Invent.Object_Renamed[Slot].Amount = (short)(withBlock.Invent.Object_Renamed[Slot].Amount + Cantidad);
        ...
        public static void QuitarBancoInvItem(short UserIndex, byte Slot, short Cantidad)
        {
...

            {
                ...
                withBlock.BancoInvent.Object_Renamed[Slot].Amount = (short)(withBlock.BancoInvent.Object_Renamed[Slot].Amount - Cantidad);
        ...
        public static void UserDejaObj(short UserIndex, short ObjIndex, short Cantidad)
        {
...

            {
                ...
                // ¿Ya tiene un objeto de este tipo?
                Slot = 1;
                while (!(withBlock.BancoInvent.Object_Renamed[Slot].ObjIndex == obji & (short)(withBlock.BancoInvent.Object_Renamed[Slot].Amount + Cantidad) <= Declaraciones.MAX_INVENTORY_OBJS))
                ...
                if (Slot > Declaraciones.MAX_BANCOINVENTORY_SLOTS)
                {
                    ...
                    while (withBlock.BancoInvent.Object_Renamed[Slot].ObjIndex != 0)
                    {
                        ...
                        if (Slot > Declaraciones.MAX_BANCOINVENTORY_SLOTS)
                        {
                            Protocol.WriteConsoleMsg(UserIndex, "No tienes mas espacio en el banco!!", Protocol.FontTypeNames.FONTTYPE_INFO);
                            return;
                        }
                    }
                ...
                {
                    // Mete el obj en el slot
                    if ((short)(withBlock.BancoInvent.Object_Renamed[Slot].Amount + Cantidad) <= Declaraciones.MAX_INVENTORY_OBJS)
                    {
                    ...
                    {
                        Protocol.WriteConsoleMsg(UserIndex, "El banco no puede cargar tantos objetos.", Protocol.FontTypeNames.FONTTYPE_INFO);
                    }
                }
            }
        }
        ...
        public static void SendUserBovedaTxt(short sendIndex, short UserIndex)
        {
            ...
            try
            {
                ...
                Protocol.WriteConsoleMsg(sendIndex, "Tiene " + Declaraciones.UserList[UserIndex].BancoInvent.NroItems + " objetos.", Protocol.FontTypeNames.FONTTYPE_INFO);
                ...
                for (j = 1; j <= Declaraciones.MAX_BANCOINVENTORY_SLOTS; j++)
                {
                    ...
                    {
                        Protocol.WriteConsoleMsg(sendIndex, "Objeto " + j + " " + Declaraciones.ObjData_Renamed[Declaraciones.UserList[UserIndex].BancoInvent.Object_Renamed[j].ObjIndex].name + " Cantidad:" + Declaraciones.UserList[UserIndex].BancoInvent.Object_Renamed[j].Amount, Protocol.FontTypeNames.FONTTYPE_INFO);
                    }
                }
            }
        ...
        public static void SendUserBovedaTxtFromChar(short sendIndex, string charName)
        {
            ...
            try
            {
                ...
                if (System.IO.File.Exists(CharFile))
                {
                    ...
                    Protocol.WriteConsoleMsg(sendIndex, "Tiene " + ES.GetVar(CharFile, "BancoInventory", "CantidadItems", EmptySpaces: ref argEmptySpaces) + " objetos.", Protocol.FontTypeNames.FONTTYPE_INFO);
                    for (j = 1; j <= Declaraciones.MAX_BANCOINVENTORY_SLOTS; j++)
                    {
                        int argEmptySpaces1 = 1024;
                        Tmp = ES.GetVar(CharFile, "BancoInventory", "Obj" + j, EmptySpaces: ref argEmptySpaces1);
