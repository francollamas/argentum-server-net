...
namespace Legacy
{
    static class ModAreas
    {
        ...
        private readonly static byte[,] AreasInfo = new byte[101, 101];
        ...
        private readonly static byte[] PosToArea = new byte[101];

        private readonly static short[] AreasRecive = new short[13];
        ...
        public static void InitAreas()
        {
            ...
            for (LoopC = 0; LoopC <= 11; LoopC++)
                AreasRecive[LoopC] = Convert.ToInt16((long)Math.Round(Math.Pow(Convert.ToInt64(2), LoopC)) | Convert.ToInt64(LoopC != 0 ? (long)Math.Round(Math.Pow(Convert.ToInt64(2), Convert.ToInt64(LoopC - 1))) : 0L) | Convert.ToInt64(LoopC != 11 ? (long)Math.Round(Math.Pow(Convert.ToInt64(2), Convert.ToInt64(LoopC + 1))) : 0L));
            ...
            CurHour = Convert.ToByte(Conversion.Fix(System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateAndTime.TimeOfDay) / 3)); // A ke parte de la hora pertenece
            ...
            for (LoopC = 1; LoopC <= loopTo; LoopC++)
            {
                ...
                ConnGroups[LoopC].OptValue = Convert.ToInt32(Migration.ParseVal(ES.GetVar(Declaraciones.DatPath + "AreasStats.dat", "Mapa" + LoopC, CurDay + "-" + CurHour, EmptySpaces: ref argEmptySpaces)));
        ...
        public static void AreasOptimizacion()
        {
            ...
            if (CurDay != Convert.ToByte(DateAndTime.Weekday(DateTime.Today) > 6 ? 1 : 2) | CurHour != Convert.ToByte(Conversion.Fix(System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateAndTime.TimeOfDay) / 3)))
            {

                tCurDay = Convert.ToByte(DateAndTime.Weekday(DateTime.Today) > 6 ? 1 : 2); // A ke tipo de dia pertenece?
                tCurHour = Convert.ToByte(Conversion.Fix(System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetHour(DateAndTime.TimeOfDay) / 3)); // A ke parte de la hora pertenece

                var loopTo = (int)Declaraciones.NumMaps;
                for (LoopC = 1; LoopC <= loopTo; LoopC++)
                {
                    int argEmptySpaces = 1024;
                    EntryValue = Convert.ToInt32(Migration.ParseVal(ES.GetVar(Declaraciones.DatPath + "AreasStats.dat", "Mapa" + LoopC, CurDay + "-" + CurHour, EmptySpaces: ref argEmptySpaces)));
                    ES.WriteVar(Declaraciones.DatPath + "AreasStats.dat", "Mapa" + LoopC, CurDay + "-" + CurHour, Convert.ToInt16((EntryValue + ConnGroups[LoopC].OptValue) / 2).ToString());

                    int argEmptySpaces1 = 1024;
                    ConnGroups[LoopC].OptValue = Convert.ToInt32(Migration.ParseVal(ES.GetVar(Declaraciones.DatPath + "AreasStats.dat", "Mapa" + LoopC, tCurDay + "-" + tCurHour, EmptySpaces: ref argEmptySpaces1)));
                    if (ConnGroups[LoopC].OptValue == 0)
                        ConnGroups[LoopC].OptValue = 1;
                    // UPGRADE_WARNING: El límite inferior de la matriz ConnGroups(LoopC).UserEntrys ha cambiado de 1 a 0. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="0F1C9BE1-AF9D-476E-83B1-17D43BECFF20"'
...

            {
                ...
                for (X = MinX; X <= loopTo; X++)
                {
                    ...
                    for (Y = MinY; Y <= loopTo1; Y++)
                    {
                        ...
                        if (Declaraciones.MapData[Map, X, Y].UserIndex != 0)
                        {
                            ...
                            if (UserIndex != TempInt)
                            {
                                ...
                                if (!(Declaraciones.UserList[TempInt].flags.AdminInvisible == 1))
                                {
                                    ...
                                    UsUaRiOs.MakeUserChar(false, Convert.ToInt16(UserIndex), Convert.ToInt16(TempInt), Convert.ToInt16(Map), Convert.ToInt16(X), Convert.ToInt16(Y), ButIndex: ref argButIndex);

                                    // Si el user estaba invisible le avisamos al nuevo cliente de eso
                                    if (Declaraciones.UserList[TempInt].flags.invisible != 0 | Declaraciones.UserList[TempInt].flags.Oculto != 0)
                                    {
                                        ...
                                        {
                                            Protocol.WriteSetInvisible(UserIndex, Declaraciones.UserList[TempInt].Char_Renamed.CharIndex, true);
                                        }
                                    }
                                }
                                ...
                                {
                                    bool argButIndex1 = false;
                                    UsUaRiOs.MakeUserChar(false, Convert.ToInt16(TempInt), UserIndex, withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y, ButIndex: ref argButIndex1);

                                    if (withBlock.flags.invisible != 0 | withBlock.flags.Oculto != 0)
                                    {
                                        if ((Declaraciones.UserList[TempInt].flags.Privilegios & Declaraciones.PlayerType.User) != 0)
                                        {
                                            Protocol.WriteSetInvisible(Convert.ToInt16(TempInt), withBlock.Char_Renamed.CharIndex, true);
                                        }
                                    }
                                }
                            ...
                            else if (Head == USER_NUEVO)
                            {
                                if (!ButIndex)
                                {
                                    bool argButIndex2 = false;
                                    UsUaRiOs.MakeUserChar(false, Convert.ToInt16(UserIndex), Convert.ToInt16(UserIndex), Convert.ToInt16(Map), Convert.ToInt16(X), Convert.ToInt16(Y), ButIndex: ref argButIndex2);
                                }
                            }
                        }
                        ...
                        {
                            short argsndIndex = Convert.ToInt16(UserIndex);
                            NPCs.MakeNPCChar(false, ref argsndIndex, ref Declaraciones.MapData[Map, X, Y].NpcIndex, Convert.ToInt16(Map), Convert.ToInt16(X), Convert.ToInt16(Y));
                        }
                        ...
                        if (Declaraciones.MapData[Map, X, Y].ObjInfo.ObjIndex != 0)
                        {
                            ...
                            {
                                Protocol.WriteObjectCreate(UserIndex, Declaraciones.ObjData_Renamed[TempInt].GrhIndex, Convert.ToByte(X), Convert.ToByte(Y));

                                if (Declaraciones.ObjData_Renamed[TempInt].OBJType == Declaraciones.eOBJType.otPuertas)
                                {
                                    General.Bloquear(false, UserIndex, Convert.ToInt16(X), Convert.ToInt16(Y), Declaraciones.MapData[Map, X, Y].Blocked != 0);
                                    General.Bloquear(false, UserIndex, Convert.ToInt16(X - 1), Convert.ToInt16(Y), Declaraciones.MapData[Map, X - 1, Y].Blocked != 0);
                                }
                            }
                        }
                    ...
                    for (X = MinX; X <= loopTo; X++)
                    {
                        ...
                        for (Y = MinY; Y <= loopTo1; Y++)
                        {
                            if (Declaraciones.MapData[withBlock.Pos.Map, X, Y].UserIndex != 0)
                                NPCs.MakeNPCChar(false, ref Declaraciones.MapData[withBlock.Pos.Map, X, Y].UserIndex, ref NpcIndex, withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y);
                        }
                    }
                }
