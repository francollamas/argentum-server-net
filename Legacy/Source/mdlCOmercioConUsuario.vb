        ...
        For OfferSlot = 1 To MAX_OFFER_SLOTS + 1
            ...
            With UserList(UserIndex)
                ...
                If OfferSlot = GOLD_OFFER_SLOT Then
                    ...
                    ' Log
                    If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then _
                        Call _
                            LogDesarrollo(
                                .name & " soltó oro en comercio seguro con " & UserList(OtroUserIndex).name &
                                ". Cantidad: " & .ComUsu.GoldAmount)
                    ' Update Usuario
                ...
                ElseIf .ComUsu.Objeto(OfferSlot) > 0 Then
                    ...
                    If ObjData_Renamed(TradingObj.ObjIndex).Log = 1 Then
                        Call _
                            LogDesarrollo(
                                .name & " le pasó en comercio seguro a " & UserList(OtroUserIndex).name & " " &
                                TradingObj.Amount & " " & ObjData_Renamed(TradingObj.ObjIndex).name)
                    End If
                    ...
                    If TradingObj.Amount > MAX_OBJ_LOGUEABLE Then
                        ...
                        If ObjData_Renamed(TradingObj.ObjIndex).NoLog <> 1 Then
                            Call _
                                LogDesarrollo(
                                    UserList(OtroUserIndex).name & " le pasó en comercio seguro a " & .name & " " &
                                    TradingObj.Amount & " " & ObjData_Renamed(TradingObj.ObjIndex).name)
                        End If
                    End If
                End If
            ...
            With UserList(OtroUserIndex)
                ...
                If OfferSlot = GOLD_OFFER_SLOT Then
                    ...
                    If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then _
                        Call _
                            LogDesarrollo(
                                .name & " soltó oro en comercio seguro con " & UserList(UserIndex).name & ". Cantidad: " &
                                .ComUsu.GoldAmount)
                    ' Update Usuario
                    Call WriteUpdateUserStats(OtroUserIndex)
                    'y se la doy al otro
                    ...
                    If .ComUsu.GoldAmount > MAX_ORO_LOGUEABLE Then _
                        Call _
                            LogDesarrollo(
                                UserList(UserIndex).name & " recibió oro en comercio seguro con " & .name &
                                ". Cantidad: " & .ComUsu.GoldAmount)
                    ' Update Otro Usuario
                    Call WriteUpdateUserStats(UserIndex)
                ...
                ElseIf .ComUsu.Objeto(OfferSlot) > 0 Then
                    ...
                    If ObjData_Renamed(TradingObj.ObjIndex).Log = 1 Then
                        Call _
                            LogDesarrollo(
                                .name & " le pasó en comercio seguro a " & UserList(UserIndex).name & " " &
                                TradingObj.Amount & " " & ObjData_Renamed(TradingObj.ObjIndex).name)
                    End If
