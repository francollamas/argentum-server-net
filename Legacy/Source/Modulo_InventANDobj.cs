...
internal static class InvNpc
{
    // Modulo Inv & Obj
    // Modulo para controlar los objetos y los inventarios.
    public static Declaraciones.WorldPos TirarItemAlPiso(ref Declaraciones.WorldPos Pos, ref Declaraciones.Obj Obj,
        [Optional] [DefaultParameterValue(true)] ref bool NotPirata)
    {
        ...
        try
        {
            ...
            if ((NuevaPos.X != 0) & (NuevaPos.Y != 0)) InvUsuario.MakeObj(ref Obj, Pos.Map, NuevaPos.X, NuevaPos.Y);
    ...
    public static short EncontrarCant(short NpcIndex, short ObjIndex)
    {
        ...
        try
        {
            ...
            for (i = 1; i <= Declaraciones.MAX_INVENTORY_SLOTS; i++)
            {
                ...
                ln = ES.GetVar(npcfile, "NPC" + Declaraciones.Npclist[NpcIndex].Numero, "Obj" + i, ref argEmptySpaces);
    ...
    public static void CargarInvent(short NpcIndex)
    {
...

        {
            ref var withBlock = ref Declaraciones.Npclist[NpcIndex];
            var argEmptySpaces = 1024;
            withBlock.Invent.NroItems = Convert.ToInt16(Migration.ParseVal(ES.GetVar(npcfile, "NPC" + withBlock.Numero,
                "NROITEMS", ref argEmptySpaces)));
    ...
    public static void TirarOroNpc(int Cantidad, ref Declaraciones.WorldPos Pos)
    {
        ...
        try
        {
            ...
            if (Cantidad > 0)
            {
                ...
                while (RemainingGold > 0)
                {
                    ...
                    var argNotPirata = true;
                    TirarItemAlPiso(ref Pos, ref MiObj, ref argNotPirata);
                }
            }
        }
        ...
        catch (Exception ex)
        {
            ...
            var argdesc = "Error en TirarOro. Error " + ex.GetType().Name + " : " + ex.Message;
