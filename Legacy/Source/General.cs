...
namespace Legacy
{
    ...
    {
        internal static clsIniReader LeerNPCs = new clsIniReader();
    ...
    /// </summary>
        ...
        public static void Bloquear(bool toMap, short sndIndex, short X, short Y, bool b)
        {
            ...
            if (toMap)
            {
                modSendData.SendData(modSendData.SendTarget.toMap, sndIndex, Protocol.PrepareMessageBlockPosition(Convert.ToByte(X), Convert.ToByte(Y), b));
            }
            else
            {
            ...
            {
                {
                    ...
                    if ((withBlock.Graphic[1] >= 1505 & withBlock.Graphic[1] <= 1520 | withBlock.Graphic[1] >= 5665 & withBlock.Graphic[1] <= 5680 | withBlock.Graphic[1] >= 13547 & withBlock.Graphic[1] <= 13562) & withBlock.Graphic[2] == 0)
                    {
                        HayAguaRet = true;
                    }
                    else
        ...
        private static bool HayLava(short Map, short X, short Y)
        {
            ...
            if (Map > 0 & Map < Declaraciones.NumMaps + 1 & X > 0 & X < 101 & Y > 0 & Y < 101)
            {
                if (Declaraciones.MapData[Map, X, Y].Graphic[1] >= 5837 & Declaraciones.MapData[Map, X, Y].Graphic[1] <= 5852)
                {
                    HayLavaRet = true;
                }
                else
        ...
        public static void LimpiarMundo()
        {
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error producido en el sub LimpiarMundo: " + ex.Message;
        ...
        public static string ReadField(short Pos, ref string Text, byte SepASCII)
        {
            ...
            {
                LastPos = CurrentPos;
                int idx = Text.IndexOf(delimiter, LastPos, StringComparison.Ordinal);
        ...
        {
            AppendLog("logs/Eventos.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + desc);
        }
        ...
        {
            AppendLog("logs/" + Index + ".log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + desc);
        }


        public static void LogError(ref string desc)
        {
            AppendLog("logs/errores.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + desc);
        }

        public static void LogStatic(ref string desc)
        {
            AppendLog("logs/Stats.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + desc);
        }

        public static void LogTarea(ref string desc)
        {
            AppendLog("logs/haciendo.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + desc);
        }


        public static void LogClanes(string str_Renamed)
        {
            AppendLog("logs/clanes.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + str_Renamed);
        }

        public static void LogIP(string str_Renamed)
        {
            AppendLog("logs/IP.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + str_Renamed);
        }


        public static void LogDesarrollo(string str_Renamed)
        {
            AppendLog("logs/desarrollo" + System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetMonth(DateTime.Today) + System.Threading.Thread.CurrentThread.CurrentCulture.Calendar.GetYear(DateTime.Today) + ".log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + str_Renamed);
        }

        public static void LogGM(ref string Nombre, ref string texto)
        {
            AppendLog("logs/" + Nombre + ".log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
        }

        public static void LogAsesinato(ref string texto)
        {
            AppendLog("logs/asesinatos.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
        }

        public static void logVentaCasa(string texto)
        {
            AppendLog("logs/propiedades.log", "----------------------------------------------------------");
            AppendLog("logs/propiedades.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
            AppendLog("logs/propiedades.log", "----------------------------------------------------------");
        }
        ...
        public static void LogHackAttemp(ref string texto)
        {
            ...
            AppendLog("logs/HackAttemps.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
            AppendLog("logs/HackAttemps.log", "----------------------------------------------------------");
        }

        public static void LogCheating(ref string texto)
        {
            AppendLog("logs/CH.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
        }
        ...
        public static void LogCriticalHackAttemp(ref string texto)
        {
            ...
            AppendLog("logs/CriticalHackAttemps.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
            AppendLog("logs/CriticalHackAttemps.log", "----------------------------------------------------------");
        }

        public static void LogAntiCheat(ref string texto)
        {
            AppendLog("logs/AntiCheat.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " " + texto);
        ...
        public static void Restart()
        {
            ...
            try
            {
                ...
                // Log it
                AppendLog("logs/Main.log", Conversions.ToString(DateTime.Today) + " " + Conversions.ToString(DateAndTime.TimeOfDay) + " servidor reiniciado.");
            }
        ...
        public static bool Intemperie(short UserIndex)
        {
...

            {
                ...
                {
                    if ((int)Declaraciones.MapData[withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y].trigger != 1 & (int)Declaraciones.MapData[withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y].trigger != 2 & (int)Declaraciones.MapData[withBlock.Pos.Map, withBlock.Pos.X, withBlock.Pos.Y].trigger != 4)
                        IntemperieRet = true;
                }
                else
                {
            ...
            {
                {
                    ...
                    if (withBlock.MascotasIndex[i] > 0)
                    {
                        ...
                        {
                            Declaraciones.Npclist[withBlock.MascotasIndex[i]].Contadores.TiempoExistencia = Declaraciones.Npclist[withBlock.MascotasIndex[i]].Contadores.TiempoExistencia - 1;
                            if (Declaraciones.Npclist[withBlock.MascotasIndex[i]].Contadores.TiempoExistencia == 0)
        ...
        public static void EfectoFrio(short UserIndex)
        {
...

            {
                ...
                else
                {
                    ...
                    {
                        Protocol.WriteConsoleMsg(UserIndex, "¡¡Estás muriendo de frío, abrigate o morirás!!", Protocol.FontTypeNames.FONTTYPE_INFO);
                        ...
                        {
                            Protocol.WriteConsoleMsg(UserIndex, "¡¡Has muerto de frío!!", Protocol.FontTypeNames.FONTTYPE_INFO);
        ...
        public static void EfectoLava(short UserIndex)
        {
            ...
            // ***************************************************
            {
                ...
                else
                {
                    ...
                    {
                        Protocol.WriteConsoleMsg(UserIndex, "¡¡Quitate de la lava, te estás quemando!!", Protocol.FontTypeNames.FONTTYPE_INFO);
                        withBlock.Stats.MinHp = (short)(withBlock.Stats.MinHp - Convert.ToInt16(Matematicas.Porcentaje(withBlock.Stats.MaxHp, 5)));

                        if (withBlock.Stats.MinHp < 1)
                        {
                            Protocol.WriteConsoleMsg(UserIndex, "¡¡Has muerto quemado!!", Protocol.FontTypeNames.FONTTYPE_INFO);
                            withBlock.Stats.MinHp = 0;
                            UsUaRiOs.UserDie(UserIndex);
                        }
        ...
        public static void EfectoMimetismo(short UserIndex)
        {
...

            {
                ...
                {
                    // restore old char
                    Protocol.WriteConsoleMsg(UserIndex, "Recuperas tu apariencia normal.", Protocol.FontTypeNames.FONTTYPE_INFO);
                    ...
                    if (withBlock.flags.Navegando != 0)
                    {
                        if (withBlock.flags.Muerto == 0)
                        {
                            ...
                            else
                            {
                                ...
                                Barco = Declaraciones.ObjData_Renamed[Declaraciones.UserList[UserIndex].Invent.BarcoObjIndex];
...

                    {
                        ...
                        UsUaRiOs.ChangeUserChar(UserIndex, withBlock1.body, withBlock1.Head, (byte)withBlock1.heading, withBlock1.WeaponAnim, withBlock1.ShieldAnim, withBlock1.CascoAnim);
                    }
        ...
        public static void EfectoInvisibilidad(short UserIndex)
        {
...

            {
                ...
                {
                    withBlock.Counters.Invisibilidad = Convert.ToInt16(Matematicas.RandomNumber(-100, 100)); // Invi variable :D
            ...
            short massta;
            {
                ...
                if (withBlock.Stats.MinSta < withBlock.Stats.MaxSta)
                {
                    ...
                    else
                    {
                        ...
                        massta = Convert.ToInt16(Matematicas.RandomNumber(1, Matematicas.Porcentaje(withBlock.Stats.MaxSta, 5)));
        ...
        public static void EfectoVeneno(short UserIndex)
        {
...

            {
                ...
                {
                    Protocol.WriteConsoleMsg(UserIndex, "Estás envenenado, si no te curas morirás.", Protocol.FontTypeNames.FONTTYPE_VENENO);
        ...
        public static void Sanar(short UserIndex, ref bool EnviarStats, short Intervalo)
        {
            ...
            short mashit;
            {
                ...
                if (withBlock.Stats.MinHp < withBlock.Stats.MaxHp)
                {
                    ...
                    {
                        mashit = Convert.ToInt16(Matematicas.RandomNumber(2, Matematicas.Porcentaje(withBlock.Stats.MaxSta, 5)));
        ...
        public static void PasarSegundo()
        {
            ...
            try
            {
                ...
                for (i = 1; i <= loopTo; i++)
                {
                    if (Declaraciones.UserList[i].flags.UserLogged)
                    {
                        ...
                        {
                            Declaraciones.UserList[i].Counters.salir = Convert.ToInt16(Declaraciones.UserList[i].Counters.salir - 1);
                            if (Declaraciones.UserList[i].Counters.salir <= 0)
                            {
                                Protocol.WriteConsoleMsg(Convert.ToInt16(i), "Gracias por jugar Argentum Online", Protocol.FontTypeNames.FONTTYPE_INFO);
            ...
            catch (Exception ex)
            {
                ...
                string argdesc = "Error en PasarSegundo. Err: " + ex.Message + " - " + ex.GetType().Name + " - UserIndex: " + i;
                LogError(ref argdesc);
            }
        }
        ...
        public static double ReiniciarAutoUpdate()
        {
            ...
            ReiniciarAutoUpdateRet = Interaction.Shell(AppDomain.CurrentDomain.BaseDirectory + "autoupdater/aoau.exe", AppWinStyle.MinimizedNoFocus);
        ...
        public static void GuardarUsuarios()
        {
            ...
            modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Grabando Personajes", Protocol.FontTypeNames.FONTTYPE_SERVER));
            ...
            modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessageConsoleMsg("Servidor> Personajes Grabados", Protocol.FontTypeNames.FONTTYPE_SERVER));
            modSendData.SendData(modSendData.SendTarget.ToAll, 0, Protocol.PrepareMessagePauseToggle());
        ...
        public static void InicializaEstadisticas()
        {
            ...
            Admin.EstadisticasWeb.Informar(clsEstadisticasIPC.EstaNotificaciones.UPTIME_SERVER, Convert.ToInt32((Ta - Admin.tInicioServer) / 1000d));
